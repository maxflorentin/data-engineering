...
CREATE TEMPORARY TABLE bi_corp_staging.zror_rosetta_key AS
	...
	LEFT JOIN bi_corp_common.dim_ren_areanegocioctr adn2 ON 
		TRIM(RE.cod_ren_area_negocio) = TRIM(adn2.cod_ren_area_negocio_niv_9) 

	LEFT JOIN bi_corp_staging.zror_producto_mis pm ON 
		TRIM(RE.cod_productogest) = TRIM(PM.cod_producto_gest)

	LEFT JOIN bi_corp_common.dim_ren_jeareanegocioctr adn ON 
		TRIM(RE.cod_ren_area_negocio) = TRIM(adn.cod_ren_areanegociohijo) ;
...
SELECT COUNT(1) FROM bi_corp_staging.zror_activo_01 WHERE cod_ren_area_negociocorp IS NULL ;

SELECT DISTINCT cod_ren_area_negocio FROM bi_corp_staging.zror_activo_01 ;

SELECT COUNT(1) FROM bi_corp_common.bt_ror_input_activo WHERE partition_date = '2020-12-31' AND cod_ren_areanegociocorp IS NOT NULL ;

SELECT COUNT(1) FROM bi_corp_staging.zror_perimetro_activo WHERE cod_ren_area_negocio  IS NULL ;

SELECT * FROM bi_corp_common.dim_ren_jeareanegocioctr WHERE cod_ren_areanegociohijo = 'IPRGE1ORMP'

set mapred.job.queue.name=root.dataeng ;
SELECT RE.cod_ren_area_negocio
	, adn.cod_ren_primercorporativo  cod_ren_area_negociocorp 
...
WHERE adn.cod_ren_primercorporativo IS NULL ;

cod_ren_areanegociohijo cod_ren_areanegociohijo
