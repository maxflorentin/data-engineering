...
SELECT da.dt_tie_date 
	...
	, date_add(da.dt_tie_date, 1) dt_tie_datenextdate
...
LEFT JOIN calendar nw1 
	ON ca.dt_tie_datenextdate = nw1.dt_tie_date
	AND nw1.flag_tie_workingday = 1
LEFT JOIN calendar nw2 
	ON date_add(ca.dt_tie_datenextdate, 1) = nw2.dt_tie_date
	AND nw2.flag_tie_workingday = 1
LEFT JOIN calendar nw3 
	ON date_add(ca.dt_tie_datenextdate, 2) = nw3.dt_tie_date
	AND nw3.flag_tie_workingday = 1
LEFT JOIN calendar nw4 
	ON date_add(ca.dt_tie_datenextdate, 3) = nw4.dt_tie_date
	AND nw4.flag_tie_workingday = 1
LEFT JOIN calendar nw5 
	ON date_add(ca.dt_tie_datenextdate, 4) = nw5.dt_tie_date
...
SELECT * FROM bi_corp_common.dim_tie_calendario WHERE dt_tie_date IN  ('2020-11-20','2020-11-21','2020-11-22','2020-11-23','2020-11-24')

SELECT * FROM bi_corp_staging.malbgc_bgdtmoh 
	
SHOW PARTITIONS bi_corp_staging.malbgc_bgdtmoh ;
...
LEFT JOIN calendar wm 
	...
	AND wm.flag_tie_workingday = 1
-- previous worwing date
LEFT JOIN bi_corp_common.dim_tie_calendario lw1 
	ON ca.dt_tie_datepreviousdate = lw1.dt_tie_date
	AND lw1.flag_tie_workingday = 1
LEFT JOIN bi_corp_common.dim_tie_calendario lw2 
	ON date_add(ca.dt_tie_datepreviousdate, -1) = lw2.dt_tie_date
	AND lw2.flag_tie_workingday = 1
LEFT JOIN bi_corp_common.dim_tie_calendario lw3 
	ON date_add(ca.dt_tie_datepreviousdate, -2) = lw3.dt_tie_date
	AND lw3.flag_tie_workingday = 1
LEFT JOIN bi_corp_common.dim_tie_calendario lw4 
	ON date_add(ca.dt_tie_datepreviousdate, -3) = lw4.dt_tie_date
	AND lw4.flag_tie_workingday = 1
LEFT JOIN bi_corp_common.dim_tie_calendario lw5 
	ON date_add(ca.dt_tie_datepreviousdate, -4) = lw5.dt_tie_date
	AND lw5.flag_tie_workingday = 1
-- next working date
LEFT JOIN bi_corp_common.dim_tie_calendario nw1 
	ON ca.dt_tie_datenextdate = nw1.dt_tie_date
	AND nw1.flag_tie_workingday = 1
LEFT JOIN bi_corp_common.dim_tie_calendario nw2 
	ON date_add(ca.dt_tie_datenextdate, 1) = nw2.dt_tie_date
	AND nw2.flag_tie_workingday = 1
LEFT JOIN bi_corp_common.dim_tie_calendario nw3 
	ON date_add(ca.dt_tie_datenextdate, 2) = nw3.dt_tie_date
	AND nw3.flag_tie_workingday = 1
LEFT JOIN bi_corp_common.dim_tie_calendario nw4 
	ON date_add(ca.dt_tie_datenextdate, 3) = nw4.dt_tie_date
	AND nw4.flag_tie_workingday = 1
LEFT JOIN bi_corp_common.dim_tie_calendario nw5 
	ON date_add(ca.dt_tie_datenextdate, 4) = nw5.dt_tie_date
	AND nw5.flag_tie_workingday = 1 
WHERE ca.int_tie_year = 2020 AND ca.dt_tie_date IN ('2020-11-20','2020-11-21','2020-11-22','2020-11-23','2020-11-24')
ORDER BY ca.dt_tie_date ;
	




SELECT date_add(ca.dt_tie_date, 1) ,  ca.dt_tie_date
FROM bi_corp_common.dim_tie_calendario ca
WHERE ca.dt_tie_date IN ('2020-11-20','2020-11-21','2020-11-22','2020-11-23','2020-11-24')



-- dt_tie_date|dt_tie_nextworkingdate|
-- -----------|----------------------|
...
-- -----------|----------------------|
SELECT ca.dt_tie_date , dt_tie_nextworkingdate
FROM bi_corp_common.dim_tie_calendario ca
WHERE ca.dt_tie_date IN ('2020-11-20','2020-11-21','2020-11-22','2020-11-23','2020-11-24')
