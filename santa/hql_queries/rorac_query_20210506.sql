...
CREATE TEMPORARY TABLE activo_idru AS
	SELECT RE.cod_ren_unidad
		...
		, IF(SUBSTRING(RE.cod_ren_contrato_rorac, 1, 3) = 'CCL' AND SUM(RE.fc_ren_sdbsmv) != 0, 1, IF(SUBSTRING(RE.cod_ren_contrato_rorac, 1, 3) = 'CCL', 2, IF(SUBSTRING(RE.cod_ren_contrato_rorac, 1, 3) = 'CTB' AND SUM(RE.fc_ren_sdbsmv) < 0, 2, 0))) flag_ren_neteo 
		, RE.fc_ren_orex
		, RE.fc_ifrs_provision
		, RE.fc_ren_costemensualcto
		, RE.cod_nivel_operacion
		, CASE WHEN SUBSTRING(RE.cod_ren_contrato_rorac, 1, 3) IN ('CCL','CTB') AND RE.cod_ren_areanegociocorp = 'AN01020210' AND RE.cod_ren_divisa_mis = 'ARS' THEN 1200201
			  WHEN SUBSTRING(RE.cod_ren_contrato_rorac, 1, 3) IN ('CCL','CTB') AND RE.cod_ren_areanegociocorp = 'AN01020210' AND RE.cod_ren_divisa_mis != 'ARS' THEN 1200649
			  WHEN SUBSTRING(RE.cod_ren_contrato_rorac, 1, 3) IN ('CCL','CTB') AND RE.cod_ren_areanegociocorp = 'AN01040220' AND RE.cod_ren_divisa_mis = 'ARS' THEN 1200233
			  WHEN SUBSTRING(RE.cod_ren_contrato_rorac, 1, 3) IN ('CCL','CTB') AND RE.cod_ren_areanegociocorp = 'AN01040220' AND RE.cod_ren_divisa_mis != 'ARS' THEN 1200681
			  WHEN SUBSTRING(RE.cod_ren_contrato_rorac, 1, 3) IN ('CCL','CTB') AND RE.cod_ren_areanegociocorp = 'AN01020410' AND RE.cod_ren_divisa_mis = 'ARS' THEN 1200137
			  WHEN SUBSTRING(RE.cod_ren_contrato_rorac, 1, 3) IN ('CCL','CTB') AND RE.cod_ren_areanegociocorp = 'AN01020410' AND RE.cod_ren_divisa_mis != 'ARS' THEN 1200585
			  WHEN SUBSTRING(RE.cod_ren_contrato_rorac, 1, 3) IN ('CCL','CTB') AND RE.cod_ren_areanegociocorp = 'AN01020200' AND RE.cod_ren_divisa_mis = 'ARS' THEN 1200169
			  WHEN SUBSTRING(RE.cod_ren_contrato_rorac, 1, 3) IN ('CCL','CTB') AND RE.cod_ren_areanegociocorp = 'AN01020200' AND RE.cod_ren_divisa_mis != 'ARS' THEN 1200617
			  WHEN SUBSTRING(RE.cod_ren_contrato_rorac, 1, 3) IN ('CCL','CTB') AND RE.cod_ren_areanegociocorp IN ('AN01010500','AN01010201') THEN 1200425
	...
	GROUP BY RE.cod_ren_unidad
		...
		, CASE WHEN SUBSTRING(RE.cod_ren_contrato_rorac, 1, 3) IN ('CCL','CTB') AND RE.cod_ren_areanegociocorp = 'AN01020210' AND RE.cod_ren_divisa_mis = 'ARS' THEN 1200201
			  ...
			  WHEN SUBSTRING(RE.cod_ren_contrato_rorac, 1, 3) IN ('CCL','CTB') AND RE.cod_ren_areanegociocorp = 'AN01020200' AND RE.cod_ren_divisa_mis != 'ARS' THEN 1200617
			  WHEN SUBSTRING(RE.cod_ren_contrato_rorac, 1, 3) IN ('CCL','CTB') AND RE.cod_ren_areanegociocorp IN ('AN01010500','AN01010201') THEN 1200425
			ELSE COALESCE(RU.id_ru, '9999999') END
		, RE.partition_date ;	
...
CREATE TEMPORARY TABLE pasivo AS
	SELECT cod_ren_unidad ,
		dt_ren_data ,
		cod_ren_finalidaddatos ,
		CONCAT(cod_ren_contrato_rorac, 'P') cod_ren_contrato_rorac ,
		cod_ren_contrato ,
		cod_ren_areanegocio ,
		cod_ren_divisa ,
		cod_ren_division ,
		...
		IF(SUBSTRING(cod_ren_contrato_rorac, 1, 3) = 'CCL' AND SUM(fc_ren_sdbsmv) != 0, 1, IF(SUBSTRING(cod_ren_contrato_rorac, 1, 3) = 'CCL', 2, IF(SUBSTRING(cod_ren_contrato_rorac, 1, 3) = 'CTB' AND SUM(fc_ren_sdbsmv) < 0, 2, 0))) flag_ren_neteo ,
		fc_ren_orex ,
		fc_ifrs_provision ,
...
-- select flag_ren_neteo , count(*) from bi_corp_common.bt_ror_input_pasivo RE WHERE RE.partition_date = '2020-12-31' GROUP BY flag_ren_neteo ;

-- select * from bi_corp_common.bt_ror_input_activo RE WHERE RE.partition_date = '2020-12-31'





SELECT * FROM  bi_corp_staging.malbgc_zbdtmig mig
WHERE mig.partition_date = '2021-05-12'
AND CAST(substring(mig.mig_new_cuenta, 4, 9) AS BIGINT) = 3890330


SHOW PARTITIONS bi_corp_bdr.jm_trz_cont_ren

CREATE TEMPORARY TABLE migracion_contratos as
select
	r.mig_old_entidad,
	r.mig_old_cent_alta,
	r.mig_old_cuenta,
	...
	r.mig_old_fech_baja,
	r.mig_new_divisa,
	row_number() over( partition by r.mig_old_entidad,
	r.mig_old_cent_alta,
	r.mig_old_cuenta,
	p_old.pecodpro,
	trim(p_old.pecodsub),
	...
	trim(p_new.pecodsub)
order by
	r.mig_new_divisa ) as rownum
from
	bi_corp_staging.malbgc_zbdtmig r
LEFT JOIN bi_corp_staging.malpe_pedt008 p_old on
	p_old.pecodent = r.mig_old_entidad
	and p_old.pecodofi = r.mig_old_cent_alta
	and concat("0", p_old.pecodpro, substring(p_old.penumcon, 4) ) = r.mig_old_cuenta
	and p_old.partition_date = '2021-05-12'
LEFT JOIN bi_corp_staging.malpe_pedt008 p_new on
	p_new.pecodent = r.mig_new_entidad
	and p_new.pecodofi = r.mig_new_cent_alta
	and concat("0", p_new.pecodpro, substring(p_new.penumcon, 4) ) = r.mig_new_cuenta
	and p_new.partition_date = '2021-05-12'
WHERE
	r.partition_date = '2021-05-12'
	AND p_old.partition_date = '2021-05-12'
	AND p_new.partition_date = '2021-05-12'
	AND SUBSTRING(r.mig_old_fech_baja, 1, 7) = '2021-05' 
AND ; 
...
SELECT cod_ren_contenido, COUNT(1)
FROM bi_corp_common.bt_ren_result
WHERE partition_date = '2020-12-31'
AND cod_ren_producto_gest = 'R-01IM002' 
GROUP BY cod_ren_contenido
