...
SELECT con.* FROM 
(
SELECT CASE WHEN h.id_cto_bdr IS NOT NULL THEN h.id_cto_bdr 
    ELSE lpad(CAST(COALESCE(LAG(h.id_cto_bdr) OVER (PARTITION BY h.id_cto_bdr),0) + ROW_NUMBER() OVER() AS INT), 9, '0') 
    END id_cto_bdr
, d.*
...
FROM (
    ...
    ) d LEFT JOIN bi_corp_bdr.normalizacion_id_contratos_history h 
    ON h.id_cto_source = d.id_cto_source
)con LEFT JOIN bi_corp_bdr.normalizacion_id_contratos_history his
    ON con.id_cto_source = his.id_cto_source
    AND his.id_cto_bdr IS NULL;
