...
WHERE a.data_date_part = '20191129'
...
AND core.PEESTOPE = 'C'
AND concat(substr(core.PEFECEST,1,4),substr(core.PEFECEST,6,2)) = substr('20191130',1,6)
UNION ALL
SELECT 'credito' source
, concat_ws('_', a.cod_entidad, a.cod_centro , a.num_cuenta, a.cod_producto, a.cod_subprodu_altair , a.cod_divisa) id_cto_source
, a.cod_entidad cred_cod_entidad
...
, '' mmff_cod_pata, '' blc_sociedad, '' blc_cargabal, '' blc_sociedad_eliminacion
FROM santander_business_risk_arda.contratos a
WHERE a.data_date_part = '20191129' AND a.cod_marca = "FA" AND substr(a.fec_castigo,1,7) = '2019-11'
UNION ALL
...
SELECT
lpad(CAST(COALESCE(LAG(h.id_cto_bdr) OVER (PARTITION BY h.id_cto_bdr),0) + ROW_NUMBER() OVER() AS INT), 9, '0') as id_cto_bdr,
d.*,
...
LEFT JOIN bi_corp_bdr.normalizacion_id_contratos_history h ON h.id_cto_source = d.id_cto_source
WHERE h.id_cto_source is null;


INSERT OVERWRITE TABLE bi_corp_bdr.normalizacion_id_contratos_test
PARTITION ( partition_date = '2019-11' )
SELECT
hs.id_cto_bdr,
hs.source,
hs.id_cto_source,
...
[2020-04-15 17:56:20,755] {logging_mixin.py:95} INFO - [2020-04-15 17:56:20,753] {base_hook.py:83} INFO - Using connection to: id: cloudera_hive_beeline. Host: hive-prod.ar.bsch, Port: 10000, Schema: default;ssl=true;sslTrustStore=/opt/cloudera/security/truststore/cdh.truststore;trustStorePassword=Feriamundial7019, Login: None, Password: None, extra: {'use_beeline': True, 'auth': '', 'principal': 'hive/hive-prod.ar.bsch@RIO.AR.BSCH'}
[2020-04-15 17:56:20,758] {logging_mixin.py:95} INFO - [2020-04-15 17:56:20,758] {zonda_hive_cli_hook.py:251} INFO - beeline -u "jdbc:hive2://hive-prod.ar.bsch:10000/default;ssl=true;sslTrustStore=/opt/cloudera/security/truststore/cdh.truststore;trustStorePassword=Feriamundial7019;principal=hive/hive-prod.ar.bsch@RIO.AR.BSCH;" -hiveconf airflow.ctx.dag_id= -hiveconf airflow.ctx.task_id= -hiveconf airflow.ctx.execution_date= -hiveconf airflow.ctx.dag_run_id= -e set hive.merge.mapfiles=true;
