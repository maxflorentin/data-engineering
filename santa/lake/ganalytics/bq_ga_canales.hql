...
(SELECT MAX(IF (custom.index = 16, custom.value,  NULL)) FROM UNNEST(H.customDimensions) AS custom) AS Identification_Type,
...
REPLACE(H.page.pageTitle,'|',';') AS Page_Title,
...
UNNEST(hits) AS H
WHERE H.dataSource = "web"
ORDER BY FullVisitorID, visitid, Hit_Number, Hit_Timestamp
)
SELECT tbl_base.*, 
