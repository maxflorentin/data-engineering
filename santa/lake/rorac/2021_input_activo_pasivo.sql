...
cto_act_pas_join0 AS (
	...
	SELECT DISTINCT A.cod_ren_contrato , A.cod_ren_contenido , A.cod_ren_contrato_rorac , A.cod_ren_area_negocio , A.cod_ren_producto_gest , A.cod_ren_producto_niv_3 
		, IF(A.perimetro IS NULL, B.perimetro, A.perimetro) perimetro
	FROM cto_act_pas A
...
cto_act_pas_join AS (
	...
	SELECT DISTINCT A.cod_ren_contrato , A.cod_ren_contenido , A.cod_ren_contrato_rorac , A.cod_ren_area_negocio , A.cod_ren_producto_gest 
		, IF(A.perimetro IS NULL, IF(A.cod_ren_producto_niv_3 = 'BG1', 'A'
				...
				, IF(SUBSTRING(A.cod_ren_producto_gest, 1, 1) = 'O', 'P', NULL ))))), A.perimetro) perimetro 
	FROM cto_act_pas_join0 A
)
...
CREATE TEMPORARY TABLE cto_contenido_a AS
	SELECT DISTINCT cod_ren_contrato , cod_ren_contenido , cod_ren_contrato_rorac , cod_ren_area_negocio
...
----------------------------------------------------------- 
	SELECT 'ARG' cod_ren_unidad ,
		...
		partition_date
	FROM bi_corp_staging.zror_activo_ws 
