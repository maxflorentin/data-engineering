...
cto_act_pas AS (
	...
	WHERE RE.partition_date = '2020-11-30'
	AND RE.cod_ren_contenido IN ('PRE','CRE','ARF','CCO','PLZ','CTA','COM','CTB','ROF','CCL')
	AND RE.cod_ren_contrato != '-99100' -- sacar filtro
...
pro_act_pas AS (
	...
	FROM bi_corp_common.bt_ren_result RE
	WHERE RE.partition_date = '2020-11-30'
	AND RE.cod_ren_contenido IN ('COM','CTB','CCL') -- ROF NO APLICA 'ROF',
...
CREATE TABLE bi_corp_staging.zror_perimetro_pasivo LIKE bi_corp_staging.zror_perimetro_activo ;
...
set spark.yarn.queue=root.dataeng;
----------------------------------
DROP TABLE bi_corp_staging.zror_producto_mis ;
CREATE TEMPORARY TABLE  bi_corp_staging.zror_producto_mis AS
	...
	WHERE partition_date = '2020-11-13'
...
set spark.yarn.queue=root.dataeng;
----------------------------------
DROP TABLE bi_corp_staging.zror_tablon_ifrs9 ;
...
set spark.yarn.queue=root.dataeng;
----------------------------------
DROP TABLE bi_corp_staging.zror_rosetta_key ;
CREATE TEMPORARY TABLE bi_corp_staging.zror_rosetta_key AS
	...
	FROM bi_corp_common.rosetta_nkey_hist MK
		JOIN bi_corp_common.rosetta_nkey_hist BDR ON 
			...
			AND BDR.partition_date = '2020-11-30'
			AND BDR.domain_code = '00004'
	WHERE MK.partition_date = '2020-11-30'
		AND MK.domain_code = '00005';

	----------------------------------------------------------------------------------------------
	----------------------------------------------------------------------- JOIN FINAL
	-- SELECT COUNT(1) FROM bi_corp_staging.zror_cto_activo ; -- 356.795.442
	---------------------------------------
	set mapred.job.queue.name=root.dataeng;
	---------------------------------------
	DROP TABLE bi_corp_staging.zror_activo_01 ;
	...
	-- SELECT COUNT(1) FROM bi_corp_staging.zror_activo_01 ; -- 356.795.442
	-- SELECT cod_ren_contenido , COUNT(1) FROM bi_corp_staging.zror_activo_01 GROUP BY cod_ren_contenido ;
	---------------------------------------
	set mapred.job.queue.name=root.dataeng;
	---------------------------------------
	DROP TABLE  bi_corp_staging.zror_activo_02 ;
	...
	set mapred.job.queue.name=root.dataeng ;
	---------------------------------------
	DROP TABLE bi_corp_staging.zror_pasivo ;
...
-- SELECT * FROM cto_contenido WHERE cod_ren_contrato = '00720041390071039100525966000' ;
-- SELECT * FROM cto_contenido_a WHERE cod_ren_contrato = '00720041390071039100525966000' ;
-- SELECT * FROM cto_contenido_p WHERE cod_ren_contrato = '00720041390071039100525966000' ;
-- SELECT * FROM bi_corp_staging.zror_perimetro_activo WHERE cod_ren_contrato = '00720041390071039100525966000' ;
-- SELECT * FROM bi_corp_staging.zror_perimetro_pasivo WHERE cod_ren_contrato = '00720041390071039100525966000' ;	
-- SELECT * FROM bi_corp_staging.zror_activo WHERE cod_ren_contrato = '00720041390071039100525966000' ;
-- SELECT * FROM bi_corp_staging.zror_pasivo WHERE cod_ren_contrato = '00720041390071039100525966000' ;
-------------------------------------------------------------------------------------
----------------------------------------------------------------------- INSERT ACTIVO
set mapred.job.queue.name=root.dataeng;
