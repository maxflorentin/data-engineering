...
class Airflow(BaseView):
    ...
    @expose('/custom_trigger')
    @login_required
    @wwwutils.action_logging
    @provide_session
    def custom_trigger(self, session=None):
        dag_id = request.args.get('dag_id')
        dag = dagbag.get_dag(dag_id)
        title = "Trigger DAG"
        subtitle = "Parameters"
        root = request.args.get('root', '')
        input_fields = dag.default_args.get('input', {})

        return self.render(
            'airflow/custom_trigger.html',
            dag=dag, title=title, subtitle=subtitle, input=input_fields, root=root)
    ...
    def trigger(self, session=None):
        ...
        for k, v in request.values.items():
            if k not in ('dag_id', 'origin', 'csrf_token'):
                run_conf[k] = v if v else None
