...
def download_nps_files(dayofrun):
    ...
    surveyids = read_config(ZONDA_DIR+"/repositories/zonda-etl/scripts/api/nps/nps_survey_reproceso.json")
...
def gen_landing_file(file, surveyName, dayofrun, npsGroups_list):
    ...
    if not df2.empty:
        ...
        if ZONDA_DIR.__contains__('airflow'):
            ...
            s3_connection.Bucket(AWS_BUCKET_S3).upload_file(Filename=filename, Key='data/in/{}_{}.csv'.
                                                            format(surveyName.lower(), dayofrun.replace('-', '')))
        ...
        print("Empty DF")
        with open(ZONDA_DIR + '/inbound/nps/fact/{}/{}_{}.csv'.format(surveyName.lower(), file.replace(' ', '_'),
                                                                       dayofrun.replace('-', '')), "w"):
            pass  #
