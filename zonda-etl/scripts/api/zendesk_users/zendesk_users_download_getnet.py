    ...
    try:
        for i in range(0,cote):
            ...
            if updated_frm >= partition_date:
                ...
                archivo_sal = open(ZONDA_DIR + "/tmp/zendesk/users/globalgetnet/zendesk_usr_globalgetnet_{}.csv".format(partition_date_filter),'a')
...
def download_zendesk_user_files(partition_date_filter, partition_date_unix, partition_date):
    ...
    for zendesk_usr_name, zendesk_usr_data in config.items():
        ...
        if zendesk_usr_data.get('item') == 'globalgetnet':
            ...
            os.replace(ZONDA_DIR + "/tmp/zendesk/users/globalgetnet/zendesk_usr_globalgetnet_{}.csv".format(partition_date_filter),
                       ZONDA_DIR + "/inbound/zendesk_usr/fact/globalgetnet/zendesk_usr_globalgetnet_{}.csv".format(partition_date_filter))
