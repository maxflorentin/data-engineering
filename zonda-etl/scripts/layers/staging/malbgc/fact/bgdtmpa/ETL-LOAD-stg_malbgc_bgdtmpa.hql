...
SELECT * FROM bi_corp_staging.malbgc_bgdtmpa where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='previous_date_from', dag_id='LOAD_STG_Malbgc_Updates') }}'
union all
SELECT * FROM bi_corp_staging.malbgc_bgdtmpa_stock where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='previous_date_from', dag_id='LOAD_STG_Malbgc_Updates') }}';

DROP TABLE IF EXISTS bgdtmpa_update_tmp;
create TEMPORARY table bgdtmpa_update_tmp as
SELECT * FROM bi_corp_staging.malbgc_bgdtmpa_updates where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='date_from', dag_id='LOAD_STG_Malbgc_Updates') }}';

INSERT OVERWRITE TABLE bi_corp_staging.malbgc_bgdtmpa PARTITION(partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='date_from', dag_id='LOAD_STG_Malbgc_Updates') }}')
SELECT
