...
SELECT * FROM bi_corp_staging.malbgc_bgdtrpp where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='previous_date_from', dag_id='LOAD_STG_Malbgc_Updates_BIS') }}'
union all
SELECT * FROM bi_corp_staging.malbgc_bgdtrpp_stock where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='previous_date_from', dag_id='LOAD_STG_Malbgc_Updates_BIS') }}';

DROP TABLE IF EXISTS bgdtrpp_update_tmp;
create TEMPORARY table bgdtrpp_update_tmp as
SELECT * FROM bi_corp_staging.malbgc_bgdtrpp_updates where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='date_from', dag_id='LOAD_STG_Malbgc_Updates_BIS') }}';

INSERT OVERWRITE TABLE bi_corp_staging.malbgc_bgdtrpp PARTITION(partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='date_from', dag_id='LOAD_STG_Malbgc_Updates_BIS') }}')
SELECT
