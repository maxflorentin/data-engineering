...
INSERT OVERWRITE TABLE bi_corp_bdr.jm_grup_rela PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' )
select distinct *
from 
(SELECT 
    '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' as G5515_FEOPERAC,
    ...
    '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' AS G5515_FECULTMO
FROM 
    (
        select s.nup
                ,s.nro_grupo
        from bi_corp_staging.sge_grupos_economicos s 
        WHERE s.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_sge_grupos_economicos', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'  
...
LEFT JOIN   (
                ...
                WHERE nro_grupo is not null
                    and partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_sge_grupos_economicos', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'  
...
FROM                      
    (                    
        ...
        where gpe.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='next_month_first_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'  
...
inner join 
    (   select distinct acli.entidad_legal
        ...
        where acli.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='next_month_first_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'  
    ) a    
...
union all
select   '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' as G5515_FEOPERAC
        ,'23100' as G5515_S1EMP
...
from 
    (select ngr.empresa
        ...
        where ngr.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_no_mrg_grupos', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'  
    ) ngr
    left join 
        (select mnr.id_destino
            ...
            where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig',key='max_partition_map_no_mrg', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
