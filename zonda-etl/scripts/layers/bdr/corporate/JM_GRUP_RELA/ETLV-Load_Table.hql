...
INSERT OVERWRITE TABLE bi_corp_bdr.jm_grup_rela PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}' )
select distinct *
from 
(SELECT 
    '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' as G5515_FEOPERAC,
    ...
    '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' AS G5515_FECULTMO
FROM 
    (
        select s.nup
                ,s.nro_grupo
        from bi_corp_staging.sge_grupos_economicos s 
        WHERE s.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_sge_grupos_economicos', dag_id='BDR_LOAD_Tables-Monthly') }}'  
...
LEFT JOIN   (
                ...
                WHERE nro_grupo is not null
                    and partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_sge_grupos_economicos', dag_id='BDR_LOAD_Tables-Monthly') }}'  
...
union all
select   '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' as G5515_FEOPERAC
        ,'23100' as G5515_S1EMP
