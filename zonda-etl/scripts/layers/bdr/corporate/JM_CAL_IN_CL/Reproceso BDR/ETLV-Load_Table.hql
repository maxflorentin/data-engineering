...
select distinct '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' as E9954_FEOPERAC 
    ...
    from 
        (
            ...
            where c.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
         ) c
    inner join 
        (
        select *
            from santander_business_risk_arda.score_comportamiento sc
            where sc.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_score_comportamiento', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
        ) sc
...
where lpad(c.g4093_idnumcli,9,'0') != '000000000';
 ...
 --Score_admision
insert into table bi_corp_bdr.jm_cal_in_cl 
partition(partition_date= '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}')        
select distinct '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' as E9954_FEOPERAC
        ,'23100' as E9954_S1EMP
        ,lpad(c.g4093_idnumcli,9,'0') as E9954_IDNUMCLI
    ...
    (
        ...
        from 
           ...
           from santander_business_risk_arda.scoring_admision sa
                where sa.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_scoring_admision', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
            ) sa
            inner join
...
select distinct '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' as E9954_FEOPERAC
    ...
    inner join  
        (    SELECT  
            ...
            where periodo = '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
        ) ms
...
select distinct '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' as E9954_FEOPERAC 
    ...
    inner join  
        ...
        where sge.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_rating_sge', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
        ) sge
...
select distinct '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' as E9954_FEOPERAC 
    ...
    from 
        ...
        where no_mrg.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_no_mrg_juridica', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
        ) no_mrg
    left join 
        (
        select cod_baremo_local, cod_baremo_alfanumerico_local, desc_baremo_local, cod_negocio_local 
...
select distinct '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' as E9954_FEOPERAC 
    ...
    inner join 
        ...
        where an.partition_date  = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
        ) an
