...
select distinct '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' as E9954_FEOPERAC 
    ...
    from 
        (
            ...
            where c.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
         ) c
    inner join 
        (
        select *
            from santander_business_risk_arda.score_comportamiento sc
            where sc.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_score_comportamiento', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
        ) sc
    ...
    (
        ...
        from 
           ...
           from santander_business_risk_arda.scoring_admision sa
                where sa.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_scoring_admision', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
            ) sa
            inner join
...
select distinct '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' as E9954_FEOPERAC
    ...
    inner join  
        (    SELECT  
            ...
            where periodo = '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
        ) ms
...
select distinct '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' as E9954_FEOPERAC 
    ...
    inner join  
        ...
        where sge.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_rating_sge', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
        ) sge
...
select distinct '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' as E9954_FEOPERAC 
    ...
    from 
        ...
        where no_mrg.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_no_mrg_juridica', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
        ) no_mrg
    left join 
        (
        select cod_baremo_local, cod_baremo_alfanumerico_local, desc_baremo_local, cod_negocio_local 
...
select distinct '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' as E9954_FEOPERAC 
    ...
    inner join 
        ...
        where an.partition_date  = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
        ) an
