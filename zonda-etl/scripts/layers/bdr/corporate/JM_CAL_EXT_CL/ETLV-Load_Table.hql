...
(
	...
	from bi_corp_staging.aqua_rating_empresas rte  inner join perim_contr co
		on rte.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_aqua_rating_empresas', dag_id='BDR_LOAD_Tables-Monthly') }}' and co.shortname = rte.unidad_operativa
		inner join bi_corp_bdr.jm_client_bii bi
	    on bi.g4093_idnumcli = lpad(co.alias_nup,9,'0') and bi.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
	union all
	select 2 as orden,
	bi.g4093_feoperac as R9415_FEOPERAC, bi.g4093_s1emp as R9415_S1EMP, bi.g4093_idnumcli as R9415_IDNUMCLI,
