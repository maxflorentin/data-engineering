...
with intervinientes_contrato AS (
		...
		from bi_corp_bdr.test_jm_interv_cto
	...
	contratos_con_bg_con_repetidos as (
		...
		from intervinientes_contrato ic
    		left join bi_corp_bdr.jm_client_bii bii
			   ...
			   and bii.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_perimetro-rtra') }}'
			left join mapeo m
...
where c.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_perimetro-rtra') }}'