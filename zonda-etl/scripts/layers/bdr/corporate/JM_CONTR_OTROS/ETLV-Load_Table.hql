 ...
 tabla as (
               select * from (
                              ...
                              inner join bi_corp_common.rosetta_nkey_hist nk
                                 ...
                                 and c.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
                                 and nk.partition_date= '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_rosetta_nkey_hist', dag_id='BDR_LOAD_Tables-Monthly') }}'
                                 and nk.domain_code ='00004'
                              inner join bi_corp_common.rosetta_nkey_hist nk2
                                  on nk2.master_key = nk.master_key
                                 and nk2.partition_date= '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_rosetta_nkey_hist', dag_id='BDR_LOAD_Tables-Monthly') }}'
                                 and nk2.domain_code = '00005'
                              inner join bi_corp_common.rosetta_rnkd_hist rn
                                  on rn.native_key = nk2.native_key
                                 and rn.partition_date=  '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_rosetta_rnkd_hist', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
with core_garra as (
    ...
    INNER JOIN bi_corp_bdr.perim_contratos_bis m
           ON m.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
          AND concat_ws("_", cg.cod_entidad, cg.cod_centro, cg.num_cuenta, cg.cod_producto, cg.cod_subprodu) = m.id_cto_source
    WHERE cg.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos_garra', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
core_refi as (
    ...
    FROM (SELECT bb.cod_entidad, bb.cod_centro, bb.num_cuenta, bb.cod_producto, bb.cod_subprodu, bb.cod_atribuido_a_segmento_especial as filler2,
          ...
          INNER JOIN (SELECT cod_entidad, cod_centro, num_cuenta, cod_producto, cod_subprodu, cod_divisa
                       ...
                       WHERE data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos_garra', dag_id='BDR_LOAD_Tables-Monthly') }}'
          ...
          WHERE bb.cod_atribuido_a_segmento_especial = 'DU'
            AND bb.data_date_part BETWEEN     '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}'
                                          AND '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day_arda',  dag_id='BDR_LOAD_Tables-Monthly') }}'
...
core_refdu as (
    ...
    FROM (SELECT concat_ws('_', refi.cod_entidad,refi.cod_centro,refi.num_cuenta,refi.cod_producto,refi.cod_subprodu) AS idf_cto_supra_s_divisa,
          ...
          INNER JOIN bi_corp_bdr.normalizacion_id_contratos b
                    ...
                    and b.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
          ...
          INNER JOIN santander_business_risk_arda.contratos_garra refi
                    ...
                    AND refi.data_date_part BETWEEN     '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}'
                                                    AND '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day_arda',  dag_id='BDR_LOAD_Tables-Monthly') }}'
           LEFT JOIN santander_business_risk_arda.contratos_garra fil
                    ...
                    AND fil.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos_garra', dag_id='BDR_LOAD_Tables-Monthly') }}'
          WHERE contr.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
from (
    SELECT bq.idf_cto_supra_s_divisa, bq.situacion_gestion, bq.Prioridad_situacion_gestion,
     ...
     WHERE cgf.data_date_part BETWEEN     '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}'
                                      AND '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day_arda',  dag_id='BDR_LOAD_Tables-Monthly') }}'
...
LEFT JOIN santander_business_risk_arda.ifrs9_tarjetas_saldos_visa tcv
         ...
         AND tcv.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_ifrs9_tarjetas_saldos_visa', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
LEFT JOIN santander_business_risk_arda.ifrs9_tarjetas_saldos_amex tca
       ...
       AND tca.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_ifrs9_tarjetas_saldos_amex', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
LEFT JOIN santander_business_risk_arda.ifrs9_tarjetas_saldos_master tcm
       ...
       AND tcm.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_ifrs9_tarjetas_saldos_master', dag_id='BDR_LOAD_Tables-Monthly') }}'

WHERE a.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
FROM (
            ...
            where cb.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
           ) prim
...
--              and dvto.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}'
              and dvto.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
--              and lcre.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}'
              and lcre.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
--              and datmax.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}'
              and datmax.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
--              and tint.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}'
              and tint.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
SELECT DISTINCT
'{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' AS E0623_FEOPERAC,
23100 AS E0623_S1EMP,
lpad(a.id_cto_bdr, 9, '0') AS E0623_CONTRA1,
'{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' AS E0623_FEC_MES,
...
'{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' AS E0623_FECULTMO,
...
CASE WHEN co_ant.E0623_CONTRA1 IS NULL AND temp_sit.situacion_gestion IN ('00005','00007','00008','00009','00013','00024') THEN '9999-12-31'
     ...
     WHEN temp_sit.situacion_gestion <> co_ant.E0623_GEST_SIT AND temp_sit.situacion_gestion NOT IN ('00005','00007','00008','00009','00013','00024') AND temp_sit.fin_utp  = '9999-12-31' THEN '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}'
     ELSE '9999-12-31'
...
--             where data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}'
             where data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_ifrs9_contratos', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
LEFT JOIN bi_corp_bdr.jm_contr_otros co_ant
          ...
          AND co_ant.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
LEFT JOIN bi_corp_bdr.datos_arda_contratos dac
            ON dac.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
select DISTINCT
       ...
       case when nvl(
                     datediff(to_date(from_unixtime(UNIX_TIMESTAMP(cast(nvl(maturity, '31/12/9999') as varchar(10)),"dd/MM/yyyy"))),
                              to_date(from_unixtime(UNIX_TIMESTAMP(cast( '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' as varchar(10)),"yyyy-MM-dd"))))
                    ,0) >= 0
            then 
               (
...
where tac.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_mmff_tactico_especie', dag_id='BDR_LOAD_Tables-Monthly') }}';
...
select 
       ...
       ,case 
               when lpad(concat(nvl(
                        datediff(   to_date(from_unixtime(UNIX_TIMESTAMP(cast( concat(SUBSTRING('{{ ti.xcom_pull(task_ids='InputConfig', key='next_month_first_day', dag_id='BDR_LOAD_Tables-Monthly') }}',1,8), '05') as varchar(10)),"yyyy-MM-dd"))), 
                                    to_date(from_unixtime(UNIX_TIMESTAMP(cast( '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' as varchar(10)),"yyyy-MM-dd")))
                     )                        
