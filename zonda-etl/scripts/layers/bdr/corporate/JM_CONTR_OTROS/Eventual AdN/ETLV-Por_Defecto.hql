	...
	contratos_con_bg_con_repetidos as (
		...
		from intervinientes_contrato ic
    		left join bi_corp_bdr.jm_client_bii bii
			   ...
			   and bii.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Area_De_Negocio') }}'
			left join mapeo m
...
select c.e0623_feoperac,
       ...
       c.e0623_unnt,
	   case
	        when (c.e0623_unnts != '00000') then c.e0623_unnts
       ...
       c.e0623_finiutct,
       c.e0623_ffinutct
from bi_corp_bdr.jm_contr_otros c 
   left join contratos_con_bg bg
      on bg.contra1 = c.e0623_contra1
where c.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Area_De_Negocio') }}'