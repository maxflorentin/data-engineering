...
CREATE TEMPORARY TABLE bi_corp_bdr.core_contratos as
select  dat.pk_s_d, dat.idf_cto_supra, dat.cod_centro, dat.cod_subprodu_altair, dat.cod_producto, dat.cod_divisa, dat.num_persona, dat.cod_reesctr, dat.fec_vencimiento, dat.fec_alta_cto, dat.cod_tip_tasa
  from (
          select concat_ws("_",a.cod_entidad,a.cod_centro,a.num_cuenta,a.cod_producto,a.cod_subprodu_altair) as pk_s_d,
                 a.idf_cto_supra,
                 a.cod_centro,
                 a.cod_subprodu_altair,
                 a.cod_producto,
                 a.cod_divisa,
                 a.num_persona,
                 a.cod_reesctr,
                 ...
                 a.fec_alta_cto,
                 a.cod_tip_tasa,
                 count(1) over (partition by a.cod_entidad,a.cod_centro,a.num_cuenta,a.cod_producto,a.cod_subprodu_altair, a.num_persona ) cant_divisas
                 --count(1) over (partition by a.cod_entidad,a.cod_centro,a.num_cuenta,a.cod_producto,a.cod_subprodu_altair, a.cod_divisa, a.num_persona ) cant_divisas
            from santander_business_risk_arda.contratos a
           WHERE a.data_date_part =  '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos', dag_id='BDR_LOAD_Cliente_bis-Test') }}'
             and (a.cod_marca != "FA" or a.cod_marca is null)
             and (a.cod_estado_rel_cliente is null
                or not (a.cod_estado_rel_cliente = "C"
                        and a.fec_baja_rel_cliente <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Cliente_bis-Test') }}'
                       )
                 )
        ) dat
  where ( (dat.cant_divisas = 2 and dat.cod_divisa = "ARS") or dat.cant_divisas = 1)
;

insert overwrite table bi_corp_bdr.test_jm_contr_bis
partition(partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Cliente_bis-Test') }}')

SELECT distinct '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Cliente_bis-Test') }}' AS G4095_FEOPERAC,
                ...
                '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Cliente_bis-Test') }}' AS G4095_FECULTMO,
                lpad(0, 9, '0') AS G4095_VENCORIG,
                lpad(0, 5, '0') AS G4095_DEUDPREL,
                '9999-12-31' AS G4095_FECENTVI,
                lpad(9, 5, '9') AS G4095_IDPROLC2
FROM bi_corp_bdr.core_contratos a
   INNER JOIN bi_corp_bdr.normalizacion_id_contratos m
                  ON a.pk_s_d = m.id_cto_source
                 AND m.source = 'credito'
                 AND m.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Cliente_bis-Test') }}'
   LEFT JOIN santander_business_risk_arda.personas p
                 ON a.num_persona = p.num_persona
 --               AND p.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Cliente_bis-Test') }}'
                AND p.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_personas', dag_id='BDR_LOAD_Cliente_bis-Test') }}'
...
--                AND cg.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Cliente_bis-Test') }}'
                AND cg.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos_garra', dag_id='BDR_LOAD_Cliente_bis-Test') }}'
   ...
   LEFT JOIN bi_corp_bdr.v_map_baremos_global_local vblg_tasa
    ...
    LEFT JOIN bi_corp_bdr.jm_trz_cont_ren ren
                ON ren.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Cliente_bis-Test') }}'
...
partition(partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Cliente_bis-Test') }}')
SELECT distinct
        '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Cliente_bis-Test') }}' AS G4095_FEOPERAC,
 ...
 inner join bi_corp_bdr.normalizacion_id_contratos nc
        on nc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Cliente_bis-Test') }}'
       and nc.source = 'mmff-tactico'
       and nc.id_cto_source = concat_ws('_', trim(tac.cuenta),trim(tac.isin))
where tac.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_mmff_tactico_especie', dag_id='BDR_LOAD_Cliente_bis-Test') }}';
...
SELECT 
        ...
        ,concat(substr('{{ ti.xcom_pull(task_ids='InputConfig', key='next_month_first_day', dag_id='BDR_LOAD_Cliente_bis-Test') }}',0,8),'05') AS G4095_FECVENTO --Valor por defecto: Actualizar mensualmente el día 5 del mes siguiente al de los datos.
        ,concat(substr('{{ ti.xcom_pull(task_ids='InputConfig', key='next_month_first_day', dag_id='BDR_LOAD_Cliente_bis-Test') }}',0,8),'05') AS G4095_FECVE2 --Valor por defecto: Actualizar mensualmente el día 5 del mes siguiente al de los datos.
...
from 
    (select *
      ...
      where crp.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_corresponsales', dag_id='BDR_LOAD_Cliente_bis-Test') }}'
    ) crp
left join 
    (select * 
        from bi_corp_bdr.normalizacion_id_contratos 
        where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Cliente_bis-Test') }}'
