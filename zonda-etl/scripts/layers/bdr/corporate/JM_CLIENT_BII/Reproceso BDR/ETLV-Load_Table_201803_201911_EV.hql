...
select '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_EV_AD_CB') }}' AS G4093_FEOPERAC
...
from
    (
       ...
       where crp.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_corresponsales', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_EV_AD_CB') }}'
    ) crp
;
...
partition(partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_EV_AD_CB') }}')
 select  distinct
       ...
       '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_EV_AD_CB') }}'  as G5519_FEOPERAC,
  ...
  inner join bi_corp_staging.mmff_tactico_especie tac
     on tac.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_mmff_tactico_especie', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_EV_AD_CB') }}'
  ...
  inner join 
   ...
   left join santander_business_risk_arda.clientes cl
    ...
    and cl.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_clientes', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_EV_AD_CB') }}'
...
'{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_EV_AD_CB') }}' as partition_date
FROM
(
  select c.*
  from
    (
      ...
      where c.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_clientes', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_EV_AD_CB') }}'
...
INNER JOIN santander_business_risk_arda.personas p ON p.num_persona = c.num_persona AND p.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_personas', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_EV_AD_CB') }}'
LEFT JOIN bi_corp_bdr.marca_utp_clientes utp on utp.num_persona = c.num_persona and utp.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_EV_AD_CB') }}'
LEFT JOIN santander_business_risk_arda.marca_comite mc on mc.num_persona = c.num_persona and mc.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_marca_comite', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_EV_AD_CB') }}'
...
LEFT JOIN santander_business_risk_arda.personas_valor_rgo rgo on rgo.num_persona = p.num_persona and rgo.indicador in ('AVI', 'AYV') and rgo.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_personas_valor_rgo', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_EV_AD_CB') }}'
