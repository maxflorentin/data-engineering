...
select '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' AS G4093_FEOPERAC
...
partition(partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}')
 select  distinct
       ...
       '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'  as G5519_FEOPERAC,
  ...
  inner join bi_corp_staging.mmff_tactico_especie tac
     on tac.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_mmff_tactico_especie', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
  ...
  inner join 
   ...
   left join santander_business_risk_arda.clientes cl
    ...
    and cl.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_clientes', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
 ...
 left JOIN santander_business_risk_arda.rating_sge sge_cli 
    ...
    and sge_cli.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_rating_sge', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
...
where cod_submarca_cliente IN ('20', '22') and partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
UNION ALL
select distinct num_persona
from santander_business_risk_arda.marca_comite
where cod_marca_subjetiva IN ('PR', 'DE', 'MO', 'CO') and data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_marca_comite', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
;
...
where rgo.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_personas_valor_rgo', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
...
SELECT DISTINCT
'{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' AS G4093_FEOPERAC,
23100 AS G4093_S1EMP,
...
(
  ...
  from
    (
      ...
      where c.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_clientes', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
...
INNER JOIN santander_business_risk_arda.personas p ON p.num_persona = c.num_persona AND p.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_personas', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
LEFT JOIN bi_corp_bdr.marca_utp_clientes utp on utp.num_persona = c.num_persona and utp.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
LEFT JOIN santander_business_risk_arda.marca_comite mc on mc.num_persona = c.num_persona and mc.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_marca_comite', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
...
LEFT JOIN santander_business_risk_arda.personas_valor_rgo rgo on rgo.num_persona = p.num_persona and rgo.indicador in ('AVI', 'AYV') and rgo.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_personas_valor_rgo', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
...
LEFT JOIN bi_corp_staging.aqua_clientes_asoc_geconomicos a 
    on trim(a.unidad_operativa) = trim(k.shortname) 
      and a.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='next_month_first_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' 
LEFT JOIN santander_business_risk_arda.rating_sge sge_cli 
