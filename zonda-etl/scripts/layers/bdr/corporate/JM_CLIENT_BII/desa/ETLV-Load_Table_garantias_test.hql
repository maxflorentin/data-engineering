...
LEFT JOIN bi_corp_bdr.test_jm_client_bii cb
      ON cb.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
...
LEFT JOIN santander_business_risk_arda.clientes c
     ...
     AND c.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_clientes', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
...
LEFT JOIN bi_corp_bdr.marca_utp_clientes utp
     ...
     AND utp.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
LEFT JOIN santander_business_risk_arda.marca_comite mc
      ON lpad(mc.num_persona,9,'0') = g.g4128_idnumcli
     AND mc.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_marca_comite', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
...
LEFT JOIN santander_business_risk_arda.personas_valor_rgo rgo
     ...
     AND rgo.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_personas_valor_rgo', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
...
left join bi_corp_bdr.JM_CLIEN_JURI juri on juri.g5508_idnumcli = lpad(trim(c.num_persona), 9, '0') and juri.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
left join (SELECT cod_divisa, data_date_part, cast((imp_cambio_fijo/100)as double) as imp_cambio_fijo_vigente, concat(substr(fec_cambio, 1,4), substr(fec_cambio, 6,2), substr(fec_cambio, 9,2)) as fec_cambio_yyyymmdd
    	    ...
    	    WHERE data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
    	      ...
    	      and cod_divisa = 'EUR'
    		  and fec_cambio = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}') cotiz on (cod_divisa ='EUR')
WHERE g.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
