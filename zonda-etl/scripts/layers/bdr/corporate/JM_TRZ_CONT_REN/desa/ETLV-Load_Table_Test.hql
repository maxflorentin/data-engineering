...
ON p_old.pecodent = r.mig_old_entidad AND p_old.pecodofi = r.mig_old_cent_alta AND concat("0", p_old.pecodpro, substring(p_old.penumcon, 4) ) = r.mig_old_cuenta AND p_old.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt008', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
LEFT JOIN bi_corp_staging.malpe_pedt008 p_new
ON p_new.pecodent = r.mig_new_entidad AND p_new.pecodofi = r.mig_new_cent_alta AND concat("0",p_new.pecodpro, substring(p_new.penumcon, 4) ) = r.mig_new_cuenta AND p_new.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt008', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
...
-- AND p_old.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}' 
