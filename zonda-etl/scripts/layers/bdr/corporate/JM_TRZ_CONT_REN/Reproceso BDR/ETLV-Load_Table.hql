...
ON c.cod_entidad = r.cod_entidad AND c.cod_centro = r.cod_centro AND c.num_cuenta = r.num_contrato AND c.cod_producto = r.cod_producto AND r.cod_subprodu = c.cod_subprodu_altair AND c.cod_divisa = r.cod_divisa AND c.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
WHERE r.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_mddtccn', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
-- WHERE r.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
AND c.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' -- '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
AND r.fec_altareg BETWEEN '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' AND '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' ; 
...
SELECT  DISTINCT '23100'                                                                                                                    AS G7025_S1EMP
       ...
       ,'{{ ti.xcom_pull(task_ids='InputConfig',key='last_working_day',dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'    AS G7025_FEC_MOD
...
WHERE n_old.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
AND n_new.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' ; -- SUCURSALES 
...
ON r.pecdgent = p.pecdgent AND p.pecodofi = r.datos_comunes_peofiori AND substring(p.penumcon, 4) = substring(r.penumcon, 4) AND p.pecodpro = r.pecodpro AND p.pecodsub = r.pecodsub AND p.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt042', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
WHERE r.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_peec867c', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' --'2019-12-13' valor fijo hasta que se cargue una novedad. no programado 
AND p.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt042', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
-- AND p.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
AND p.pefecest BETWEEN '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' AND '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
AND p.peestope = 'C' ; 
...
ON p_old.pecodent = r.mig_old_entidad AND p_old.pecodofi = r.mig_old_cent_alta AND concat("0", p_old.pecodpro, substring(p_old.penumcon, 4) ) = r.mig_old_cuenta AND p_old.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt008', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
LEFT JOIN bi_corp_staging.malpe_pedt008 p_new
ON p_new.pecodent = r.mig_new_entidad AND p_new.pecodofi = r.mig_new_cent_alta AND concat("0",p_new.pecodpro, substring(p_new.penumcon, 4) ) = r.mig_new_cuenta AND p_new.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt008', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
WHERE r.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malbgc_zbdtmig', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
-- WHERE r.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
AND p_old.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt008', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
AND p_new.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt008', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
-- AND p_old.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
AND r.mig_old_fech_baja BETWEEN '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' AND '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' ; 
...
INNER JOIN bi_corp_bdr.normalizacion_id_contratos n_old
ON n_old.cred_cod_entidad = r.mig_old_entidad AND n_old.cred_cod_centro = r.mig_old_cent_alta AND substring(n_old.cred_num_cuenta, 4) = substring(r.mig_old_cuenta, 4) AND n_old.cred_cod_producto = r.old_pecodpro AND n_old.cred_cod_subprodu_altair = r.old_pecodsub AND n_old.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
INNER JOIN bi_corp_bdr.normalizacion_id_contratos n_new
ON n_new.cred_cod_entidad = r.mig_new_entidad AND n_new.cred_cod_centro = r.mig_new_cent_alta AND substring(n_new.cred_num_cuenta, 4) = substring(r.mig_new_cuenta, 4) AND n_new.cred_cod_producto = r.new_pecodpro AND n_new.cred_cod_subprodu_altair = r.new_pecodsub AND n_new.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
LEFT JOIN bi_corp_bdr.v_baremos_local bl
ON bl.cod_negocio_local = '90' AND bl.cod_baremo_local = '1'
LEFT JOIN bi_corp_bdr.v_map_baremos_global_local mp
ON mp.cod_negocio_local = 90 AND mp.cod_baremo_local = bl.cod_baremo_local
WHERE n_old.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
AND n_new.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' ; -- UPGRADE DE TARJETAS 
...
INNER JOIN bi_corp_bdr.normalizacion_id_contratos n_old
ON n_old.cred_cod_entidad = r.old_entidad AND n_old.cred_cod_centro = r.old_centro AND substring(n_old.cred_num_cuenta, 2) = substring(r.old_cuenta, 2) AND n_old.cred_cod_producto = r.old_producto AND n_old.cred_cod_subprodu_altair = trim(r.old_subprodu) AND n_old.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
INNER JOIN bi_corp_bdr.normalizacion_id_contratos n_new
ON n_new.cred_cod_entidad = r.new_entidad AND n_new.cred_cod_centro = r.new_centro AND substring(n_new.cred_num_cuenta, 2) = substring(r.new_cuenta, 2) AND n_new.cred_cod_producto = r.new_producto AND n_new.cred_cod_subprodu_altair = trim(r.new_subprodu) AND n_new.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
LEFT JOIN bi_corp_bdr.v_baremos_local bl
...
WHERE n_old.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
AND n_new.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
AND r.partition_date BETWEEN '{{ ti.xcom_pull(task_ids='InputConfig', key='first_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' AND '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' ; --Renumeracion contratos corresponsales 

INSERT INTO TABLE bi_corp_bdr.jm_trz_cont_ren partition(partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}')
SELECT  '23100'                                                                                                                          AS G7025_S1EMP 
       ...
       ,concat(substr('{{ ti.xcom_pull(task_ids='InputConfig',key='next_month_first_day',dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}',0,8),'06') AS G7025_FEALTREL --Día siguiente a la fecha de vencimiento actual vencen el 05 de cada mes 
       ,'{{ ti.xcom_pull(task_ids='InputConfig',key='last_working_day',dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' AS G7025_FEC_MOD --last_working_day 
...
(
	...
	WHERE crp.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_corresponsales', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'  
) crp
LEFT JOIN 
(
	SELECT  *
...
(
	SELECT  alh.rubro_altair 
	       ,trim(group_map[cast(month('{{ ti.xcom_pull(task_ids='InputConfig',key='last_calendar_day',dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}') AS string)]) AS importe
	FROM 
	(
		SELECT  rubro_altair
