            ...
            select distinct ngm.id_cto_bdr, gmg.num_persona_garante 
                ...
                inner join 
                    (select *
                        ...
                        where gmg.fec_vcto >= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
                            and gmg.fec_alta <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
                    ) gmg                        
...
from
    ...
    (select *
        ...
        where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_gtdtrgb', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
    ) rgb on gcd.num_garantia = rgb.gttcrgb_rgb_num_garantia
    left join
    (select *
        from bi_corp_staging.gtdtrbc
        where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_gtdtrbc', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
