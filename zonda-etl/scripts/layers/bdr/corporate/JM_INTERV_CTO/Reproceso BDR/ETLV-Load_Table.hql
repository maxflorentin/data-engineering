...
INNER JOIN (SELECT data_date_part, cod_entidad, cod_centro, num_cuenta, cod_producto, cod_subprodu, cal_participacion, cod_estado_relacion,
             ...
             WHERE data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_relacion_contrato_cliente', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
               AND cal_participacion IN ("TI", "AV", "FI", "GA")
               AND (cod_estado_relacion = 'A' or (cod_estado_relacion = 'C' and fec_baja > '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'))
...
INNER JOIN bi_corp_bdr.jm_contr_bis cb
            on cb.g4095_contra1 = m.id_cto_bdr
           AND cb.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
...
LEFT JOIN bi_corp_staging.bgdtmae mae
           ...
           and mae.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_bgdtmae', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
...
WHERE c.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_relacion_contrato_cliente', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
;
...
inner join bi_corp_staging.mdr_contrapartes mdr
     on mdr.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_mdr_contrapartes', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
