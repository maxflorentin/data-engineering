...
SELECT DISTINCT
       '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' AS G4128_FEOPERAC,
       ...
       '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' AS G4128_FECULTMO
...
INNER JOIN (SELECT data_date_part, cod_entidad, cod_centro, num_cuenta, cod_producto, cod_subprodu, cal_participacion, cod_estado_relacion,
             ...
             WHERE data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_relacion_contrato_cliente', dag_id='BDR_LOAD_Tables-Monthly') }}'
               AND cal_participacion IN ("TI", "AV", "FI", "GA")
               AND (cod_estado_relacion = 'A' or (cod_estado_relacion = 'C' and fec_baja > '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}'))
...
INNER JOIN bi_corp_bdr.jm_contr_bis cb
           ...
           AND cb.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
LEFT JOIN bi_corp_staging.bgdtmae mae
           ...
           and mae.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_bgdtmae', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
WHERE c.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_relacion_contrato_cliente', dag_id='BDR_LOAD_Tables-Monthly') }}'
;
...
inner join bi_corp_staging.mdr_contrapartes mdr
     on mdr.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_mdr_contrapartes', dag_id='BDR_LOAD_Tables-Monthly') }}'
