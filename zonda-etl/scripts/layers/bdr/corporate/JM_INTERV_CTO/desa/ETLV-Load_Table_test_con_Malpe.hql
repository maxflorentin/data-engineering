...
insert OVERWRITE TABLE bi_corp_bdr.test2_jm_interv_cto
...
FROM bi_corp_bdr.perim_malpe_pedt008_1er_titular c

INNER JOIN bi_corp_bdr.test2_normalizacion_id_contratos m
...
INNER JOIN bi_corp_bdr.test2_jm_contr_bis cb
...
insert INTO TABLE bi_corp_bdr.test2_jm_interv_cto
partition(partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}')
SELECT DISTINCT
       '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}' AS G4128_FEOPERAC,
       23100 AS G4128_S1EMP,
...
inner join bi_corp_bdr.test2_normalizacion_id_contratos nc
