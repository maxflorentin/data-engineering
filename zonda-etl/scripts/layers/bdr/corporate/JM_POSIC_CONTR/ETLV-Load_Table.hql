...
FROM santander_business_risk_arda.contratos_adsf b
    INNER JOIN (SELECT cod_entidad
                ...
                WHERE data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos_adsf', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
WHERE b.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos_adsf', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
FROM santander_business_risk_arda.contratos_deudores_adic x
    INNER JOIN (SELECT  cod_entidad
                ...
                WHERE data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos_deudores_adic', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
WHERE x.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos_deudores_adic', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
SELECT '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' AS E0621_FEOPERAC
        ...
        ,'{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' AS E0621_FEC_MES
...
with cto_perimetro as (
    ...
    inner join santander_business_risk_arda.CONTRATOS co
            on co.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
select
      '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' as E0621_FEOPERAC,
      23100 as E0621_S1EMP,
...
inner join bi_corp_bdr.jm_interv_cto ic
    on ic.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
   and ic.g4128_contra1 = t.id_cto_bdr
   and ic.g4128_tipintev = '00001'
inner join bi_corp_bdr.jm_client_bii cl
    on cl.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
    ...
    from
        ...
        left join 
            (
                ...
                where ict.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'  
        ...
        left join 
            (
                ...
                where fec_periodo = '{{ ti.xcom_pull(task_ids='InputConfig', key='month', dag_id='BDR_LOAD_Tables-Monthly') }}' 
...
left join 
    (
        select  centro
                ...
                ,subproducto
                ,mora_pcr16
         from bi_corp_common.stk_mora_marcas_riesgos
        where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_stk_mora_marcas_riesgos', dag_id='BDR_LOAD_Tables-Monthly') }}'
    ) mrm on
        trim(mrm.centro) = trim(norm.cred_cod_centro)
        and trim(mrm.entidad) = trim(norm.cred_cod_entidad)
        and trim(mrm.contrato) = trim(norm.cred_num_cuenta)
...
left join 
    ...
    where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig',key='max_partition_recla_moro_cargabal', dag_id='BDR_LOAD_Tables-Monthly') }}'
    ) recla on
...
left join 
    (
        ...
        where rac.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_recla_adicional_cargabal', dag_id='BDR_LOAD_Tables-Monthly') }}'
        and periodo = '{{ ti.xcom_pull(task_ids='InputConfig', key='month', dag_id='BDR_LOAD_Tables-Monthly') }}' 
    ) mrm on
...
partition(partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}')
 select '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' E0621_FEOPERAC
...
from 
    (
        ...
        where tac.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_mmff_tactico_especie', dag_id='BDR_LOAD_Tables-Monthly') }}'
    ) tac
     inner join 
    (
        ...
        where nc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
left join 
    (
        select  alh.rubro_altair 
                ,trim(group_map[cast(month(to_date(from_unixtime(UNIX_TIMESTAMP(cast( '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' as varchar(10)),"yyyy-MM-dd")))) as string)]) as importe
        from 
            (    
            select rubro_altair, 
                map('1',enero
