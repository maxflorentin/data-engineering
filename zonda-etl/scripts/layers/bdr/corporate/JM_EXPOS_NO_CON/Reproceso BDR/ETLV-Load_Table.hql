...
insert overwrite table bi_corp_bdr.jm_expos_no_con partition(partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' )
select
    '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' as E0627_FEOPERAC
    ,lpad(nvl(ble.cod_baremo_global,0),5,"0") as E0627_S1EMP 
    ,lpad(nvl(nic.id_cto_bdr,0),9,"0") as E0627_CONTRA1
    ,'{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' as E0627_FEC_MES
...
FROM (select blc.* 
        ...
        where (substring(trim(blc.cargabal),1,1) = '1' or substring(trim(blc.cargabal),1,3) in ('260','261','269'))
            and blc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
      union all        
      select blc.*
        from
          (select blc.* 
            from bi_corp_bdr.balances_ajustes blc 
            where blc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
...
LEFT JOIN (select * 
                ...
                where nic.partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
...
LEFT JOIN (select distinct jpc.e0621_ctacgbal, jpc.e0621_s1emp
            ...
            where jpc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
