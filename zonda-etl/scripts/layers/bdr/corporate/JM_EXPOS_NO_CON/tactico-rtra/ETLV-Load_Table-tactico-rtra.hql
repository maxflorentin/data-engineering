...
insert overwrite table bi_corp_bdr.test_jm_expos_no_con partition(partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_perimetro-rtra') }}' )
select
    '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_perimetro-rtra') }}' as E0627_FEOPERAC
    ,lpad(nvl(ble.cod_baremo_global,0),5,"0") as E0627_S1EMP 
    ,lpad(nvl(nic.id_cto_bdr,0),9,"0") as E0627_CONTRA1
    ,'{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_perimetro-rtra') }}' as E0627_FEC_MES
...
FROM (select blc.* 
        ...
        where (substring(trim(blc.cargabal),1,1) = '1' or substring(trim(blc.cargabal),1,3) in ('260','261','269'))
            and blc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_perimetro-rtra') }}' 
      union all        
      select blc.*
        from
          (select blc.* 
            from bi_corp_bdr.balances_ajustes blc 
            where blc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_perimetro-rtra') }}'
        ...
        left join 
          (select distinct rcp.cargabal 
              ...
              where rcp.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_rubros_cargabal_provisiones', dag_id='BDR_LOAD_perimetro-rtra') }}'
          ) rcp 
...
LEFT JOIN (select * 
                ...
                where nic.partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_perimetro-rtra') }}'
...
LEFT JOIN (select distinct jpc.e0621_ctacgbal, jpc.e0621_s1emp
            from  bi_corp_bdr.test_jm_posic_contr jpc
            where jpc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_perimetro-rtra') }}'
