...
insert overwrite table bi_corp_bdr.jm_expos_no_con partition(partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}' )
select
...
FROM (select blc.* 
        ...
        where (substring(trim(blc.cargabal),1,1) = '1' or substring(trim(blc.cargabal),1,3) in ('260','261','269'))
            and blc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
      union all        
      select blc.*
        from
          (select blc.* 
            from bi_corp_bdr.balances_ajustes blc 
            where blc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
        ...
        left join 
          (select distinct rcp.cargabal 
              ...
              where rcp.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_rubros_cargabal_provisiones', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
          ) rcp 
...
LEFT JOIN (select distinct jpc.e0621_ctacgbal, jpc.e0621_s1emp
            ...
            where jpc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
