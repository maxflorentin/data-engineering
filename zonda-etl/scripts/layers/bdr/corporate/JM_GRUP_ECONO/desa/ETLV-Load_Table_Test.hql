...
insert overwrite table bi_corp_bdr.test_jm_grup_econo partition(partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Area_De_Negocio') }}') 
SELECT '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Area_De_Negocio') }}' as G5512_FEOPERAC
            ...
            FROM 
                (
                    ...
                    from
                        ...
                        where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_sge_grupos_economicos', dag_id='BDR_LOAD_Area_De_Negocio') }}'
                        ) sge
                    left outer join 
                        (select distinct trim(mpe.penumgru) as penumgru
                            ,mpe.pedesgru 
                        from bi_corp_staging.malpe_pedt024 mpe 
                        where mpe.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt024', dag_id='BDR_LOAD_Area_De_Negocio') }}'   
                        ) mpe 
            ...
            left outer join
                (select sge.* 
                    from
                        ...
                        from bi_corp_staging.sge_grupos_economicos sge
                        where  sge.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_sge_grupos_economicos', dag_id='BDR_LOAD_Area_De_Negocio') }}'
                         ) sge
                         where sge.seq = 1
                        ...
                        select 
                            ...
                            left OUTER JOIN 
                                (select distinct substr(tcm.par_de_divisas,1,3) as par_de_divisas 
                                  ...
                                  where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_aqua_tipo_cambio', dag_id='BDR_LOAD_Area_De_Negocio') }}' 
