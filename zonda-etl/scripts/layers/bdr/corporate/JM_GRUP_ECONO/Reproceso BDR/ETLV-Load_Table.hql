...
insert overwrite table bi_corp_bdr.jm_grup_econo partition(partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}') 
SELECT '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' as G5512_FEOPERAC
            ...
            FROM 
                (
                    ...
                    from
                        ...
                        where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_sge_grupos_economicos', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
                        ) sge
                    left outer join 
                        (select distinct trim(mpe.penumgru) as penumgru
                            ,mpe.pedesgru 
                        from bi_corp_staging.malpe_pedt024 mpe 
                        where mpe.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt024', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'   
                        ) mpe 
            ...
            left outer join
                (select sge.* 
                    from
                        ...
                        from bi_corp_staging.sge_grupos_economicos sge
                        where  sge.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_sge_grupos_economicos', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
                         ) sge
                         where sge.seq = 1
