...
insert overwrite table bi_corp_bdr.jm_vta_carter partition(partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}')
select  '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' as R9736_FEOPERAC
...
from    (
            ...
            where hvt.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_moria_vc_historico_ventas', dag_id='BDR_LOAD_Tables-Monthly') }}' 
                    and hvt.ind_rechazo = 'N' 
                    and hvt.fecha_alta >= '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}'     
                    and hvt.fecha_alta < '{{ ti.xcom_pull(task_ids='InputConfig', key='next_month_first_day', dag_id='BDR_LOAD_Tables-Monthly') }}'  
    ...
    left join   
            (
                ...
                where vta.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_moria_vc_ventas', dag_id='BDR_LOAD_Tables-Monthly') }}'  
    ...
    left join 
            (
                ...
                where cto.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'  
            ) cto
