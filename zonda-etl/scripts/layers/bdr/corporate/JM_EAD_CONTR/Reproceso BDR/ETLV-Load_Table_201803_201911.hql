...
select t.G5519_FEOPERAC,
  ...
  from (select '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'  as G5519_FEOPERAC,
          ...
          from (select cast(esp.suma_nominales as double) / (sum(cast(esp.suma_nominales as double) ) over (partition by esp.especie)) porcentaje,
                 ...
                 where esp.partition_date  = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_mmff_tactico_especie', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
               ) esp
         inner join bi_corp_bdr.normalizacion_id_contratos nc
          ...
          left join santander_business_risk_arda.cotizaciones_bcra c
            on c.data_date_part  = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_cotizaciones_bcra', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
