...
INNER JOIN (SELECT t_cod_entidad, t_cod_centro, t_num_cuenta, t_cod_producto, t_cod_subprodu_altair, count(*) as q_div
            ...
            WHERE cast(periodo as string) = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_ifrs9_tablon', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
WHERE cast(tab.periodo as string) = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_ifrs9_tablon', dag_id='BDR_LOAD_Tables-Monthly') }}';
...
SELECT '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' as N0625_FEOPERAC,
...
INSERT INTO TABLE  bi_corp_bdr.jm_prov_esotr partition(partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}')
SELECT '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' as N0625_FEOPERAC,
        23100 as N0625_S1EMP,
        m.id_cto_bdr as N0625_CONTRA,
        lpad("2", 5, "0") as N0625_TIP_IMPT,
...
partition(partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}')
 select '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}'  N0625_FEOPERAC
...
from 
    ...
    where cbi.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
    ) cbi
inner join 
         (
...
(   
        ...
        where ift.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
