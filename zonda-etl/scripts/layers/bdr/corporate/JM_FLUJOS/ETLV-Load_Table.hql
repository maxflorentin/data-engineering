  ...
  FROM
       (
        ...
        INNER JOIN bi_corp_bdr.normalizacion_id_contratos b
              ...
              AND b.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
        ...
        WHERE a.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
          ...
          AND cg.data_date_part between      '{{ ti.xcom_pull(task_ids='InputConfig', key='first_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}'
                                         and '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}'
       ) bb;
...
INNER JOIN (
            SELECT r9746_contra1, max(r9746_fecmvto) as ult_mov
             ...
             WHERE PARTITION_DATE = '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
INNER JOIN bi_corp_bdr.normalizacion_id_contratos nc
       ...
       AND nc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
INNER JOIN santander_business_risk_arda.contratos_garra cg
       ...
       AND cg.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_last_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}'

 WHERE fl.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
-- PERSONAS CON CONTRATOS CON IMPORTE IRREGULAR > 0
       ...
       where data_date_part between     '{{ ti.xcom_pull(task_ids='InputConfig', key='first_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}'
                                    and '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
LEFT JOIN bi_corp_bdr.jm_trz_cont_ren ren
 ...
 WHERE core.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt042', dag_id='BDR_LOAD_Tables-Monthly') }}'
   ...
   AND concat(substr(core.PEFECEST,1,4),substr(core.PEFECEST,6,2)) = '{{ ti.xcom_pull(task_ids='InputConfig', key='month', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
SELECT cod_entidad, cod_centro, num_cuenta, cod_producto, cod_subprodu_altair, cod_marca, min(fec_castigo) fec_castigo
 ...
 WHERE data_date_part between     '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}'
                              and '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}'
   AND cod_marca = 'FA'
...
SELECT empresa, cod_baremo_local, cod_baremo_global
 ...
 WHERE cod_negocio_local = '1'
   AND '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
SELECT m.id_cto_bdr, m.id_cto_source, m.cred_cod_entidad as cod_entidad, m.cred_cod_centro as cod_centro,
 ...
 WHERE m.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
SELECT
'{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' AS R9746_FEOPERAC,
...
where core_fimp.data_date_part between '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}' and nvl(regexp_replace(cancelados.ult_act, '-', ''), '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}')
and (confa.fec_castigo is null or confa.fec_castigo = '9999-12-31' or confa.fec_castigo = '0001-01-01' or regexp_replace(substring(confa.fec_castigo, 1, 7), '-', '') = '{{ ti.xcom_pull(task_ids='InputConfig', key='month', dag_id='BDR_LOAD_Tables-Monthly') }}')
group by '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}',
...
--CONDICIONES GENERALES DEL INSERT
WHERE fl.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
WITH canceladosPrimerDia AS ( --solo me quedo con los cancelados el 1/12
  SELECT c.id_cto_bdr
	...
	WHERE c.fec_cancelacion = '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}' --'${primerDiaDeEsteMes}'
),
flujosMesAnterior AS ( --todos los flujos de NOVIEMBRE ordenados por fecha de movimiento
	...
	FROM bi_corp_bdr.jm_flujos fl
	WHERE fl.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}' --'${mesAnterior}'
),
...
insert INTO table bi_corp_bdr.jm_flujos
partition(partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}')
SELECT r9746_feoperac,
       r9746_s1emp,
       r9746_contra1,
       '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day_arda', dag_id='BDR_LOAD_Tables-Monthly') }}' as r9746_fecmvto,
       '00008' as r9746_clasmvto,
       concat('-',lpad(regexp_replace(format_number(0, 2),"\\,|\\.",""),16,"0")) as R9746_IMPORTE,
       concat('-',lpad(regexp_replace(format_number(0, 2),"\\,|\\.",""),16,"0")) as R9746_SALONBAL
FROM diferencia;