...
select cbb.g4093_idnumcli , cbb.g4093_cod_sec2 
 ...
 where cbb.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'  --'2020-03' 
...
(
    ...
    from    (
                select acsg.unidad_operativa
                    ...
                    where acsg.partition_date =  '{{ ti.xcom_pull(task_ids='InputConfig', key='next_month_first_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'  
            ) acsg
        inner join 
            ( 
                    select mkg.nup
        ...
        left OUTER JOIN 
            (
                ...
                where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_aqua_tipo_cambio', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
...
where naj.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_no_mrg_juridica', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' 
)
...
with sge_grupos_1 as (
    SELECT lpad(sge.nup,9,"0") as nup 
            ...
            where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_sge_grupos_economicos', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'   --'2020-04-14' 
)  
...
from 
    (
        ...
        where data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_personas_comp_fisicas', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'   --'20200630'
...
from 
 ...
 and monotributo not in ('NI','61', 'BC', 'BL', 'BT','BP', 'BV')
  ...
  where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='next_month_first_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'  --'2020-08-01'
  ...
  where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt015', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'  --'2020-06-30'
...
with bcra_1 as (
 ...
 from 
     ...
     where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_bcra', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' --'2020-06-01'
     ) bcr
   left join 
      (
        select *
...
from bcra_1 bcr
    ...
    left join 
        (select *
            ...
            left join 
                    (select * from bi_corp_staging.bcra_sector where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_bcra_sector', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' ) bcs  --'2020-06-01'
    ...
    left join 
        (select * from bi_corp_staging.bcra_sector_desc where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig',key='max_partition_bcra_sector_desc', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'  ) bcsd -- '2020-06-01'
...
from 
    ...
    where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='next_month_first_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'  ) idi --'2020-06-01' 
 left join 
  (
    select *
    from santander_business_risk_arda.PERSONAS_COMP_FISICAS
...
SELECT DISTINCT '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' as G5508_FEOPERAC
        ...
        ,'{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'  as G5508_FECULTMO
...
(
    ...
    where crp.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_corresponsales_adicional_juridica', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
) crp
