...
select distinct 
       ...
       G4134_BIENGAR1,
       case
         when G4134_FECHVALR < add_months('{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}',-12) then
           '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}'
         ELSE
           G4134_FECHVALR
...
from (
SELECT DISTINCT
'{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' AS G4134_FEOPERAC,
...
'{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly') }}' AS G4134_FECULTMO,
lpad('1', 5, '0') AS G4134_TIPVALN,
lpad(c.cod_des_textored, 5, '0') AS G4134_TIP_GARA,
'ARS' AS G4134_CODDIV
FROM bi_corp_staging.gtdtmae m
LEFT JOIN bi_corp_staging.gtdtrgb gb ON m.gttcmae_mae_num_garantia = gb.gttcrgb_rgb_num_garantia AND gb.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}'
LEFT JOIN bi_corp_staging.gtdtbie b ON b.gttcbie_bie_num_bien = gb.gttcrgb_rgb_num_bien AND b.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}'
LEFT JOIN bi_corp_staging.gtdttas t ON  t.gttctas_tas_num_bien = b.gttcbie_bie_num_bien AND t.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}'
LEFT JOIN bi_corp_staging.gtdtcod c ON  c.cod_cod_codigo = m.gttcmae_mae_cod_garantia AND c.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}'
LEFT JOIN bi_corp_staging.gtdtvab v ON  v.gttcvab_vab_num_bien = b.gttcbie_bie_num_bien AND v.gttcvab_vab_tip_valor = '015' AND v.partition_date =  '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}'
LEFT JOIN bi_corp_bdr.cotizaciones tmp ON tmp.cod_divisa = m.gttcmae_mae_cod_divisa
WHERE m.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}'
