...
-- STAGE1: Todos los contratos contra las ESPECIFICAS y el primer contrato contra las GENERICAS.
INSERT OVERWRITE TABLE bi_corp_bdr.aux_garant_garantias_contratos_div_test PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_garantias-Test') }}')
SELECT
      gc1.cod_entidad
    , gc1.num_persona
    , gc1.id_cto_div
        ...
        FROM
            ...
            INNER JOIN
                (SELECT * FROM bi_corp_bdr.aux_garant_garantias_especificas_div
                	WHERE partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_garantias-Test') }}'
                ) ge
                	ON (ge.partition_date = cto.partition_date and ge.id_cto_div = cto.id_cto_div)
        where 1=1
            and cto.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_garantias-Test') }}'
