...
INSERT OVERWRITE TABLE bi_corp_bdr.aux_garant_garantias_contratos_div_test PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_garantias_loop-Test') }}')
SELECT 
	...
	cod_entidad_bdr
	FROM bi_corp_bdr.aux_garant_garantias_contratos_div_test  cto_ant 
	WHERE partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_garantias_loop-Test') }}'
            ...
            (
				...
				LEFT OUTER JOIN 
						...
						SELECT partition_date,num_persona ,id_cto_div,max(orden_contrato ) as max_orden, MIN(imp_deuda_remanente) as deuda_remanente
							FROM bi_corp_bdr.aux_garant_garantias_contratos_div_test gc 
							WHERE partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_garantias_loop-Test') }}'
							--and num_persona ='02765272'
				...
				WHERE cto.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_garantias_loop-Test') }}'
