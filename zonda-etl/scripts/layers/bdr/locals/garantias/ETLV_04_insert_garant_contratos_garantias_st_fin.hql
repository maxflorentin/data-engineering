...
INSERT OVERWRITE TABLE bi_corp_bdr.aux_garant_garantias_contratos_div PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}')
SELECT
      gc.cod_entidad
    , gc.num_persona
    , gc.id_cto_div
...
FROM
	...
	LEFT OUTER JOIN
		...
		ON (ini.cod_entidad = gc.cod_entidad and ini.num_persona = gc.num_persona and ini.id_cto_div = gc.id_cto_div and ini.num_garantia = gc.num_garantia
		    and ini.partition_date < '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}')
WHERE
	gc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}'
