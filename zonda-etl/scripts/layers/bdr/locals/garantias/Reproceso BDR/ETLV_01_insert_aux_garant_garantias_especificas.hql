...
INSERT OVERWRITE TABLE bi_corp_bdr.aux_garant_garantias_especificas_div PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}')
 SELECT
 ...
 FROM
 	...
 	WHERE 1=1
 	    and mae.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
	...
	INNER JOIN
				(SELECT
					...
					WHERE
						rgo.partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
						and (rgo.gttcrgo_rgo_fec_altrelac <='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' and rgo.gttcrgo_rgo_fec_bajrelac >='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' )
 	...
 	LEFT OUTER JOIN
 				...
 				WHERE 1=1
 					and rgb.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
    ...
    LEFT OUTER JOIN
			(
				...
				--WHERE data_date_part = translate('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}','-','')
				WHERE data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
				  ...
				  --and concat(substr(fec_cambio, 1,4), substr(fec_cambio, 6,2), substr(fec_cambio, 9,2)) = translate('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}','-','')
                  and concat(substr(fec_cambio, 1,4), substr(fec_cambio, 6,2), substr(fec_cambio, 9,2)) = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
			)
			cotizaciones
			--ON (cotizaciones.data_date_part = translate('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}','-','') and cotizaciones.cod_divisa = MAESTRO.gttcmae_mae_cod_divisa)
			ON (cotizaciones.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' and cotizaciones.cod_divisa = MAESTRO.gttcmae_mae_cod_divisa)
 WHERE 1=1
 	and MAESTRO.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
...
UNION ALL ----------------------- todos los datos provenientes de ABKT
	 ...
	 FROM
		(	SELECT
			...
			WHERE (cgr.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}')
	 	)  CONTRAG
	    LEFT OUTER JOIN
				(
					SELECT cod_divisa, data_date_part, (imp_cambio_fijo/100) as imp_cambio_fijo_vigente, concat(substr(fec_cambio, 1,4), substr(fec_cambio, 6,2), substr(fec_cambio, 9,2)) as fec_cambio_yyyymmdd
				...
				cotizaciones
				ON (cotizaciones.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' and cotizaciones.cod_divisa = CONTRAG.cod_divisa  )
	 WHERE 1=1
	 	and CONTRAG.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
	;
...
FROM bi_corp_bdr.aux_garant_garantias_especificas_div ge
	LEFT OUTER JOIN
			...
			ON (ge.cod_garantia = pig.cod_cod_codigo and ge.cod_entidad = pig.cod_cod_entidad
			    and pig.partition_date = '2020-04-02')
	LEFT OUTER JOIN
			...
			ON (ge.cod_garantia = ins.cod_cod_codigo and ge.cod_entidad = ins.cod_cod_entidad
			 and ins.partition_date = '2020-04-02')
	LEFT OUTER JOIN
    			...
    			and avl.partition_date = '2020-05-08')
     LEFT OUTER JOIN
     			...
     			and tip.partition_date = '2020-05-08')
    LEFT OUTER JOIN
     			...
     			and gara.partition_date = '2020-05-08')
    LEFT OUTER JOIN
                (
															...
															FROM   (
																			SELECT * FROM (
											                	                	...
											                	                	WHERE 1=1
											                	                		...
											                	                		and rbc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
                				...
                				WHERE rgb.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
				) bien
...
WHERE ge.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
