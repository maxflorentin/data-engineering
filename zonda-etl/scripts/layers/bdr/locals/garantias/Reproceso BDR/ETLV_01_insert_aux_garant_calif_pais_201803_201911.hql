...
--INSERT OVERWRITE TABLE bi_corp_bdr.aux_garant_calif_pais PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}')
INSERT OVERWRITE TABLE bi_corp_bdr.aux_garant_calif_pais PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}')
    ...
    FROM 
            (SELECT 
                ...
                and partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
                and tc.gen_idioma = 'E'
            ) gen
        LEFT OUTER JOIN 
            (SELECT codigo_iso_pais
               ...
               --where rtp.partition_date = IF(to_Date(add_months(trunc('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}','MM'),1)) < '2020-04-01','2020-04-01' ,to_Date(add_months(trunc('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}','MM'),1)))
               --PDC cambio partition_date por last_working_date
               where rtp.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
                ) rtp1
