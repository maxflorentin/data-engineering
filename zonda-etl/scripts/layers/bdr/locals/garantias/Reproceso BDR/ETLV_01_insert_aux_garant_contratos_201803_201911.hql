...
INSERT OVERWRITE TABLE bi_corp_bdr.aux_garant_contratos_div PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}')
SELECT
	...
	, cast(null as string) as facturacion_total_cliente
	, sum(posic.imp_deuda) over (partition by ctoa.g4128_idnumcli) as deuda_total_cliente
	, cast(null as string) as RW_contrato
    , CASE
		...
		ELSE 1
	  END as RW_cliente
	, cast(null as string) as RW_contraparte
	, cast(null as string) as factor_reductor
	, cast(null as string) as APR_SIN_MITIGAR
	, posic.imp_deuda
	, cast(null as string) as orden_x_contrato
	, cast(null as string) as orden_contrato
FROM
    bi_corp_bdr.jm_contr_bis ctob
    INNER JOIN bi_corp_bdr.normalizacion_id_contratos nor
        ON ('{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' = nor.partition_date and ctob.g4095_contra1 = nor.id_cto_bdr and nor.`source` ='credito')
    INNER JOIN
        (SELECT pos.partition_date, pos.e0621_contra1,pos.pos_balance, pos.orden_producto, SUM(pos.imp_deuda) as imp_deuda
            ...
            ,case
                when (E0621_CTACGBAL is null or trim(E0621_CTACGBAL)='') then '1'
        ...
        ON (posic.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' and ctob.g4095_contra1 = posic.e0621_contra1)
    LEFT OUTER JOIN
        (
        SELECT c.g4128_s1emp, c.g4128_idnumcli, c.g4128_contra1 FROM bi_corp_bdr.jm_interv_cto c where c.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' AND c.g4128_tipintev='00001'
        ) ctoa
	...
	LEFT OUTER JOIN
		...
		FROM bi_corp_staging.malpe_pedt001 P01
		    ...
		    ON  ('{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' = aux_garant_calif_pais.partition_date and p01.pepaires = aux_garant_calif_pais.pais)
    	where P01.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
    	) pers
	    ON (SUBSTRING(ctoa.g4128_idnumcli,2,8) = pers.penumper)
	LEFT OUTER JOIN
	     bi_corp_bdr.aux_garant_calif_empresa rtg_empresa
	     ON (SUBSTRING(ctoa.g4128_idnumcli,2,8) = rtg_empresa.alias_nup and rtg_empresa.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' )
	LEFT OUTER JOIN
			(
				...
				WHERE data_date_part = translate('{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}','-','')
						and ind_divisa = 'D' and ind_cotizacion = 'S' and cod_segmento = ''
						and concat(substr(fec_cambio, 1,4), substr(fec_cambio, 6,2), substr(fec_cambio, 9,2)) = translate('{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}','-','')
						and cod_divisa = 'EUR'
				limit 1
			)
			cotizaciones
			ON (cotizaciones.data_date_part = translate('{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}','-','') )
WHERE ctob.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'
;
...
SELECT
	...
	, cast(null as string) as orden_contrato
FROM bi_corp_bdr.aux_garant_contratos_div where (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}')
