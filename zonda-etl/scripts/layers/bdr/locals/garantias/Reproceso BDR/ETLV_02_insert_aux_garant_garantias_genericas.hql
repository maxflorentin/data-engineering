...
INSERT OVERWRITE TABLE bi_corp_bdr.aux_garant_garantias_genericas PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}')
SELECT
...
FROM
	...
	INNER JOIN bi_corp_staging.gtdtrgc rgc
		ON (    rgc.gttcrgc_rgc_num_garantia = mae.gttcmae_mae_num_garantia
			...
			and rgc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
			and rgc.gttcrgc_rgc_fec_inivali <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
			and rgc.gttcrgc_rgc_fec_finvali >= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
			)
	LEFT OUTER JOIN
	    (SELECT * FROM bi_corp_staging.gtdtrgb WHERE partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
	...
	cotizaciones
	ON (cotizaciones.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' and cotizaciones.cod_divisa = mae.gttcmae_mae_cod_divisa)
	LEFT OUTER JOIN
	     bi_corp_bdr.aux_garant_calif_empresa rtg_empresa
	     ON (bien.gttcrbc_rbc_num_persona = rtg_empresa.alias_nup and rtg_empresa.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}' )
	LEFT OUTER JOIN
...
WHERE 1=1
	...
	and mae.gttcmae_mae_fec_vcto >= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
...
SELECT
	...
	CAST( row_number() OVER(PARTITION BY  num_persona order by orden_cod_garantia asc, RW_GARANTIA asc, fec_vcto desc, imp_limite_pesos desc) as INT) as orden,
        ...
        cod_garantia_bdr,
        	...
        	COD_BIEN,
        	COD_POSTAL
FROM bi_corp_bdr.aux_garant_garantias_genericas where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}';