...
INSERT OVERWRITE TABLE bi_corp_bdr.aux_garant_garantias_contratos_div PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}')
SELECT
      gc1.cod_entidad
    , gc1.num_persona
    , gc1.id_cto_div
...
(
        ...
        where 1=1
            and cto.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
        ...
        where 1=1
        	...
        	and cto2.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
        ...
        WHERE cto3.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
...
WHERE
	gc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
...
FROM
	...
	LEFT OUTER JOIN
		...
		ON (ini.cod_entidad = gc.cod_entidad and ini.num_persona = gc.num_persona and ini.id_cto_div = gc.id_cto_div and ini.num_garantia = gc.num_garantia
		    and ini.partition_date < '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}')
WHERE
	gc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201912_202103') }}'
	 --and gc.num_persona = '00026343'
	   --and gc.id_cto IN ('0072_0493_039100025848_39_0037_ARS','0072_0493_322140501061_21_0012_ARS')
