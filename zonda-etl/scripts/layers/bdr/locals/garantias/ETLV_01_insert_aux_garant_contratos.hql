...
INSERT OVERWRITE TABLE bi_corp_bdr.aux_garant_contratos_div PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}')
SELECT
...
FROM
    ...
    INNER JOIN bi_corp_bdr.normalizacion_id_contratos nor
        ON ('{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}' = nor.partition_date and ctob.g4095_contra1 = nor.id_cto_bdr and nor.`source` ='credito')
    INNER JOIN
        ...
        ON (posic.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}' and ctob.g4095_contra1 = posic.e0621_contra1)
    LEFT OUTER JOIN
        (
        SELECT c.g4128_s1emp, c.g4128_idnumcli, c.g4128_contra1 FROM bi_corp_bdr.jm_interv_cto c where c.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}' AND c.g4128_tipintev='00001'
        ) ctoa
	...
	LEFT OUTER JOIN
		...
		FROM bi_corp_staging.malpe_pedt001 P01
		    ...
		    ON  ('{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}' = aux_garant_calif_pais.partition_date and p01.pepaires = aux_garant_calif_pais.pais)
    	where P01.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}'
    	) pers
	    ON (SUBSTRING(ctoa.g4128_idnumcli,2,8) = pers.penumper)
	LEFT OUTER JOIN
	     bi_corp_bdr.aux_garant_calif_empresa rtg_empresa
	     ON (SUBSTRING(ctoa.g4128_idnumcli,2,8) = rtg_empresa.alias_nup and rtg_empresa.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}' )
	LEFT OUTER JOIN
			(
				...
				WHERE data_date_part = translate('{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}','-','')
						and ind_divisa = 'D' and ind_cotizacion = 'S' and cod_segmento = ''
						and concat(substr(fec_cambio, 1,4), substr(fec_cambio, 6,2), substr(fec_cambio, 9,2)) = translate('{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}','-','')
						and cod_divisa = 'EUR'
				limit 1
			)
			cotizaciones
			ON (cotizaciones.data_date_part = translate('{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}','-','') )
WHERE ctob.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
;
...
SELECT
	...
	, cast(null as string) as orden_contrato
FROM bi_corp_bdr.aux_garant_contratos_div where (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}')
