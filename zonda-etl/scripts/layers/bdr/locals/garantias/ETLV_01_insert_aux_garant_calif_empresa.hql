...
INSERT OVERWRITE TABLE bi_corp_bdr.aux_garant_calif_empresa PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}')
	...
	FROM
		...
		INNER JOIN
			...
			where rte.partition_date= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}'
