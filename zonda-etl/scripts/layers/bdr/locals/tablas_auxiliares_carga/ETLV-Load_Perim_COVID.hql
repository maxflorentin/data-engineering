...
with  contrato_org as
 ...
 SELECT distinct entidad
    ...
    and partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malug_ugdtmae', dag_id='BDR_LOAD_Tables-Monthly') }}'
) 
...
select corg.entidad
    ...
    where df.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malug_ugdtmae', dag_id='BDR_LOAD_Tables-Monthly') }}'
union all
 SELECT     entidad
            ,oficina
            ,cuenta
