...
SELECT  
        '{{ ti.xcom_pull(task_ids='InputConfig', key='next_month_first_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}'  as fec_dato
        ,'{{ ti.xcom_pull(task_ids='InputConfig', key='month', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' as fec_periodo
    ...
    from    (
                   ...
                   where  rating_interno != 'SIN_CALIFI'  
                    and partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='next_month_first_day', dag_id='BDR_LOAD_Tables-Reproceso_BDR_Monthly_201803_201911') }}' 
            ) acsg
        inner join 
            ( 
                    select mkg.nup
