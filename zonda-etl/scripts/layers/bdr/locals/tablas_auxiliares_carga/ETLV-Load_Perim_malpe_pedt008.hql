...
from (
       ...
       where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt008', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
              and pecalpar = 'TI'
) subtable
where orden = 1
	and (fec_baja BETWEEN add_months('{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt008', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}', -12) 
                         and '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt008', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
