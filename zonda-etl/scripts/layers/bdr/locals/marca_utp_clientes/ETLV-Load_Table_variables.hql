...
select distinct num_persona as num_persona, 
                fec_alta_submarca_cliente as max_fec_inicio_vigencia,
                case when substring(cm.partition_date, 1, 7) <= '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='LOAD_BDR_Marca_UTP_Variables') }}'
                     then cm.partition_date else cast(null as string)
                     end as max_fec_baja,
                'mora' as source
from bi_corp_staging.view_clientes_en_mora cm
where cm.cod_submarca_cliente IN ('20', '22') and cm.partition_date = '{{ var.json.marca_utp_clientes.clientes_mora_lwd }}'
;

CREATE TEMPORARY TABLE bi_corp_bdr.marca_comite as
select distinct mc.num_persona, mc.fec_inicio_vigencia, 
                case when mc_old.fec_inicio_vigencia is null 
                     then 'y' else 'n' end as inicio_vigencia, 
                mc.fec_revision as fec_baja, 
                mc.data_date_part
...
where mc.data_date_part <= '{{ var.json.marca_utp_clientes.marca_comite_lwda }}'
...
case when max_fec_baja >= max_fec_inicio_vigencia and substring(max_fec_baja, 1, 7) <= '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='LOAD_BDR_Marca_UTP_Variables') }}'
then max_fec_baja
...
'{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='LOAD_BDR_Marca_UTP_Variables') }}' as partition_date -- MONTH PARTITION BDR
