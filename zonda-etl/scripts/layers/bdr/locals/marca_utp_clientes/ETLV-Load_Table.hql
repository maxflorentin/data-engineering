...
case when substring(cm.partition_date, 1, 7) <= '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
then cm.partition_date else null
end as max_fec_baja,
'mora' as source
from bi_corp_staging.view_clientes_en_mora cm
where cm.cod_submarca_cliente IN ('20', '22') and cm.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly') }}'
;
...
where mc.data_date_part <= '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_marca_comite', dag_id='BDR_LOAD_Tables-Monthly') }}'
...
case when max_fec_baja >= max_fec_inicio_vigencia and substring(max_fec_baja, 1, 7) <= '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}'
then max_fec_baja
...
'{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly') }}' as partition_date -- MONTH PARTITION BDR
