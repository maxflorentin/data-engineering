...
INSERT OVERWRITE TABLE bi_corp_bdr.test_balances_ajustes 
...
FROM (select trim(blc.cargabal) as cargabal,
        ...
        where blc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
      ...
      union all  
        select trim(pra.cargabal) as cargabal, 
            ...
            where pra.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
       ...
       select trim(sda.cargabal) as cargabal, 
            ...
            where sda.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
