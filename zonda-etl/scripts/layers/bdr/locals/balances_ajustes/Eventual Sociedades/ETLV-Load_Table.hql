...
insert overwrite table bi_corp_bdr.balances_ajustes partition(partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}')
select 
blc.cargabal,
blc.sociedad,
...
FROM (select trim(blc.cargabal) as cargabal,
        ...
        where blc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
      ...
      union all  
        select trim(pra.cargabal) as cargabal, 
            ...
            where pra.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
       ...
       select trim(sda.cargabal) as cargabal, 
            ...
            where sda.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
