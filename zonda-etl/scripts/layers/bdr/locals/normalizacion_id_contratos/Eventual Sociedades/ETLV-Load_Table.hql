...
WHERE a.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
AND (a.cod_estado_rel_cliente is null or (NOT (a.cod_estado_rel_cliente = 'C' and a.fec_baja_rel_cliente <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}')))
...
WHERE core.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt042', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
AND core.PEESTOPE = 'C'
and substr(core.PEFECEST,1,7) = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
UNION ALL
...
, '' mmff_cod_pata, '' blc_sociedad, '' blc_cargabal, '' blc_sociedad_eliminacion
FROM santander_business_risk_arda.contratos a
WHERE a.data_date_part between     '{{ ti.xcom_pull(task_ids='InputConfig', key='first_working_day_arda', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
                               and '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
  AND a.cod_marca = "FA"
  AND substr(a.fec_castigo,1,7) = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
UNION ALL
...
WHERE partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
...
WHERE r.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_mddtccn', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
  and r.fec_altareg between     '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
                            and '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
UNION ALL
-- perimetro de renumeraciones (CONTRATO VIEJO)
SELECT
'credito' source
...
LEFT JOIN bi_corp_staging.malpe_pedt008 p
     ...
     and p.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt008', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
INNER JOIN productos_riesgos pr on pr.cod_producto = p.pecodpro
WHERE r.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malbgc_zbdtmig', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
  and r.mig_old_fech_baja between     '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
                                  and '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
UNION ALL
-- perimetro de renumeraciones (CONTRATO VIEJO)
SELECT
...
WHERE r.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_peec867c', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
  and r.pefecest between     '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
                         and '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
UNION ALL
     ...
     (select max(a.data_date_part) as data_date_part, a.fec_info, a.cod_cartera, a.cod_especie, a.cod_sis_origen, a.cod_disponibilidad
      ...
      inner join
                (select max(b.fec_info) as fec_info, b.cod_cartera, b.cod_especie, b.cod_sis_origen, b.cod_disponibilidad
                  ...
                  where b.data_date_part <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day_arda', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
                    and b.fec_info <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
       ...
       where a.data_date_part <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day_arda', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
         and a.fec_info <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
...
from (
  ...
  where b.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
  and b.fec_data <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
  and b.cod_cartera in ('ALC', 'INV')
  and b.cod_pata = 2
) ds
UNION ALL
...
where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_mmff_tactico_especie', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
union all
...
where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_corresponsales', dag_id='BDR_LOAD_Tables-Sociedades-Monthly') }}'
) contratos;
