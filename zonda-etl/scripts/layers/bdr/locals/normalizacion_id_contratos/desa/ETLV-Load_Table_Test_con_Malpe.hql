...
--WHERE a.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
WHERE a.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_contratos', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
--AND (a.cod_estado_rel_cliente is null or (NOT (a.cod_estado_rel_cliente = 'C' and a.fec_baja_rel_cliente <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}')))
AND (a.cod_estado_rel_cliente is null or (NOT (a.cod_estado_rel_cliente = 'C' and a.fec_baja_rel_cliente <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}')))
...
--WHERE core.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
WHERE core.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt042', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
AND core.PEESTOPE = 'C'
and substr(core.PEFECEST,1,7) = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
UNION ALL
...
, '' mmff_cod_pata, '' blc_sociedad, '' blc_cargabal, '' blc_sociedad_eliminacion
FROM santander_business_risk_arda.contratos a
WHERE a.data_date_part between     '{{ ti.xcom_pull(task_ids='InputConfig', key='first_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
                               and '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
  AND a.cod_marca = "FA"
  AND substr(a.fec_castigo,1,7) = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
UNION ALL
...
FROM bi_corp_bdr.test_balances_ajustes
...
WHERE r.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_mddtccn', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
  and r.fec_altareg between     '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
                            and '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
UNION ALL
-- perimetro de renumeraciones (CONTRATO VIEJO)
SELECT
'credito' source
...
LEFT JOIN bi_corp_bdr.perim_malpe_pedt008_1er_titular p
    ...
    AND p.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
...
WHERE r.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malbgc_zbdtmig', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
    AND r.mig_old_fech_baja between '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
                                and '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
UNION ALL
-- perimetro de renumeraciones (CONTRATO VIEJO)
SELECT
...
--WHERE r.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
WHERE r.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_peec867c', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
  and r.pefecest between     '{{ ti.xcom_pull(task_ids='InputConfig', key='first_calendar_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
                         and '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
UNION ALL
     ...
     (select max(a.data_date_part) as data_date_part, a.fec_info, a.cod_cartera, a.cod_especie, a.cod_sis_origen, a.cod_disponibilidad
      ...
      inner join
                (select max(b.fec_info) as fec_info, b.cod_cartera, b.cod_especie, b.cod_sis_origen, b.cod_disponibilidad
                  ...
                  where b.data_date_part <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day_arda', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
                    and b.fec_info <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
       ...
       where a.data_date_part <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day_arda', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
         and a.fec_info <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
...
from (
  ...
  where b.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
  and b.fec_data <= '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
  and b.cod_cartera in ('ALC', 'INV')
  and b.cod_pata = 2
) ds
UNION ALL
...
where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='bdr_max_partition_mmff_tactico_especie', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
union all
...
where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_corresponsales', dag_id='BDR_LOAD_Tables-Monthly-Desa') }}'
) contratos;
...
INSERT OVERWRITE TABLE bi_corp_bdr.test2_normalizacion_id_contratos_history
select id_cto_bdr
  ,source
  ,id_cto_source
  ,cred_cod_entidad
...
INSERT OVERWRITE TABLE bi_corp_bdr.test2_normalizacion_id_contratos
...
LEFT JOIN bi_corp_bdr.test2_normalizacion_id_contratos_history hs ON hs.id_cto_source = ds.id_cto_source;