...
WHERE a.data_date_part = '20210129'
AND (a.cod_estado_rel_cliente is null or (NOT (a.cod_estado_rel_cliente = 'C' and a.fec_baja_rel_cliente <= '2021-01-31')))
...
WHERE core.partition_date = '2021-01-29'
AND core.PEESTOPE = 'C'
and substr(core.PEFECEST,1,7) = '2021-01'
UNION ALL
...
, '' mmff_cod_pata, '' blc_sociedad, '' blc_cargabal, '' blc_sociedad_eliminacion
FROM santander_business_risk_arda.contratos a
WHERE a.data_date_part between '20210104' and '20210129'
  AND a.cod_marca = "FA"
  AND substr(a.fec_castigo,1,7) = '2021-01'
UNION ALL
...
WHERE r.partition_date = '2021-01-29' 
  and r.fec_altareg between '2021-01-01' and '2021-01-31' 
UNION ALL
-- perimetro de renumeraciones (CONTRATO VIEJO)
SELECT
'credito' source
...
LEFT JOIN bi_corp_staging.malpe_pedt008 p
     ...
     and p.partition_date = '2021-01-29'
INNER JOIN productos_riesgos pr on pr.cod_producto = p.pecodpro
WHERE r.partition_date = '2021-01-29' 
  and r.mig_old_fech_baja between '2021-01-01' and '2021-01-31' 
UNION ALL
-- perimetro de renumeraciones (CONTRATO VIEJO)
SELECT
'credito' source
...
WHERE r.partition_date = '2020-07-17' 
  and r.pefecest between  '2021-01-01' and '2021-01-31' 
UNION ALL
...
WHERE r.partition_date = '2020-07-17' 
and r.pefecest between '2021-01-01' 
                       and '2021-01-31' 
-- perimetro de MMFF
-- ESPECIES
UNION ALL
     ...
     (select max(a.data_date_part) as data_date_part, a.fec_info, a.cod_cartera, a.cod_especie, a.cod_sis_origen, a.cod_disponibilidad
      ...
      inner join
                (select max(b.fec_info) as fec_info, b.cod_cartera, b.cod_especie, b.cod_sis_origen, b.cod_disponibilidad
                  ...
                  where b.data_date_part <= '20210131' 
                    and b.fec_info <= '2021-01-31' 
       ...
       where a.data_date_part <= '20210131' 
         and a.fec_info <= '2021-01-31' 
...
from (
  ...
  where b.data_date_part = '20210129' 
  and b.fec_data <= '2021-01-29' 
  and b.cod_cartera in ('ALC', 'INV')
  and b.cod_pata = 2
) ds
UNION ALL
...
where partition_date = '2020-09-01' 
) contratos;
