...
DROP TABLE IF EXISTS temp_saldos_depositos_prestamos_M;
CREATE TEMPORARY TABLE temp_saldos_depositos_prestamos_M
AS
SELECT
    ...
    sum(RE.fc_ren_saldo_puntual_ml) * (RE.fc_ren_tasint / 100) as fc_ren_saldo_ponderado_ml,
    sum(RE.fc_ren_saldo_puntual_mo) * (RE.fc_ren_tasint / 100) as fc_ren_saldo_ponderado_mo,
    count(DISTINCT RE.cod_ren_contrato) as fc_ren_contratos

FROM
   bi_corp_common.bt_ren_result AS RE
LEFT JOIN bi_corp_common.dim_ren_productosctrco prod2 ON RE.cod_ren_producto_gest = prod2.cod_ren_producto
WHERE
  RE.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_to', dag_id='LOAD_BSN_Rentabilidad_PrestamosYDepositos_Monthly') }}'
  AND
    (
...
INSERT OVERWRITE TABLE bi_corp_business.agg_cdg_prestamosdepositos_monthly
...
SELECT
    ...
    RD.partition_date
FROM
    temp_saldos_depositos_prestamos_M RD
    LEFT JOIN bi_corp_common.dim_areanegocio_marca adn ON RD.AREA_NEGOCIO = adn.cod_ren_area_negocio
    and adn.partition_Date = case when '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_BSN_Rentabilidad_PrestamosYDepositos_Monthly') }}' < '2021-04-21' then '2021-04-21' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_dim_areanegocio_marca', dag_id='LOAD_BSN_Rentabilidad_PrestamosYDepositos_Monthly') }}' end
