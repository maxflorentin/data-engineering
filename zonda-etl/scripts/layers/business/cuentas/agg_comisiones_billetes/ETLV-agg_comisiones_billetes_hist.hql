...
and c.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_BSN_Cuentas_Comisiones_Billetes-Hist') }}'
...
WHERE prod.DATA_DATE_PART = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_producto', dag_id='LOAD_BSN_Cuentas_Comisiones_Billetes-Hist') }}';
...
WHERE SUBP.DATA_DATE_PART = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_subproducto', dag_id='LOAD_BSN_Cuentas_Comisiones_Billetes-Hist') }}';
...
where z.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_rio44_ba_suc_zona', dag_id='LOAD_BSN_Cuentas_Comisiones_Billetes-Hist') }}';
...
where suc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_tcdt050', dag_id='LOAD_BSN_Cuentas_Comisiones_Billetes-Hist') }}';
...
'{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_BSN_Cuentas_Comisiones_Billetes-Hist') }}' as partition_date
...
LEFT JOIN ( SELECT cod_cue_cuenta, cod_suc_sucursal, cod_cue_divisa, dt_cue_fecha_mov, cod_cue_producto,cod_cue_subproducto,
					...
					WHERE partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_BSN_Cuentas_Comisiones_Billetes-Hist') }}'
...
LEFT JOIN bi_corp_staging.bgdtmae mae on IMP.CENTRO = mae.centro_alta
										...
										AND mae.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_bgdtmae', dag_id='LOAD_BSN_Cuentas_Comisiones_Billetes-Hist') }}'
LEFT JOIN bi_corp_staging.malpe_ptb_pedt008 A on imp.CUENTA = concat('0', A.pecodpro, SUBSTRING(A.penumcon, 4))
											...
											AND A.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_ptb_pedt008', dag_id='LOAD_BSN_Cuentas_Comisiones_Billetes-Hist') }}'
											AND A.pecdgent = '0072'
											AND A.pecalpar = 'TI'
											AND A.pecodpro in ('02', '03','05', '06', '07')
LEFT JOIN bi_corp_common.stk_per_personas PER on PER.COD_PER_NUP= cast(A.penumper as int)
											AND per.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_stk_per_personas', dag_id='LOAD_BSN_Cuentas_Comisiones_Billetes-Hist') }}'
LEFT JOIN bi_corp_common.stk_cue_campanias cam ON IMP.CUENTA = cam.cod_cue_cuenta
											...
											AND cam.partition_date= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_BSN_Cuentas_Comisiones_Billetes-Hist') }}'
...
ELSE NULL END,
IMP.FECHA,
'{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_BSN_Cuentas_Comisiones_Billetes-Hist') }}'