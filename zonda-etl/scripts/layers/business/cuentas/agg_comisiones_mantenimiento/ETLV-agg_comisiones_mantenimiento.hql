...
DROP TABLE IF EXISTS temp_max_fecha_comcob;
CREATE TEMPORARY TABLE temp_max_fecha_comcob
AS
select max(c.partition_date) as partition_date from bi_corp_common.bt_cue_comisiones_cobradas c
where c.cod_cue_concepto='MANT' AND c.partition_date in (select max(partition_date) from bi_corp_common.bt_cue_comisiones_cobradas
						                                    WHERE cod_cue_concepto='MANT' AND partition_date >= date_sub(current_date(), 7) AND partition_date <= to_date(current_date()) );
...
and tdgb.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malbgc_tbgb001', dag_id='LOAD_BSN_Cuentas_Comisiones_Mantenimiento-Daily') }}';
...
and mco.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malbgc_bgdtmco', dag_id='LOAD_BSN_Cuentas_Comisiones_Mantenimiento-Daily') }}'
UNION ALL
select
distinct
mco2.mco_suscriptor,
...
where mco2.partition_date  = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malbgc_bgdtmco', dag_id='LOAD_BSN_Cuentas_Comisiones_Mantenimiento-Daily') }}';
...
WHERE prod.DATA_DATE_PART = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_producto', dag_id='LOAD_BSN_Cuentas_Comisiones_Mantenimiento-Daily') }}';
...
WHERE SUBP.DATA_DATE_PART = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_subproducto', dag_id='LOAD_BSN_Cuentas_Comisiones_Mantenimiento-Daily') }}';
...
where z.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_rio44_ba_suc_zona', dag_id='LOAD_BSN_Cuentas_Comisiones_Mantenimiento-Daily') }}';
...
where suc.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_tcdt050', dag_id='LOAD_BSN_Cuentas_Comisiones_Mantenimiento-Daily') }}';
...
'{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='LOAD_BSN_Cuentas_Comisiones_Mantenimiento-Daily') }}' as partition_date

FROM bi_corp_staging.bgdtmae mae
INNER JOIN temp_max_fecha_comcob aux on mae.partition_date= aux.partition_date
LEFT JOIN  bi_corp_common.bt_cue_comisiones_cobradas lco on LCO.cod_suc_sucursal_cargo = mae.centro_alta
										...
										AND SUBSTRING(lco.partition_date, 1, 7) = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='LOAD_BSN_Cuentas_Comisiones_Mantenimiento-Daily') }}'
...
LEFT JOIN bi_corp_staging.malpe_ptb_pedt008 A on mae.cuenta = concat('0', A.pecodpro, SUBSTRING(A.penumcon, 4))
										...
										AND A.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_ptb_pedt008', dag_id='LOAD_BSN_Cuentas_Comisiones_Mantenimiento-Daily') }}'
										AND A.pecdgent = '0072'
										AND A.pecalpar = 'TI'
										AND A.pecodpro in ('02', '03','05', '06', '07')

LEFT JOIN bi_corp_common.stk_per_personas PER on PER.COD_PER_NUP= cast(A.penumper as int)
										AND per.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_stk_per_personas', dag_id='LOAD_BSN_Cuentas_Comisiones_Mantenimiento-Daily') }}'
...
LEFT JOIN bi_corp_common.stk_cue_campanias cam ON mae.CUENTA = cam.cod_cue_cuenta
												...
												AND cam.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_stk_cue_campanias', dag_id='LOAD_BSN_Cuentas_Comisiones_Mantenimiento-Daily') }}'
...
WHERE mae.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_bgdtmae', dag_id='LOAD_BSN_Cuentas_Comisiones_Mantenimiento-Daily') }}'
...
'{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='LOAD_BSN_Cuentas_Comisiones_Mantenimiento-Daily') }}'
