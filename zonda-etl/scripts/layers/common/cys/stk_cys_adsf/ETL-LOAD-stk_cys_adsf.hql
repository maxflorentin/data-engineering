...
SELECT 'D.ADIC' cod_cys_interface
	...
	, NULL ds_cys_bucket
	, '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_to', dag_id='LOAD_CMN_CyS-Monthly') }}' partition_date
...
LEFT JOIN bi_corp_staging.malpe_ptb_pedt030 p30 
	...
	AND p30.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_prevmonth_malpe_pedt030', dag_id='LOAD_CMN_CyS-Monthly') }}' -- 
...
LEFT JOIN bi_corp_staging.risksql5_productos p 
	...
	AND p.fecha_informacion = '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_to', dag_id='LOAD_CMN_CyS-Monthly') }}'
WHERE rda.data_date_part = '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_last_working_day_arda', dag_id='LOAD_CMN_CyS-Monthly') }}' ) -- 
...
LEFT JOIN bi_corp_staging.risksql5_productos p 
	...
	AND p.fecha_informacion = '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_to', dag_id='LOAD_CMN_CyS-Monthly') }}'
WHERE CA.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_to', dag_id='LOAD_CMN_CyS-Monthly') }}' ; -- 
