...
where p8a.partition_date = '2021-03-10'
and p8a.pecalpar = 'TI'
...
where p15a.partition_date = '2021-03-10'
...
where p1.partition_date = '2021-03-10';
...
select
      ...
      '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Transferencias_Hist') }}' partition_date
...
where trf.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Transferencias_Hist') }}';
...
where trf.partition_date between '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Transferencias_Hist') }}'
and last_day('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Transferencias_Hist') }}');
...
left join bi_corp_common.dim_trf_conceptos con
	on (tatrc.cod_trf_concepto = con.cod_trf_concepto
		...
		and '2021-03-10' = con.partition_date)
left join bi_corp_common.dim_trf_productos pro
	on (tatrc.cod_trf_producto = pro.cod_trf_producto
		...
		and '2021-03-10'= pro.partition_date)
left join bi_corp_common.dim_trf_camaras cam
	on (tatrc.cod_trf_camara = cam.cod_trf_camara
		...
		and '2021-03-10' = cam.partition_date)
left join bi_corp_common.dim_trf_canales can
	on (tatrc.cod_trf_canal = can.cod_trf_canal
		...
		and '2021-03-10' = can.partition_date)
left join bi_corp_common.dim_trf_rechazos rec
	on (tatrc.cod_trf_mot_rechazo = rec.cod_trf_rechazo
		...
		and '2021-03-10' = rec.partition_date)
left join bi_corp_common.dim_trf_entidades eto
	on (tatrc.cod_trf_entidad_origen = eto.cod_trf_entidad
		...
		and '2021-03-10' = eto.partition_date)
left join bi_corp_common.dim_trf_entidades etd
	on (tatrc.cod_trf_entidad_destino = etd.cod_trf_entidad
		...
		and '2021-03-10' = etd.partition_date)
union all
select
