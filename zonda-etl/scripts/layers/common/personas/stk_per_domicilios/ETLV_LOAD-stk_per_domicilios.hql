...
INSERT OVERWRITE TABLE bi_corp_common.stk_per_domicilios
PARTITION(partition_date)
SELECT
CAST(malpe_pedt003.penumper as BIGINT) cod_per_nup
...
, trim(substr(malpe_pedt003.peobserv, 3, 20)) ds_per_edificio
...
, trim(malpe_pedt003.pedesloc) ds_per_localidad
, malpe_pedt003.pecodpro cod_per_cod_provincia
, trim(b.provincia) ds_per_provincia
, trim(malpe_pedt003.pecodpos) ds_per_cod_postal
, trim(malpe_pedt003.pecodpai) cod_per_pais
, trim(c.pais) ds_per_pais
, CASE WHEN malpe_pedt003.petitdom = 'SI' THEN 1 ELSE 0 END flag_per_titular_domicilo
, CASE WHEN malpe_pedt003.pemarnor = 'S' THEN 1 ELSE 0 END flag_per_normalizacion
, CASE WHEN malpe_pedt003.pemardom = 'S' THEN 1 ELSE 0 END flag_per_domicilio_erroneo
, CASE WHEN malpe_pedt003.pemarcor = 'S' THEN 1 ELSE 0 END flag_per_correspondencia
...
FROM bi_corp_staging.malpe_ptb_pedt003 malpe_pedt003
LEFT OUTER JOIN 
...
LEFT OUTER JOIN 
	...
	FROM bi_corp_staging.tcdtgen pais WHERE pais.gentabla = '0112' AND pais.gen_idioma = 'E' AND pais.partition_date IN (SELECT max(partition_date)
	FROM bi_corp_staging.tcdtgen)
	) c ON trim(c.pecodpai) = trim(malpe_pedt003.pecodpai) 
WHERE NOT malpe_pedt003.petipdom LIKE 'ELE'
AND malpe_pedt003.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Daily') }}'
"