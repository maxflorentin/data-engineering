...
WHERE partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'
and trim(petipdoc) = 'T'
...
WHERE partition_date =  '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}' ) p001

LEFT OUTER JOIN
(SELECT penumper, pemaremp flag_empleado, petipact
FROM bi_corp_staging.malpe_pedt002
WHERE partition_date =  '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}' ) p002
	ON p001.penumper = p002.penumper

LEFT JOIN
(SELECT penumper, peramemp
FROM bi_corp_staging.malpe_pedt005
WHERE partition_date =  '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_last_working_day', dag_id='LOAD_CMN_Personas-Hist') }}' ) p005
...
WHERE partition_date =  '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}' )p8 GROUP BY penumper) p008
...
where partition_Date=IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}' <= '2021-02-18','2021-02-18','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}' )and
 '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'
 	between pedt009_pefecini  and  case when pedt009_pefecfin ='9999-12-31' then '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'
...
and peestref = 'R' ) p16 where orden1=1 ) p016
 ...
 WHERE pe30.partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')p030
...
WHERE partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')p52 GROUP BY penumper) p052
...
(SELECT max(a.pedt036_peimping) peimping, a.pedt036_penumper penumper
 ...
 WHERE  a.pedt036_pefecing IN (select max(b.pedt036_pefecing)
                            ...
                            b.pedt036_pefecing  <= IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}' <  '2020-06-26',  '2020-06-26','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}' )
                            and a.pedt036_penumper=b.pedt036_penumper)

    and a.partition_date =  IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}' <  '2020-06-26',  '2020-06-26','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')
...
WHERE partition_date =  '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}' ) p223
...
WHERE partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}' ) p040
	ON p001.penumper = p040.penumper

LEFT JOIN
(SELECT penumper, pecodafip
FROM bi_corp_staging.malpe_pedt063
WHERE partition_date =  '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}' ) p063
	ON p001.penumper = p063.penumper

LEFT JOIN
(SELECT tcdtrac_cod_aco_afip , tcdtrac_cod_aco_csf10
 FROM  bi_corp_staging.maltc_tcdtrac
 where partition_date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}' < '2020-06-16','2020-06-16','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')) tcdtrac
...
(select p.penumpar, p.penumgru, p.pedesgru
 ...
 (select row_number() OVER (partition by p25.penumpar  order by p25.pefecalt desc) as orden,
      ...
      WHERE p25.partition_date=  '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_last_working_day', dag_id='LOAD_CMN_Personas-Hist') }}'
 	  AND p24.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_last_working_day', dag_id='LOAD_CMN_Personas-Hist') }}') p WHERE p.orden=1) p025_024
...
where gentabla = '0116' AND
	partition_Date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'< '2020-01-08','2020-01-08','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')) tcd_0116
...
(select trim(gen_clave) as peforjur,trim(substr(gen_datos,1,40)) as descripcion
	...
	partition_Date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'< '2020-01-08','2020-01-08','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')) tcd_0231
...
(	select trim(gen_clave) as petipocu,trim(substr(gen_datos,1,40)) as descripcion
	...
	partition_Date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'< '2020-01-08','2020-01-08','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')) tcd_0305
...
where gentabla = '1677' AND
	partition_Date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'< '2020-01-08','2020-01-08','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')) tcd_1677 --46
...
partition_Date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'< '2020-01-08','2020-01-08','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')) tcd_0255
...
LEFT JOIN (
 select trim(substr(gen_datos,1,40)) descripcion, substr(gen_clave,2,1) codigo
		...
		partition_Date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'< '2020-01-08','2020-01-08','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')) tcd_1154
...
(select trim(substr(gen_clave,1,1)) as petipdoc, gen_datos as descripcion
	...
	partition_Date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'< '2020-01-08','2020-01-08','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')) tcd_0113
...
LEFT JOIN (
	...
	partition_Date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'< '2020-01-08','2020-01-08','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')) tcd_0304
...
where gentabla = '0112' and gen_idioma = 'E' and
	partition_Date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'< '2020-01-08','2020-01-08','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')) tcd_0112
...
where gentabla = '0351' AND
	partition_Date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'< '2020-01-08','2020-01-08','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')) tcd_0351
...
where gentabla = '0508' AND
	partition_Date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'< '2020-01-08','2020-01-08','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')) tcd_0508
...
partition_Date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'< '2020-01-08','2020-01-08','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}')) tcd_0680_mip
...
left join(
		...
		where partition_date= IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}' < '2020-04-03','2020-04-03','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}' )) contra
...
(select me.dt_test_inversor_ult_fecha dt_test_inversor_ult_fecha ,
   ...
   from (select  row_number() OVER (partition by core.nup_cliente order by from_unixtime(unix_timestamp(core.fecha_test,'dd/MM/yyyy'), 'yyyy-MM-dd') desc) as orden,
        ...
        where cat.partition_date  = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'
        and core.partition_date <= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}') me
...
FROM (select waguxdex_con_diaimpag,
       ...
       WHERE partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}') g group by g.waguxdex_num_persona) garra
...
(select distinct isec_nup, isec_cod_cla_efd situacion_bcra
 ...
 WHERE partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='first_working_day', dag_id='LOAD_CMN_Personas-Hist') }}' ) adsf
...
(
	...
	where  partition_Date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'
		and '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'>='2021-04-08'
	union all
...
(
	...
	where partition_Date= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}'
		and '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Personas-Hist') }}' >= '2021-03-14'
		and fecha_fin='null'
	union all
	select penumper, 
