...
SELECT da.dt_tie_date 
	...
	, IF(fe.`200_nbfhfest` IS NOT NULL AND TRIM(fe.`200_nbfhfest`) != 'SABADO', fe.`200_nbfhfest`, cast(NULL as string)) ds_tie_holiday
	-- ------------------------------------------------------------------
	...
	, '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_MALTC_tcdv0200') }}' partition_date
FROM dates da
LEFT JOIN bi_corp_staging.maltc_tcdv0200 fe ON
	fe.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_MALTC_tcdv0200') }}'
