...
AND solicitante.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Zendesk') }}'
left join bi_corp_common.stk_zen_usuarios remitente
on trim(ticket.submitter_id)=remitente.cod_zen_usuario
AND remitente.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Zendesk') }}'
left join bi_corp_common.stk_zen_usuarios asignado
on trim(ticket.assignee_id)= asignado.cod_zen_usuario
AND asignado.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Zendesk') }}'
...
WHERE ticket.partition_date= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Zendesk') }}'
