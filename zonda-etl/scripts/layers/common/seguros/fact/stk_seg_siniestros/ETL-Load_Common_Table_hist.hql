...
CREATE TEMPORARY TABLE IF NOT EXISTS TEMP_STK_SEG_SINIESTROS as
    ...
    inner join bi_corp_staging.rio6_cg_ref_codes cg
      on (csai.csai_st_siniestro = cg.rv_low_value and
          csai.partition_date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}' <'2020-11-24','2020-11-24','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}') and
          cg.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}')
	...
	LEFT JOIN bi_corp_common.dim_seg_tipo_seguro TS
        ON (CAST(CACE.cace_carp_cd_ramo AS BIGINT) = TS.cod_seg_ramo and
            ...
            TS.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}' and
            CACE.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}'
            )
