...
select   CASE WHEN TPDT01.penumper IS NULL THEN CAST(CACN.cacn_nu_nup AS BIGINT) ELSE TPDT01.penumper END cod_per_nup,
    ...
    INNER JOIN bi_corp_staging.rio6_cart_operaciones CAOP
        ON (CAOD.caod_caop_cd_operacion = CAOP.caop_cd_operacion and
            ...
            CAOD.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}' and
            CAOP.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}')
    ...
    LEFT JOIN bi_corp_staging.rio6_cart_causas_anulacion_recibos CANA
        ON (
           ...
           and CANA.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}'
           )
WHERE NOT (
  ...
  select CASE WHEN TPDT01.penumper IS NULL THEN CAST(CACN.cacn_nu_nup AS BIGINT) ELSE TPDT01.penumper END cod_per_nup,
    ...
    INNER JOIN bi_corp_staging.rio6_autt_certificados AUCE
        ON (CACE.cace_casu_cd_sucursal = AUCE.auce_cace_casu_cd_sucursal and
            ...
            AUCE.partition_date=IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}'<'2020-11-24','2020-11-24','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}'))
    INNER JOIN bi_corp_staging.rio6_autt_vehiculos AUVE
        ON (AUVE.auve_nu_serial_carroceria = AUCE.auce_auve_nu_serial_carroceria and
            AUVE.partition_date=IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}'<'2020-11-24','2020-11-24','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}'))
...
select CAST(CACEB.cace_carp_cd_ramo AS BIGINT) cace_carp_cd_ramo,
   ...
   INNER JOIN bi_corp_staging.rio6_cart_operaciones_diarias CAODB
    ON (CAODB.caod_casu_cd_sucursal    = CACEB.cace_casu_cd_sucursal and
        ...
        CACEB.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}' and
        CAODB.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}'
       )
...
ON (CAODA.caod_casu_cd_sucursal    = CACEA.cace_casu_cd_sucursal and
    ...
    CACEA.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}' and
    CAODA.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}'
    )
...
to_date(CAOD3.caod_fe_operacion) caod_fe_operacion,
   ...
   INNER JOIN bi_corp_staging.rio6_cart_operaciones_diarias CAOD3
    ON (CAOD3.caod_casu_cd_sucursal    = CACE3.cace_casu_cd_sucursal and
        ...
        CACE3.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}' and
        CAOD3.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}'
       )
