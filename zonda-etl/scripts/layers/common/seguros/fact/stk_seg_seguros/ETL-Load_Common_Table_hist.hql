...
select CASE WHEN TPDT01.penumper IS NULL THEN CAST(CACN.cacn_nu_nup AS BIGINT) ELSE TPDT01.penumper END cod_per_nup,
    ...
    LEFT JOIN bi_corp_staging.rio6_cart_productos CAPU
        ON (CACE.cace_carp_cd_ramo     = CAPU.capu_carp_cd_ramo and
            ...
            CACE.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}' and
  ...
  LEFT JOIN bi_corp_staging.rio6_hogt_certificados HOCE
    ON (CACE.cace_casu_cd_sucursal = HOCE.hoce_cace_casu_cd_sucursal and
      ...
      HOCE.partition_date=IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}'<'2020-11-29','2020-11-29','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}')
       )
  LEFT JOIN bi_corp_staging.rio6_hogt_tp_vivienda HOTV
    ON (HOCE.hoce_tp_vivienda = HOTV.hotv_tp_vivienda and
      HOTV.partition_date=IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}'<'2020-11-29','2020-11-29','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}')
       )
  ...
  LEFT JOIN bi_corp_staging.rio6_cart_estados CAES
    ON (CACE.cace_caes_cd_estado = CAES.caes_cd_estado and
        CAES.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}')
  ...
  LEFT JOIN bi_corp_staging.rio6_cart_ciudades CACI
    ...
    INNER JOIN bi_corp_staging.rio6_autt_tipos_vehiculos AUTH
      ON (AUTH.AUTH_CD_TIPO_VEHICULO = auve.auve_auth_cd_tipo_vehiculo and
          auth.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}')
