...
select  CAST(CAPB.capb_carp_cd_ramo AS BIGINT) cod_seg_ramo,
   ...
   INNER JOIN bi_corp_staging.rio6_cart_prodplanes capb
        ON (capb.capb_carp_cd_ramo     = capu.capu_carp_cd_ramo and
            ...
            capb.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}' and
...
LEFT JOIN bi_corp_staging.rio6_CG_REF_CODES CG
    ON (TPP.cod_seg_ramo = CAST(CG.rv_high_value AS BIGINT) and
        ...
        CG.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}' and
...
from bi_corp_staging.rio6_cart_certificados cace,
 ...
 where auce.auce_cace_casu_cd_sucursal = cace.cace_casu_cd_sucursal
   ...
   and cace.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}'
   and auce.partition_date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}'<'2020-11-24','2020-11-24','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}')
   and auve.partition_date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}'<'2020-11-24','2020-11-24','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}')
   and aumo.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Seguros_hist') }}'
