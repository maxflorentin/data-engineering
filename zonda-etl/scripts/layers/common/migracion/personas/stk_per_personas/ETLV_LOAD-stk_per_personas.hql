...
, TO_DATE(p001.pefecnac) dt_per_fecha_nacimiento
...
, p001.fc_per_edad
, NULL fc_cuentas
, cast(cuentas.fc_tarjetas as decimal(15,2)) fc_tarjetas
, NULL fc_prestamos
, NULL fc_seguros
, NULL fc_plazo_fijo
, NULL fc_fci
...
, NULL cod_campana
, NULL cod_sujeto
, TO_DATE(p001.pefecini) dt_fecha_inicio_actividades
...
WHERE partition_date =  '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}' ) p001

LEFT OUTER JOIN
(SELECT penumper, pemaremp flag_empleado, petipact
FROM bi_corp_staging.malpe_pedt002
WHERE partition_date =  '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}' ) p002
	ON p001.penumper = p002.penumper

LEFT JOIN
(SELECT penumper, peramemp
FROM bi_corp_staging.malpe_pedt005
WHERE partition_date =  '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_to', dag_id='LOAD_STG_Malpe-Daily') }}' ) p005
...
WHERE partition_date =  '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}' )p8 GROUP BY penumper) p008
...
, CASE WHEN SUBSTR(pesegcla,1,1) = '6' THEN 'S1'
	...
	WHEN SUBSTR(pesegcla,1,1) IN ('5','4') THEN 'C1' --PC1
...
FROM bi_corp_staging.malpe_pedt030
 WHERE partition_date =  '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}' ) p030
...
, CASE WHEN peindica IN ('AVI') THEN pevalind ELSE NULL END cod_vinculacion -- AVY
...
WHERE partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}')p52 GROUP BY penumper) p052
...
WHERE partition_date =  '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}' ) p223
...
WHERE partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}' ) p040
	ON p001.penumper = p040.penumper

LEFT JOIN
(SELECT penumper, pecodafip
FROM bi_corp_staging.malpe_pedt063
WHERE partition_date =  '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}' ) p063
...
WHERE p25.partition_date= '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_to', dag_id='LOAD_STG_Malpe-Daily') }}'
AND p24.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}') p025_024
...
(select tar.cod_per_nup_tarjeta nup, max(tar.flag_woman) flag_woman, max(tar.flag_nova) flag_nova
   from (select cod_per_nup_tarjeta,
	...
	where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}') tar group by cod_per_nup_tarjeta) tarj
...
(select cod_tcr_nup_titular nup , sum(fc_tarjetas) fc_tarjetas, max(flag_superclub) flag_superclub, max(flag_AA) flag_AA
	from (select cod_tcr_nup_titular ,
          ...
          from bi_corp_common.stk_tcr_cuentas
		  where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}' and cod_tcr_nup_titular is not null)tc GROUP by cod_tcr_nup_titular) cuentas
...
(select trim(gen_clave) as peforjur,trim(substr(gen_datos,1,40)) as descripcion
	...
	partition_Date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}') tcd_0231
...
(	select trim(gen_clave) as peforjur,trim(substr(gen_datos,1,40)) as descripcion
	...
	partition_Date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}') tcd_0305
...
LEFT JOIN (
 select trim(substr(gen_datos,1,40)) descripcion, substr(gen_clave,2,1) codigo
		...
		partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}') tcd_1154
...
(select trim(substr(gen_clave,1,1)) as petipdoc, gen_datos as descripcion
	...
	partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}') tcd_0113
...
LEFT JOIN (
	...
	partition_date= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}') tcd_0304
...
LEFT JOIN
	...
	and partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}') tcd_0680_avi
...
(select gen_clave as pevalind,substr(gen_datos,13,2) cod_mipyme, trim(substr(gen_clave, 4,1)) cod_mipyme_letra,
	...
	END ds_mipyme
from bi_corp_staging.tcdtgen
where gentabla = '0680' and gen_clave like 'MIP%' and partition_date= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}') tcd_0680_mip
...
left join(
		...
		where partition_date= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}') contra
	on p001.penumper= contra.nup

left join (
			select nro_grupo, nup
			from bi_corp_staging.sge_grupos_economicos
			where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_STG_Malpe-Daily') }}') sge
...
left join (
    		select unidad_operativa, entidad_legal cod_grupo, nombre_legal_kgl ds_codigo, glcs_grupo
				...
				where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='LOAD_STG_Malpe-Daily') }}') aqua_asoc
