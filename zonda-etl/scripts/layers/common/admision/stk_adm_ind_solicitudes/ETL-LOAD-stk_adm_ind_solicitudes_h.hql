...
where partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
) x
where x.rownum = 1
;
...
left join bi_corp_staging.alcen_tramites_ptovta_tarjeta tpt on cast(td.nro_solicitud_fe as bigint) = cast(tpt.nro_tramite_fe as bigint) and tpt.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
where td.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
...
when x.cod_adm_estadoultresolriesgos IN ('MD','IN','ER','RV','OD','OV')  AND "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}" <= date_add(to_date(trim(x.ts_adm_fecdesicionsw)), 15) THEN 'PENDIENTE'
when x.cod_adm_estadoultresolriesgos IN ('MD','IN','ER','RV','OD','OV')  AND "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}" > date_add(to_date(trim(x.ts_adm_fecdesicionsw)), 15) THEN 'VENCIDA'
...
left join bi_corp_staging.alcen_testado t on s.cod_sucursal = t.cod_sucursal and s.nro_solicitud = t.nro_solicitud  and t.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
left join bi_corp_staging.alcen_solicitud_preevaluacion sp on s.cod_sucursal = sp.cod_sucursal and s.nro_solicitud = sp.nro_solicitud  and sp.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
left join bi_corp_staging.alcen_testado t2 on sp.cod_sucursal_preev = t2.cod_sucursal and sp.nro_solicitud_preev = t2.nro_solicitud  and t2.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
left join bi_corp_staging.alcen_participante p on s.cod_sucursal = p.cod_sucursal and s.nro_solicitud = p.nro_solicitud  and trim(p.nro_participante) = '1' and p.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
left join bi_corp_staging.alcen_tdatos_adicionales da on s.cod_sucursal = da.cod_sucursal and s.nro_solicitud = da.nro_solicitud and da.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
...
left join bi_corp_staging.alcen_ttramites_hash th on concat(lpad(s.cod_sucursal, 3, '0') ,lpad(s.nro_solicitud, 17, '0')) = th.cod_tramite and th.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
left join bi_corp_staging.alcen_laboral l on s.cod_sucursal = l.cod_sucursal and s.nro_solicitud = l.nro_solicitud and trim(l.nro_participante) = '1' and trim(l.cod_emp_tpo) != '' and tpo_empleo = '0' and l.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
left join bi_corp_staging.risksql5_admision_promociones dp on substring(trim(s.cod_prom_scor),3,2) = trim(dp.promocion) and trim(dp.promocion) != '' and dp.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
left join bi_corp_staging.risksql5_admision_productos_promocion dpp on cast(substring(trim(s.cod_prom_scor),1,2) as int) = cast(trim(dpp.producto) as int) and dpp.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
left join bi_corp_staging.risksql5_admision_canales_de_venta dc on trim(s.cod_canal) = trim(dc.codigo) and dc.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
left join bi_corp_risk.parametria_sucursales ps on trim(s.cod_sucursal) = trim(ps.sucursal) and ps.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
...
left join bi_corp_common.stk_per_personas pp on pp.cod_per_nup = cast(p.ide_nup as bigint) and pp.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
left join bi_corp_staging.alcen_tconcesionario cc on trim(cc.cod_concesionario) = trim(da.cod_canal) and cc.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Admision_Ind-History') }}"
