...
(SELECT p.wabgpper_entidad wabgpper_entidad , p.wabgpper_centro_alta wabgpper_centro_alta, p.wabgpper_cuenta wabgpper_cuenta, p.wabgpper_secuencia wabgpper_secuencia , p.wabgpper_secuencia_ren , p.wabgpper_nup wabgpper_nup
 FROM (select  row_number() OVER (partition by wabgpper_entidad , wabgpper_centro_alta , wabgpper_cuenta,wabgpper_secuencia, wabgpper_secuencia_ren order by wabgpper_ordpar asc) as orden,
      ...
      WHERE partition_date =  "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_Plazo_Fijo-Common-History') }}"
...
(SELECT substr(gen_clave,1,2) as producto,substr(gen_clave,3,4) as subproducto,trim(substr(gen_Datos,20,40)) as descripcion
 ...
 WHERE gentabla = '0309' and partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_Plazo_Fijo-Common-History') }}") tcdt039
...
( SELECT substr(gen_clave,1,2) as canal ,trim(substr(gen_Datos,1,40)) as descripcion
  FROM bi_corp_staging.tcdtgen
  WHERE gentabla = '0222' and partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_Plazo_Fijo-Common-History') }}" ) tcdt0222
 ON tcdt0222.canal =  a.ipf_ipf_canal_apertura

WHERE a.partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_Plazo_Fijo-Common-History') }}"
AND  a.ipf_ipf_fecha_opera <= "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_Plazo_Fijo-Common-History') }}"
    AND a.ipf_ipf_fecha_proven > "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_Plazo_Fijo-Common-History') }}"
    AND a.ipf_ipf_ind_estado NOT IN ('Z', 'P', 'R')
    AND suc.partition_date = a.partition_date
    AND "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_Plazo_Fijo-Common-History') }}" >= "2020-07-01"
...
AND  a.ipf_ipf_fecha_opera <= "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_Plazo_Fijo-Common-History') }}"
    ...
    AND a.ipf_ipf_ind_estado NOT IN ('Z', 'P', 'R')
    AND suc.partition_date = a.partition_date
    AND "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_Plazo_Fijo-Common-History') }}" BETWEEN "2020-06-19" AND "2020-06-30"
...
TRIM(cod_pla_producto) cod_pla_producto,
...
WHERE partition_date = "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_Plazo_Fijo-Common-History') }}"
AND "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_Plazo_Fijo-Common-History') }}" <= "2020-06-18"
