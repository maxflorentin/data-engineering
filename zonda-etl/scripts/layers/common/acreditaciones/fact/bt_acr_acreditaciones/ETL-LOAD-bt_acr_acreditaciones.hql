...
left join bi_corp_common.dim_acr_tipo_acred dtp on
	(sue.partition_date = dtp.partition_date
	and sue.sistema = dtp.cod_acr_sistema)
left join bi_corp_staging.malpe_pedt001 p1n on
	(sue.partition_date = p1n.partition_date
	and sue.nup_empleado = p1n.penumper)
left join bi_corp_staging.malpe_pedt001 p1c on
	(sue.partition_date = p1c.partition_date
	and sue.nup_empleador = p1c.penumper)
where
	sue.partition_date >= trunc('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Acreditaciones') }}','MM')
	and sue.partition_date <= last_day('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Acreditaciones') }}') ;