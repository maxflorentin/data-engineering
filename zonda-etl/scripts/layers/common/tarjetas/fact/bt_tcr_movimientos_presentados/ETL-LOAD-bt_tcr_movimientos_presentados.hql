...
WHERE p08.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt008', dag_id='LOAD_CMN_Tarjetas_Credito_Movimientos_Pres-Daily') }}'
...
where partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_amas_wamas02', dag_id='LOAD_CMN_Tarjetas_Credito_Movimientos_Pres-Daily') }}';

INSERT OVERWRITE TABLE bi_corp_common.bt_tcr_movimientos_presentados
...
SELECT
    ...
    CASE WHEN trim(w600.600_monorig) IN ('','null','NULL') THEN null ELSE trim(w600.600_monorig) END                AS cod_tcr_divisa_origen,
...
LEFT JOIN pedt008_final p08
    ...
    AND (CASE w600.600_bcodest
        ...
        WHEN "72" THEN "40"
        END) = p08.pecodpro
WHERE w600.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Tarjetas_Credito_Movimientos_Pres-Daily') }}';
