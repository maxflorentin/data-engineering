...
inner join (select centro, contrato, producto, subproducto, divisa, max(cast(num_sec as int)) as num_sec,partition_date
			...
			where partition_date=(case when '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_normalizacion', dag_id='LOAD_CMN_PRESTAMOS-Daily') }}'='None' then '2019-12-09' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_normalizacion', dag_id='LOAD_CMN_PRESTAMOS-Daily') }}' end)
...
and norm.partition_date = (case when '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_normalizacion', dag_id='LOAD_CMN_PRESTAMOS-Daily') }}'='None' then '2019-12-09' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_normalizacion', dag_id='LOAD_CMN_PRESTAMOS-Daily') }}' end);
...
left join bi_corp_staging.vmalug_ugdtmae mae on contratos_di.waguxdex_cod_centro=mae.oficina and contratos_di.waguxdex_num_contrato=mae.cuenta and mae.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='LOAD_CMN_PRESTAMOS-Daily') }}'
left join bi_corp_staging.risksql5_productos p on trim(p.codigo_producto) = contratos_di.waguxdex_cod_producto and trim(p.codigo_subproducto) = contratos_di.waguxdex_cod_subprodu and p.fecha_informacion = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_risksql5_productos', dag_id='LOAD_CMN_PRESTAMOS-Daily') }}'
