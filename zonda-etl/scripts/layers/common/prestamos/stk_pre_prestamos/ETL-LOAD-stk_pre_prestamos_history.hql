...
--Correr mensual desde 2019-01-02 hasta 2019-10-01 inclusive
--Correr diario desde 2019-10-21 inclusive
...
from (
      ...
      where partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
      AND pecalpar = 'TI'
...
where 
(('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}' < '2020-08-14' and rec.partition_date = '2020-08-14')
or
('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}' >= '2020-08-14' and rec.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'))
and 
(('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}' < '2020-08-14' and mvr.partition_date = '2020-08-14')
or
('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}' >= '2020-08-14' and mvr.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'))
...
inner join (select rec.rec_entidad,
            ...
            from bi_corp_staging.malug_ugdtrec rec
            where (('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}' < '2020-08-14' and rec.partition_date = '2020-08-14')
                    or
                    ('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}' >= '2020-08-14' and rec.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'))
            and rec_indpeco ='0'      
...
inner join bi_corp_staging.gtdtveh veh on rgb.gttcrgb_rgb_num_bien=veh.gttcveh_veh_num_bien and veh.partition_date = (case when '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_gtdtveh', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'='None' then '2020-08-24' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_gtdtveh', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}' end)
left join bi_corp_staging.gtdtmae mae on mae.gttcmae_mae_num_garantia=rgb.gttcrgb_rgb_num_garantia and mae.partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
left join bi_corp_staging.gtdtgar gar on gar.gttcgar_gar_cod_garantia= mae.gttcmae_mae_cod_garantia and gar.partition_date = (case when '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_gtdtgar', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'='None' then '2020-01-07' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_gtdtgar', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}' end)
left join bi_corp_staging.rio6_autt_marcas ma on cast(veh.gttcveh_veh_cod_marcaveh as int)=cast(ma.auma_cd_marca as int) and ma.partition_date =(case when '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_rio6_autt_marcas', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'='None' then '2020-09-01' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_rio6_autt_marcas', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}' end)
left join bi_corp_staging.rio6_autt_modelos mo on cast(trim(mo.aumo_cd_modelo) as int)=cast(trim(veh.gttcveh_veh_cod_modelove) as int) and cast(trim(mo.aumo_auma_cd_marca) as int)=cast(trim(veh.gttcveh_veh_cod_marcaveh) as int) and mo.partition_date =(case when '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_rio6_autt_modelos', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'='None' then '2020-09-01' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_rio6_autt_modelos', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}' end)
where rgo.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
and rgb.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
and veh.partition_date = (case when '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_gtdtveh', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'='None' then '2020-08-24' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_gtdtveh', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}' end)
and rgo.gttcrgo_rgo_fec_altrelac <= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
and rgo.gttcrgo_rgo_fec_bajrelac >= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
and rgb.gttcrgb_rgb_fec_inivali <= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
and rgb.gttcrgb_rgb_fec_finvali >= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
...
where mae.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
and mae.feforma >= sol.dt_cys_fechalta)t;

INSERT OVERWRITE TABLE bi_corp_common.stk_pre_prestamos
    PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}')
select
...
case when (ptcosq.ptcosq_fec_primer_imp is not null and ptcosq.ptcosq_fec_primer_imp not in ('0001-01-01','9999-12-31')) then (datediff(to_date('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'), to_date(ptcosq.ptcosq_fec_primer_imp)) / 30) + 1 else 0 end as fc_pre_cantidadciclosimpagos, --RIO87
...
from bi_corp_staging.vmalug_ugdtmae mae
left join bi_corp_staging.vmalug_ugdtdrb drb on drb.entidad=mae.entidad and drb.cuenta=mae.cuenta and drb.oficina=mae.oficina and drb.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
left join bi_corp_staging.vmalug_ugdtaux aux on mae.entidad = aux.entidad and mae.oficina = aux.oficina and mae.cuenta = aux.cuenta and aux.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
left join bi_corp_common.stk_cys_integracionsucursales i1 on mae.entidad = i1.cod_cys_entidadorigen and cast(mae.oficina as int) = i1.cod_suc_sucursalorigen and cast(mae.cuenta as bigint) = i1.cod_nro_cuentaorigen and mae.DIVISA=i1.cod_div_divisaorigen and mae.PRODUCTO=i1.cod_prod_productoorigen and mae.SUBPRO=i1.cod_prod_subproductoorigen and i1.partition_date >= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}' and i1.partition_date <= '2020-08-14'
...
left join bi_corp_staging.garra_wagucdex contratos_di on contratos_di.waguxdex_cod_centro=mae.oficina and contratos_di.waguxdex_num_contrato=mae.cuenta and contratos_di.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
left join pedt70 p70 on p70.pecodofi = mae.oficina and p70.penumcon = mae.cuenta
left join bi_corp_staging.malpe_pedt042 p42 on mae.oficina =p42.pecodofi and mae.cuenta = p42.penumcon and mae.producto = p42.pecodpro and mae.subpro = p42.pecodsub and p42.pecodpro in ('35','36','37','38','39','71','74','70') and p42.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
left join ultima_cuota u on u.entidad = coalesce(i1.cod_cys_entidaddestino,mae.entidad) and cast(u.oficina as int) = coalesce(i1.cod_suc_sucursaldestino,cast(mae.oficina as int)) and cast(u.cuenta as bigint) = coalesce(i1.cod_nro_cuentadestino,cast(mae.cuenta as bigint))
left join bi_corp_staging.bgdtmae bgdt on bgdt.centro_alta=mae.centro_vin and bgdt.cuenta=mae.cuenta_vin and bgdt.divisa='ARS' and bgdt.partition_date =(case when '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_bgdtmae', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'='None' then '2019-06-19' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_bgdtmae', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}' end)
left join ultimo_recibo r on r.rec_entidad = coalesce(i1.cod_cys_entidaddestino,mae.entidad) and cast(r.rec_oficina as int) = coalesce(i1.cod_suc_sucursaldestino,cast(mae.oficina as int)) and cast(r.rec_cuenta as bigint) = coalesce(i1.cod_nro_cuentadestino,cast(mae.cuenta as bigint))
left join bi_corp_staging.malug_ptcosq ptcosq on mae.entidad = ptcosq.ptcosq_entidad and mae.oficina = ptcosq.ptcosq_oficina and mae.cuenta = ptcosq.ptcosq_cuenta and ptcosq.partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
left join bi_corp_staging.malug_rpe rpe on mae.entidad = rpe.rpe_entidad and mae.oficina = rpe.rpe_oficina and mae.cuenta = rpe.rpe_cuenta and rpe.rpe_codconli = '101' and rpe.partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malug_rpe', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
left join garantia gar on gar.entidad=mae.entidad and gar.cuenta=mae.cuenta and gar.oficina=mae.oficina
left join bi_corp_staging.moria_wtri0010 wt on wt.wtri0010_cod_suc=cast(mae.OFICINA as int) and wt.wtri0010_nro_cta=cast(mae.CUENTA as bigint) and wt.wtri0010_cod_prod=mae.PRODUCTO and wt.wtri0010_cod_subpr=mae.SUBPRO and wt.wtri0010_cod_mone=mae.DIVISA and wt.partition_date=(case when '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_moria_wtri0010', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'='None' then '2019-01-31' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_moria_wtri0010', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}' end)
left join bi_corp_staging.adsf_isec4060 adsf on adsf.isec4060_centro=mae.OFICINA and adsf.isec4060_cuenta=mae.CUENTA and adsf.isec4060_producto=mae.PRODUCTO and adsf.isec4060_subproducto =mae.SUBPRO and adsf.isec4060_divisa=mae.DIVISA and adsf.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_adsf_isec4060', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
left join bi_corp_staging.malug_ptmosq ptmosq on mae.cuenta=ptmosq.ptmosq_mae_cuenta and mae.oficina=ptmosq.ptmosq_mae_oficina and mae.entidad=ptmosq.ptmosq_mae_entidad and ptmosq.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_prestamos-History') }}'
