...
WHERE partition_date > date_sub('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_reestruct-History') }}',7)
and partition_date < '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_reestruct-History') }}';
...
WHERE s.partition_date > date_sub('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_reestruct-History') }}',7)
AND s.partition_date < '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_reestruct-History') }}';

DROP TABLE IF EXISTS ree_new;
CREATE TEMPORARY TABLE ree_new AS
select *
FROM bi_corp_common.stk_pre_reestructuraciones
WHERE partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_reestruct-History') }}';
...
INSERT OVERWRITE TABLE bi_corp_common.bt_pre_bajasreestructuraciones
    PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_reestruct-History') }}')
SELECT *
FROM (
