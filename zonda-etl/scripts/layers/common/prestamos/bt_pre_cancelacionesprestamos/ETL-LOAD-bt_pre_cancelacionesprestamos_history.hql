...
from (
      ...
      where partition_date= (case when '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_cancelacionesprest-Hist') }}' < '2020-08-07' then '2020-08-07' else '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_cancelacionesprest-Hist') }}' end)
      AND pecalpar = 'TI'
...
(case when '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_cancelacionesprest-Hist') }}' < '2020-08-07'
     then '2020-08-07'
     else '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_cancelacionesprest-Hist') }}' end);


INSERT OVERWRITE TABLE bi_corp_common.bt_pre_cancelacionesprestamos
    PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_cancelacionesprest-Hist') }}')
select distinct
...
left join ugdtmae_tmp mae on mae.entidad=mov.mov_entidad and mae.oficina=mov.mov_oficina and mae.cuenta=mov.mov_cuenta
where (('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_cancelacionesprest-Hist') }}' < '2020-08-07' and mov.partition_date = '2020-08-07')
        or
      ('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_cancelacionesprest-Hist') }}' >= '2020-08-07' and mov.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_cancelacionesprest-Hist') }}'))
and mov_feoper = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_cancelacionesprest-Hist') }}'
and mov_indretro='N'
and mov_tipo_mov='C'
