...
from (
      ...
      where partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_ptb_pedt008', dag_id='LOAD_CMN_PRESTAMOS_cuotaspendientes-Daily-History') }}'
      AND pecalpar = 'TI'
...
INSERT OVERWRITE TABLE bi_corp_common.stk_pre_cuotaspendientes
    PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_cuotaspendientes-Daily-History') }}')
select
...
left join bi_corp_staging.vmalug_ugdtmae mae on cq.cuotasq_rec_cuenta=mae.cuenta and cq.cuotasq_rec_oficina=mae.oficina and cq.cuotasq_rec_entidad=mae.entidad and mae.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_cuotaspendientes-Daily-History') }}'
inner join bi_corp_staging.malug_ptmosq ptmosq on cq.cuotasq_rec_cuenta=ptmosq.ptmosq_mae_cuenta and cq.cuotasq_rec_oficina=ptmosq.ptmosq_mae_oficina and cq.cuotasq_rec_entidad=ptmosq.ptmosq_mae_entidad and ptmosq.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_cuotaspendientes-Daily-History') }}'
left join bi_corp_staging.vmalug_ugdtdrb drb on drb.entidad=mae.entidad and drb.cuenta=mae.cuenta and drb.oficina=mae.oficina and drb.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_cuotaspendientes-Daily-History') }}'
...
where cq.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_cuotaspendientes-Daily-History') }}'
