...
from (
      ...
      where partition_date= (case when '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_pagosprest-Hist') }}' < '2020-08-14' then '2020-08-14' else '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_pagosprest-Hist') }}' end)
      AND pecalpar = 'TI'
...
left join bi_corp_staging.malug_ugdtmvr mvr on mvr.mvr_entidad = rec.rec_entidad and mvr.mvr_cuenta=rec.rec_cuenta and mvr.mvr_oficina=rec.rec_oficina and rec.rec_feliq=mvr.mvr_feliq
where
(('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_pagosprest-Hist') }}' < '2020-08-14' and rec.partition_date = '2020-08-14')
or
('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_pagosprest-Hist') }}' >= '2020-08-14' and rec.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_pagosprest-Hist') }}'))
and
(('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_pagosprest-Hist') }}' < '2020-08-14' and mvr.partition_date = '2020-08-14')
or
('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_pagosprest-Hist') }}' >= '2020-08-14' and mvr.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_pagosprest-Hist') }}'))
and mvr.mvr_indretro='N'
and mvr.mvr_feoper <= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_pagosprest-Hist') }}'
group by mvr.mvr_entidad,mvr.mvr_cuenta,mvr.mvr_oficina,mvr.mvr_feliq
having count(*) > 1;

DROP TABLE IF EXISTS ugdtmae_tmp;
CREATE TEMPORARY TABLE ugdtmae_tmp AS
select s.*
FROM bi_corp_staging.vmalug_ugdtmae s
where s.partition_date =
(case when '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_pagosprest-Hist') }}' < '2020-08-14'
     then '2020-08-14'
     else '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_pagosprest-Hist') }}' end);


INSERT OVERWRITE TABLE bi_corp_common.bt_pre_pagosprestamos
    PARTITION (partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_pagosprest-Hist') }}')
select
...
left join ugdtmae_tmp mae on mvr.mvr_entidad=mae.entidad and mvr.mvr_cuenta=mae.cuenta and mvr.mvr_oficina=mae.oficina
...
and mvr_indretro='N'
and mvr_feoper = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_PRESTAMOS_pagosprest-Hist') }}';
