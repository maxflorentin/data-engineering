...
from
(select pecdgent, pecodofi, penumcon, pecodpro, min(cast(REGEXP_REPLACE(PEORDPAR, "^0+", '')as int)) PEORDPAR
from bi_corp_staging.malpe_pedt008
where partition_date= (CASE WHEN '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cheques_Clearing') }}' <= '2019-12-04' then '2019-12-04' ELSE '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cheques_Clearing') }}' END)
...
insert overwrite table bi_corp_common.stk_chq_rechazadossantander
partition(partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cheques_Clearing') }}')
...
p1.penumper as cod_per_nupgirada,
p2.penumper as cod_per_nupacred,
cast(null as string) as cod_chq_segmento,
cast(null as string) as cod_chq_agrupacionsegmento,
cast(null as string) as cod_chq_subsegmento,
cast(null as string) as cod_chq_clasesegmento,
...
cast(null as string) as cod_chq_producto,
cast(null as string) as cod_chq_subproducto,
suc_origen as cod_suc_sucursaltrx,
...
case when trim(bh.fec_pres) in ('00000000','') then cast(null as string) else concat(substring(bh.fec_pres,1,4),'-',substring(bh.fec_pres,5,2),'-',substring(bh.fec_pres,7,2)) end as dt_chq_fechapresentacion,
case when trim(bh.fec_deposito) in ('00000000','') then cast(null as string) else concat(substring(bh.fec_deposito,1,4),'-',substring(bh.fec_deposito,5,2),'-',substring(bh.fec_deposito,7,2)) end as dt_chq_fechadeposito,
mot_rech_ppal as ds_chq_motivorechazoppal,
mot_rech_depo as ds_chq_motivorechazodepo,
...
where bh.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cheques_Clearing') }}'
and bh.estado = 'R'

union all

select
bh.banco_cmc7 as cod_chq_entidad ,
cast(bh.nro_cheque_cmc7 as bigint) as cod_chq_nrocheque ,
cast(bh.nro_cuenta_cmc7 as bigint) as cod_chq_cuentacheque ,
cast(lpad(bh.sucursal_cmc7, 4, '0') as bigint) as cod_chq_sucursalgirada,
cast(bh.cod_postal as int) as cod_chq_codigopostal,
...
mot_rech_depo as ds_chq_motivorechazodepo,
concat(bh.banco_cmc7, bh.sucursal_cmc7, bh.nro_cheque_cmc7, bh.nro_cuenta_cmc7, bh.cod_postal) as ds_chq_cmc7
from bi_corp_staging.acle_base_historica_fuera_canje bh
left join pedt008_use p1 on p1.pecdgent = '0072' and bh.suc_girada = p1.PECODOFI and substring(bh.cta_girada, 4) = substring(p1.penumcon, 4) and substring(bh.cta_girada, 2, 2) = p1.pecodpro
left join pedt008_use p2 on p2.pecdgent = '0072' and bh.suc_acred = p2.PECODOFI and substring(bh.cta_acred, 4) = substring(p2.penumcon, 4) and substring(bh.cta_acred, 2, 2) = p2.pecodpro
where bh.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cheques_Clearing') }}'
and bh.estado = 'R'
