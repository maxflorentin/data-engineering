...
--      ____                _                  ___   ___ 
--     / __ \ ____ _ _____ (_)_   __ ____     <  /  |__ \
--    / /_/ // __ `// ___// /| | / // __ \    / /   __/ /
--   / ____// /_/ /(__  )/ / | |/ // /_/ /   / /_  / __/ 
--  /_/     \__,_//____//_/  |___/ \____/   /_/(_)/____/ 
-------------------------------------------------------------------------
-- PASIVO_RESULT --
CREATE TEMPORARY TABLE pasivo_result AS  
SELECT 
...
--		OR RE.imp_efec_enc_mo != '0')
	AND RE.cod_contenido IN ('PLZ','CTA','COM','CTB','ROF') 
...
-------------------------------------------------------------------------
-- PASIVO_RESULT_G 
CREATE TEMPORARY TABLE pasivo_result_g AS
SELECT RE.*
...
AND gen.cod_ren_contenido IN ('PLZ','CTA','COM','CCL','CTB','ROF') ;

-------------------------------------------------------------------------
-- PASIVO_RESULT_GC 
CREATE TEMPORARY TABLE pasivo_result_gc AS
SELECT RE.*	
	, cli.cod_ren_vincula
	, cli.cod_per_tipopersona
	, cli.ds_per_nombre_apellido
FROM pasivo_result_g RE
...
-------------------------------------------------------------------------
-- RESULT_PASIVO (+ JERARQUIAS)
DROP TABLE IF EXISTS result_pasivo ;
CREATE TEMPORARY TABLE result_pasivo AS
SELECT RE.* ,
	adn.cod_ren_areanegociopadre id_division , -- 20201013 -- OK
...
---------------------------------------------------------------
LEFT JOIN bi_corp_common.dim_ren_areanegocioctr adn2 ON --20201001
	RE.cod_area_negocio = adn2.cod_ren_area_negocio -- 20201013
----------------------------------------------------------------	
LEFT JOIN bi_corp_common.dim_ren_jeareanegocioctr adn ON 
	RE.cod_area_negocio = adn.cod_ren_areanegociohijo
----------------------------------------------------------------	
LEFT JOIN producto_mis pm ON --20201001
...
-------------------------------------------------------------------------
-- RESULT_PASIVO_I 
DROP TABLE IF EXISTS result_pasivo_i ;
...
SELECT 
	...
	id_division AS cod_ren_division , -- 20201001
	cod_producto_mis , -- producto_generic
	idf_pers_ods ,
	CASE WHEN cod_ren_producto_niv_3 = 'BG2'THEN SUM(fc_ren_resultado_total_ml) ELSE 0 END AS fc_ren_margenmes ,
	...
	cod_gestor_prod	 ,
	cod_gestor	 ,
	CASE WHEN cod_ren_producto_niv_3 = 'BG2' THEN (SUM(fc_ren_ing_per_ml)
										+ SUM(fc_ren_imp_ing_cap_ml)
										+ SUM(fc_ren_sdo_cap_med_ml)
										+ SUM(fc_ren_sdo_med_int_ml)
										+ SUM(fc_ren_sdo_med_reajuste_ml)
										+ SUM(fc_ren_sdo_med_insolv_ml)) ELSE 0 END AS fc_ren_sdbsfm ,
	CASE WHEN cod_ren_producto_niv_3 = 'BG2' AND cod_ren_cta_resultados_niv_9 = '1031' THEN SUM(fc_ren_resultado_total_real_ml) ELSE 0 END AS fc_ren_interes ,
	CASE WHEN cod_ren_producto_niv_3 = 'BG2' AND cod_ren_cta_resultados_niv_9 = '1032' THEN SUM(fc_ren_resultado_total_real_ml) ELSE 0 END AS fc_ren_comfin ,
	CASE WHEN cod_ren_producto_niv_3 = 'BG2' AND cod_ren_cta_resultados_niv_8 = '1051' THEN SUM(fc_ren_resultado_total_real_ml) ELSE 0 END AS fc_ren_comnofin ,
	cod_ren_vincula ,
	CASE WHEN cod_ren_producto_niv_3 = 'BG2' AND cod_ren_cta_resultados_niv_8 = '1052' THEN SUM(fc_ren_resultado_total_real_ml) ELSE 0 END AS fc_ren_rof ,
    ...
    cod_ren_primercorporativo AS area_negocio_corp , 
	cod_producto_gest ,
	cod_segmento_gest ,
	...
	CASE WHEN cod_ren_producto_niv_3 = 'BG2' AND cod_ren_cta_resultados_niv_8 = '1054' THEN SUM(fc_ren_resultado_total_real_ml) ELSE 0 END AS fc_ren_orex ,
...
SELECT RE.cod_ren_unidad ,
	...
	RE.cod_gestor_prod ,
	RE.cod_gestor ,
	NULL , -- RK.id_cto_bdr ,  -- devolucion_españa
	...
	'20200630v1' ds_ren_ficheromis ,
	RE.dt_ren_reestruccontrato ,
	RE.dt_ren_fec_extr_datos ,
	RE.flagneteo ,
	SUM(RE.fc_ren_orex) ,
	NULL , -- RI.s_provision_amount ,  -- devolucion_españa
