...
WITH max_margen_resultado AS (
	...
	SELECT cod_ren_contenido
		...
		, MAX(IF(cod_ren_contenido IN ('PLZ','CTA'), 'P', IF(cod_ren_contenido IN ('PRE','CRE','ARF','CCO','ROF'), 'A', IF(cod_ren_producto_niv_3 = 'BG1', 'A', IF(cod_ren_producto_niv_3 = 'BG2', 'P', NULL)))))  perimetro
	FROM bi_corp_common.bt_ren_result
	WHERE partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_to', dag_id='LOAD_CMN_RORAC_inputs') }}' 
...
WHERE br.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_to', dag_id='LOAD_CMN_RORAC_inputs') }}' 
...
WITH result_saldos AS (
	...
	FROM bi_corp_common.bt_ren_result WHERE partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_to', dag_id='LOAD_CMN_RORAC_inputs') }}'
		AND cod_ren_cta_resultados_niv_6 NOT IN ('1073', '1074', '1075')
		AND cod_ren_cta_resultados_niv_3 NOT IN ('99000', '1091')
		AND cod_ren_cta_resultados_niv_7 != '1064'
		AND cod_ren_cta_resultados_niv_5 != '1081'
...
WHERE s3.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_to', dag_id='LOAD_CMN_RORAC_inputs') }}' 
...
WITH generic AS (
	...
	WHERE gr.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='month_to', dag_id='LOAD_CMN_RORAC_inputs') }}'
...
SELECT s6.cod_ren_contenido
		...
		, IF(s6.perimetro IS NULL AND SUBSTRING(s6.cod_ren_producto_gest, 1, 2) = 'C+', 'A'
			, IF(s6.perimetro IS NULL AND SUBSTRING(s6.cod_ren_producto_gest, 1, 2) = 'C-', 'P', s6.perimetro)) perimetro 
...
WITH saldo_ifrs AS (
	...
	WHERE periodo = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day_arda', dag_id='LOAD_CMN_RORAC_inputs') }}'
...
WITH rs_activo AS (
	...
	LEFT JOIN 
		...
		WHERE 	partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='LOAD_CMN_RORAC_inputs') }}' ) SE
    ...
    SELECT DISTINCT cod_ren_pers_ods
        , CASE WHEN SUBSTRING(pesegcal, 1, 1) IN ("A", "B", "C", "S") AND cod_baremo_global = "00006" THEN 1200009 
	         WHEN SUBSTRING(pesegcal, 1, 1) IN ("A", "B", "C", "S") AND cod_baremo_global IN ("00013", "00182", "00183") THEN 1200041
	         WHEN SUBSTRING(pesegcal, 1, 1) IN ("A", "B", "C", "S") AND cod_baremo_global = "00017" THEN 1200073
	         WHEN SUBSTRING(pesegcal, 1, 1) IN ("A", "B", "C", "S") AND cod_baremo_global NOT IN ("00017", "00006", "00013", "00182", "00183") THEN 1200105
	         WHEN SUBSTRING(pesegcal, 1, 2) IN ("P1", "C1") THEN 1200137
	         WHEN SUBSTRING(pesegcal, 1, 2) = "P2" THEN 1200169
	         WHEN SUBSTRING(pesegcal, 1, 2) IN ("EM", "G1", "G2") THEN 1200201
	         WHEN SUBSTRING(pesegcal, 1, 2) IN ("IU", "IP") THEN 1200233
	         WHEN CAST(cod_baremo_local AS INT) = 1 AND SUBSTRING(pesegcal, 1, 2) IN ("F2", "OF", "GL", "LA", "LO", "MU", "OT", "FI", "F1") THEN 1040176
	         WHEN CAST(cod_baremo_local AS INT) = 11 AND SUBSTRING(pesegcal, 1, 2) IN ("F2", "OF", "GL", "LA", "LO", "MU", "OT", "FI", "F1") THEN	1200290
	         WHEN CAST(cod_baremo_local AS INT) = 17 AND SUBSTRING(pesegcal, 1, 2) IN ("F2", "OF", "GL", "LA", "LO", "MU", "OT", "FI", "F1") THEN	1200322
	         WHEN SUBSTRING(pesegcal, 1, 2) = "PU" AND cod_ren_divisa_mis = "ARS"  THEN 1200354
	         WHEN SUBSTRING(pesegcal, 1, 2) = "PU" AND cod_ren_divisa_mis <> "ARS"  THEN 1040181 ELSE 9999999 END id_ru  
	    , pesegcal 
	    , cod_baremo_local
	    , cod_baremo_global
	    , cod_producto
...
SELECT cod_ren_contrato_rorac
	...
	, perimetro
	, fc_ifrs_provision
	, CASE WHEN TRIM(cod_ren_contenido) IN ('CCL','CTB') AND TRIM(cod_ren_area_negociocorp) = 'AN01020210' AND TRIM(cod_ren_divisa_mis) = 'ARS' THEN 1200201
			  WHEN TRIM(cod_ren_contenido) IN ('CCL','CTB') AND TRIM(cod_ren_area_negociocorp) = 'AN01020210' AND TRIM(cod_ren_divisa_mis) != 'ARS' THEN 1200649
			  WHEN TRIM(cod_ren_contenido) IN ('CCL','CTB') AND TRIM(cod_ren_area_negociocorp) = 'AN01040220' AND TRIM(cod_ren_divisa_mis) = 'ARS' THEN 1200233
			  WHEN TRIM(cod_ren_contenido) IN ('CCL','CTB') AND TRIM(cod_ren_area_negociocorp) = 'AN01040220' AND TRIM(cod_ren_divisa_mis) != 'ARS' THEN 1200681
			  WHEN TRIM(cod_ren_contenido) IN ('CCL','CTB') AND TRIM(cod_ren_area_negociocorp) = 'AN01020410' AND TRIM(cod_ren_divisa_mis) = 'ARS' THEN 1200137
			  WHEN TRIM(cod_ren_contenido) IN ('CCL','CTB') AND TRIM(cod_ren_area_negociocorp) = 'AN01020410' AND TRIM(cod_ren_divisa_mis) != 'ARS' THEN 1200585
			  WHEN TRIM(cod_ren_contenido) IN ('CCL','CTB') AND TRIM(cod_ren_area_negociocorp) = 'AN01020200' AND TRIM(cod_ren_divisa_mis) = 'ARS' THEN 1200169
			  WHEN TRIM(cod_ren_contenido) IN ('CCL','CTB') AND TRIM(cod_ren_area_negociocorp) = 'AN01020200' AND TRIM(cod_ren_divisa_mis) != 'ARS' THEN 1200617
			  WHEN TRIM(cod_ren_contenido) IN ('CCL','CTB') AND TRIM(cod_ren_area_negociocorp) IN ('AN01010500','AN01010201') THEN 1200425
