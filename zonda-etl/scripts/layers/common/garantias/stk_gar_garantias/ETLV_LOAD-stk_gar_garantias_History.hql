...
select gttcvab_vab_cod_divisa,gttcvab_vab_imp_total,gttcvab_vab_num_bien, gttcvab_vab_timest_umo
 from (select row_number() OVER (partition by gttcvab_vab_num_bien order by gttcvab_vab_timest_umo desc) as orden,
      ...
      where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}' and gttcvab_vab_tip_valor = '003')esc where orden=1
),
VAB AS (
select gttcvab_vab_cod_divisa,gttcvab_vab_imp_total,gttcvab_vab_num_bien, gttcvab_vab_timest_umo
 from (select row_number() OVER (partition by gttcvab_vab_num_bien order by gttcvab_vab_timest_umo desc) as orden,
         gttcvab_vab_cod_divisa ,
 	     ...
 	     gttcvab_vab_timest_umo
  	  FROM bi_corp_staging.gtdtvab
      where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}' and gttcvab_vab_tip_valor = '001')v where orden=1
),
VAB_ORI AS (
select gttcvab_vab_cod_divisa,gttcvab_vab_imp_total,gttcvab_vab_num_bien,gttcvab_vab_timest_umo
...
select gttcvab_vab_cod_divisa,gttcvab_vab_imp_total,gttcvab_vab_num_bien,gttcvab_vab_timest_umo
 from (select row_number() OVER (partition by gttcvab_vab_num_bien order by gttcvab_vab_timest_umo desc) as orden,
         gttcvab_vab_cod_divisa ,
 	     ...
 	     gttcvab_vab_timest_umo
  	  FROM bi_corp_staging.gtdtvab
      where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}' and gttcvab_vab_tip_valor = '015')uva where orden=1
),
HIP_UVA_ORI AS (
select garantia, bien, divisa, valor_ori, fecha_alta
     from  (select row_number() OVER (partition by gttchis_his_num_garantia order by gttchis_his_num_sechisto asc) as orden,
      ...
      where partition_date=IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}'<'2020-08-24','2020-08-24','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}')
...
select gttcvab_vab_cod_divisa,gttcvab_vab_imp_total,gttcvab_vab_num_bien,gttcvab_vab_timest_umo
 from (select row_number() OVER (partition by gttcvab_vab_num_bien order by gttcvab_vab_timest_umo desc) as orden,
         gttcvab_vab_cod_divisa ,
 	     ...
 	     gttcvab_vab_timest_umo
  	  FROM bi_corp_staging.gtdtvab
      where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}' and gttcvab_vab_tip_valor = '002')tas where orden=1
),
TASA_ORI AS(
select gttcvab_vab_cod_divisa,gttcvab_vab_imp_total,gttcvab_vab_num_bien,gttcvab_vab_timest_umo
 from (select row_number() OVER (partition by gttcvab_vab_num_bien order by gttcvab_vab_timest_umo asc) as orden,
...
select gttcvab_vab_cod_divisa,gttcvab_vab_imp_total,gttcvab_vab_num_bien, gttcvab_vab_timest_umo
 from (select row_number() OVER (partition by gttcvab_vab_num_bien order by gttcvab_vab_timest_umo desc) as orden,
         gttcvab_vab_cod_divisa ,
 	     ...
 	     gttcvab_vab_timest_umo
  	  FROM bi_corp_staging.gtdtvab
      where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}' and gttcvab_vab_tip_valor = '014')val where orden=1
),
COTIZACIONES AS(
SELECT tcdt081_cddiviss	cod_divisa, partition_Date, (tcdt081_cambfix/100) as imp_cambio_fijo_vigente,tcdt081_fhcambio, concat(substr(tcdt081_fhcambio, 1,4), substr(tcdt081_fhcambio, 6,2), substr(tcdt081_fhcambio, 9,2)) as fec_cambio_yyyymmdd
				FROM bi_corp_staging.maltc_tcdt081
				WHERE partition_Date= IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}'<'2020-08-24','2020-08-24','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}')
				and tcdt081_inddivbi = 'D' and tcdt081_indcot = 'S' and tcdt081_segmento = ''
                and tcdt081_fhcambio ='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}'
),
UVA AS(
select tcdt090_valor uva , tcdt090_tcfechin , tcdt090_tcfechfi fecha
from bi_corp_staging.maltc_tcdt090
where tcdt090_coefici='UVA' and
partition_date= IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}'<'2020-10-13','2020-10-13','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}')
...
partition_date= IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}'<'2020-10-13','2020-10-13','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}')
 ...
 where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}'
 ),
 ...
 select gttcrgb_rgb_num_garantia,gttcrgb_rgb_num_bien, gttcrgb_rgb_fec_finvali,gttcrgb_rgb_fec_inivali,gttcrgb_rgb_por_biegaran
  from (select row_number() OVER (partition by gttcrgb_rgb_num_garantia, gttcrgb_rgb_num_bien order by gttcrgb_rgb_timest_umo desc) as orden,
       ...
       where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}'
       and gttcrgb_rgb_fec_finvali> '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History')}}') rg
       where orden=1
 ),
   RGB_NUP AS (
  select gttcrgb_rgb_num_garantia,gttcrgb_rgb_num_bien, gttcrgb_rgb_fec_finvali,gttcrgb_rgb_fec_inivali,gttcrgb_rgb_por_biegaran
   from (select row_number() OVER (partition by gttcrgb_rgb_num_garantia, gttcrgb_rgb_num_bien order by gttcrgb_rgb_timest_umo desc) as orden,
          gttcrgb_rgb_num_garantia ,
   	     ...
   	     gttcrgb_rgb_por_biegaran
    	  FROM bi_corp_staging.gtdtrgb
        where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}') rgn
        where orden=1
  ),
  ...
  RTG_PAIS AS(
 		...
 		ON  (P01.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}'
 		AND aux_garant_calif_pais.partition_date=IF('{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_last_working_day', dag_id='LOAD_CMN_GARANTIAS_History') }}' < '2020-02-28', '2020-02-28','{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_last_working_day', dag_id='LOAD_CMN_GARANTIAS_History') }}' )
 ...
 where fecha_informacion=IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}' < '2020-06-11', '2020-06-11','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}') and pa.uva='true' and categoria_particulares='HIPOTECARIO'

 union all
  select codigo_producto,codigo_subproducto
  from  bi_corp_staging.risksql5_productos P
 where fecha_informacion=IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}' < '2020-06-11', '2020-06-11','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}') and descripcion  LIKE '%CER%' and categoria_particulares='HIPOTECARIO')
...
IF (vab_ori.gttcvab_vab_cod_divisa !='ARS' OR tasa_ori.gttcvab_vab_cod_divisa !='ARS',
   CASE WHEN vab_ori.gttcvab_vab_cod_divisa IS NOT NULL
   		THEN cotizaciones_rgb.tcdt081_fhcambio
   		ELSE cotizaciones_tasa.tcdt081_fhcambio END,
...
(select *
 ...
 where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}'
  AND gttcrgo_rgo_fec_altrelac <= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}'
  and gttcrgo_rgo_fec_bajrelac>= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}' ) rgo
...
LEFT JOIN
 (select gttcrbc_rbc_num_bien,gttcrbc_rbc_num_persona
   from ( select row_number() OVER (partition by gttcrbc_rbc_num_bien, gttcrbc_rbc_num_persona order by gttcrbc_rbc_timest_umo desc) as orden,
        ...
        where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}' ) rg
        where orden=1 ) rbc
...
LEFT JOIN
	...
	FROM bi_corp_staging.tcdtgen dom WHERE dom.gentabla = '0009' AND partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}') b
...
LEFT JOIN cotizaciones AS cotizaciones_rgb ON (cotizaciones_rgb.tcdt081_fhcambio = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}' and cotizaciones_rgb.cod_divisa = vab_ori.gttcvab_vab_cod_divisa)
LEFT JOIN cotizaciones AS cotizaciones_rgo ON (cotizaciones_rgo.tcdt081_fhcambio = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}' and cotizaciones_rgo.cod_divisa = rgo.gttcrgo_rgo_contrato_rgo_cod_divcont)
LEFT JOIN cotizaciones AS cotizaciones_mae ON (cotizaciones_mae.tcdt081_fhcambio = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}' and cotizaciones_mae.cod_divisa = mae.gttcmae_mae_cod_divisa)
LEFT JOIN cotizaciones AS cotizaciones_tasa ON  (cotizaciones_tasa.tcdt081_fhcambio = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}' and cotizaciones_tasa.cod_divisa = tasa_ori.gttcvab_vab_cod_divisa)
...
LEFT JOIN bi_corp_bdr.aux_garant_calif_empresa rtg_empresa ON (rbc.gttcrbc_rbc_num_persona = rtg_empresa.alias_nup and rtg_empresa.partition_date =IF('{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_last_working_day', dag_id='LOAD_CMN_GARANTIAS_History') }}'< '2020-02-28','2020-02-28','{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month_last_working_day', dag_id='LOAD_CMN_GARANTIAS_History') }}'))
LEFT JOIN VAB_ORI_GAR gar_ori ON mae.gttcmae_mae_num_garantia= gar_ori.garantia

where mae.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}'  and mae.gttcmae_mae_tip_cobertur = '001'
...
IF (vab_ori.gttcvab_vab_cod_divisa !='ARS' OR tasa_ori.gttcvab_vab_cod_divisa !='ARS',
   CASE WHEN vab_ori.gttcvab_vab_cod_divisa IS NOT NULL
   		THEN cotizaciones_rgb.tcdt081_fhcambio
   		ELSE cotizaciones_tasa.tcdt081_fhcambio
   		END,
   CASE WHEN vab.gttcvab_vab_cod_divisa IS NOT NULL
   		THEN SUBSTRING(vab.gttcvab_vab_timest_umo,1,10)
   		ELSE SUBSTRING(tasa.gttcvab_vab_timest_umo,1,10) END)                             					 dt_gar_bien_fechaactualizacion,
...
where mae.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}'  and mae.gttcmae_mae_tip_cobertur = '002'
...
FROM
		(	SELECT
			...
			WHERE (cgr.partition_date = IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}' < '2020-05-20','2020-05-20' ,'{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}' ) )
	 	)  CONTRAG
	 	LEFT JOIN  bi_corp_staging.contratos_adsf_cf adsf on (adsf.isec_cta_cuenta=CONTRAG.id_cto
        AND  adsf.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='first_working_day', dag_id='LOAD_CMN_GARANTIAS_History') }}' )
        LEFT JOIN cotizaciones  ON (cotizaciones.tcdt081_fhcambio = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_GARANTIAS_History') }}'  and cotizaciones.cod_divisa = CONTRAG.cod_divisa)
    WHERE '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='LOAD_CMN_GARANTIAS_History') }}' > '2020-05-20'
