...
left join bi_corp_common.dim_inv_codigos_fondos f on f.cod_inv_fondo_depositaria = cast(s.cod_fondo as int) and f.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Inversiones') }}'
left join TEMP_SUC_OPERATIVA t on t.cod_inv_cuenta = cast(s.cta as int)
where s.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Inversiones') }}';