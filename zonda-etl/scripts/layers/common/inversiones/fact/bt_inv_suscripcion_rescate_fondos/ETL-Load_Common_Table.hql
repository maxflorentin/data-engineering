...
a.partition_date
    ...
    case when CONCAT(SUBSTRING(s.dtinput,1,4),'-',SUBSTRING(s.dtinput,5,2),'-',SUBSTRING(s.dtinput,7,2)) in ('9999-12-31', '0001-01-01','0000-00-00') then CAST(NULL as string)
    ...
    case when CONCAT(SUBSTRING(s.dtsolic,1,4),'-',SUBSTRING(s.dtsolic,5,2),'-',SUBSTRING(s.dtsolic,7,2)) in ('9999-12-31', '0001-01-01','0000-00-00') then CAST(NULL as string)
         else CONCAT(SUBSTRING(s.dtsolic,1,4),'-',SUBSTRING(s.dtsolic,5,2),'-',SUBSTRING(s.dtsolic,7,2)) end dt_inv_solic,
    case when CONCAT(SUBSTRING(s.dtconver,1,4),'-',SUBSTRING(s.dtconver,5,2),'-',SUBSTRING(s.dtconver,7,2)) in ('9999-12-31', '0001-01-01','0000-00-00') then CAST(NULL as string)
         else CONCAT(SUBSTRING(s.dtconver,1,4),'-',SUBSTRING(s.dtconver,5,2),'-',SUBSTRING(s.dtconver,7,2)) end dt_inv_conver,
    case when CONCAT(SUBSTRING(s.dtestorno,1,4),'-',SUBSTRING(s.dtestorno,5,2),'-',SUBSTRING(s.dtestorno,7,2)) in ('9999-12-31', '0001-01-01','0000-00-00') then CAST(NULL as string)
    ...
    case when CONCAT(SUBSTRING(s.dtultrgt,1,4),'-',SUBSTRING(s.dtultrgt,5,2),'-',SUBSTRING(s.dtultrgt,7,2)) in ('9999-12-31', '0001-01-01','0000-00-00') then CAST(NULL as string)
         else CONCAT(SUBSTRING(s.dtultrgt,1,4),'-',SUBSTRING(s.dtultrgt,5,2),'-',SUBSTRING(s.dtultrgt,7,2)) end dt_inv_ult_reg,
    case when CONCAT(SUBSTRING(s.dtbloqueio,1,4),'-',SUBSTRING(s.dtbloqueio,5,2),'-',SUBSTRING(s.dtbloqueio,7,2)) in ('9999-12-31', '0001-01-01','0000-00-00') then CAST(NULL as string)
    ...
    case when CONCAT(SUBSTRING(s.dtcomprom,1,4),'-',SUBSTRING(s.dtcomprom,5,2),'-',SUBSTRING(s.dtcomprom,7,2)) in ('9999-12-31', '0001-01-01','0000-00-00') then CAST(NULL as string)
    ...
    CAST(NULL as bigint) fc_inv_perc_comis,
    CAST(NULL as bigint) fc_inv_val_rgt_brt,
    CAST(NULL as bigint) fc_inv_val_rgt_liq,
    CAST(NULL as bigint) fc_inv_val_cust_rgt,
    CAST(NULL as bigint) fc_inv_val_cota_rgt,
    CAST(NULL as bigint) fc_inv_cotacao_indi,
    ...
    where s.partition_date >= date_sub('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Inversiones') }}',30)
      and s.partition_date <= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Inversiones') }}') a
where a.row_num = 1;
...
a.partition_date
    ...
    case when CONCAT(SUBSTRING(r.dtinput,1,4),'-',SUBSTRING(r.dtinput,5,2),'-',SUBSTRING(r.dtinput,7,2)) in ('9999-12-31', '0001-01-01','0000-00-00') then CAST(NULL as string)
    ...
    case when CONCAT(SUBSTRING(r.dtsolic,1,4),'-',SUBSTRING(r.dtsolic,5,2),'-',SUBSTRING(r.dtsolic,7,2)) in ('9999-12-31', '0001-01-01','0000-00-00') then CAST(NULL as string)
         else CONCAT(SUBSTRING(r.dtsolic,1,4),'-',SUBSTRING(r.dtsolic,5,2),'-',SUBSTRING(r.dtsolic,7,2)) end dt_inv_solic,
    case when CONCAT(SUBSTRING(r.dtconver,1,4),'-',SUBSTRING(r.dtconver,5,2),'-',SUBSTRING(r.dtconver,7,2)) in ('9999-12-31', '0001-01-01','0000-00-00') then CAST(NULL as string)
         else CONCAT(SUBSTRING(r.dtconver,1,4),'-',SUBSTRING(r.dtconver,5,2),'-',SUBSTRING(r.dtconver,7,2)) end dt_inv_conver,
    case when CONCAT(SUBSTRING(r.dtestorno,1,4),'-',SUBSTRING(r.dtestorno,5,2),'-',SUBSTRING(r.dtestorno,7,2)) in ('9999-12-31', '0001-01-01','0000-00-00') then CAST(NULL as string)
    ...
    CAST(NULL as bigint) cod_inv_oper_orig,
    CAST(NULL as bigint) cod_inv_resg_orig,
    CAST(NULL as bigint) cod_inv_ctod_orig,
    CAST(NULL as string) dt_inv_ult_reg,
    CAST(NULL as string) dt_inv_bloqueio,
    CAST(NULL as bigint) cod_inv_bloqueio,
    CAST(NULL as bigint) cod_inv_dias_ut_dec,
    CAST(NULL as bigint) fc_inv_cota_base,
    CAST(NULL as bigint) fc_inv_apl_liq,
    CAST(NULL as bigint) fc_inv_qt_cot_apl,
    CAST(NULL as bigint) fc_inv_sd_cot_atu,
    CAST(NULL as bigint) fc_inv_cota_liq_d0,
    CAST(NULL as bigint) fc_inv_cotacao_pact,
    CAST(NULL as string) dt_inv_comprom,
    0 flag_inv_apl_transf,
    cast(r.nr_rgt as bigint) cod_inv_nr_rgt,
    cast(r.nr_cert_dest as bigint) cod_inv_nr_cert_dest,
    case when CONCAT(SUBSTRING(r.dtpagto,1,4),'-',SUBSTRING(r.dtpagto,5,2),'-',SUBSTRING(r.dtpagto,7,2)) in ('9999-12-31', '0001-01-01','0000-00-00') then CAST(NULL as string)
    ...
    where r.partition_date >= date_sub('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Inversiones') }}',30)
      and r.partition_date <= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Inversiones') }}') a
where a.row_num = 1;
...
on (cast(df.cod_inv_fondo_depositaria as bigint) = f.cod_inv_fondo and
    df.partition_date= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Inversiones') }}')
...
on (f.cod_inv_canal = c.cod_inv_canal and f.cod_inv_agte = c.cod_inv_agente_colocador and
    c.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Inversiones') }}')
...
on (f.cod_inv_canal_dg = cdg.cod_inv_canal and f.cod_inv_agte_dg = cdg.cod_inv_agente_colocador and
    cdg.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Inversiones') }}')
...
on (f.cod_inv_canal_cd = ccd.cod_inv_canal and f.cod_inv_agte_cd = ccd.cod_inv_agente_colocador and
    ccd.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Inversiones') }}')
where f.partition_date >= date_sub('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Inversiones') }}',7) and f.partition_date <= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Inversiones') }}';