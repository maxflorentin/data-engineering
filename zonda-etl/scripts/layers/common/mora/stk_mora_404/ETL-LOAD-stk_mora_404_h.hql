...
SELECT
case when w.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='LOAD_CMN_MORA_History') }}' then '{{ ti.xcom_pull(task_ids='InputConfig', key='month', dag_id='LOAD_CMN_MORA_History') }}' else null end AS periodo,
...
LEFT JOIN bi_corp_common.stk_per_personas pe on pe.cod_per_nup = cast(trim(w.waguxdex_num_persona) as bigint) and pe.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MORA_History') }}'
LEFT JOIN bi_corp_staging.risksql5_segmentos s ON trim(pe.cod_per_segmentoduro) = trim(s.ramo) and s.fecha_informacion = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MORA_History') }}'
LEFT JOIN bi_corp_staging.risksql5_productos p ON trim(w.waguxdex_cod_producto) = trim(p.codigo_producto) and trim(w.waguxdex_cod_subprodu) = trim(p.codigo_subproducto)  and p.fecha_informacion = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MORA_History') }}'
WHERE w.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MORA_History') }}' and
...
where c.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MORA_History') }}'
and c.periodo = (case when c.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_working_day', dag_id='LOAD_CMN_MORA_History') }}' then  '{{ ti.xcom_pull(task_ids='InputConfig', key='month', dag_id='LOAD_CMN_MORA_History') }}' else '{{ ti.xcom_pull(task_ids='InputConfig', key='previous_month', dag_id='LOAD_CMN_MORA_History') }}' end)
...
partition(partition_date= '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MORA_History') }}' )
