...
select
id_correspondencia															ID_MYA_CORRESPONDENCIA,
...
pro.partition_Date= IF("{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-History') }}" < "2020-09-02","2020-09-02", "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-History') }}"))
left join
(SELECT *
 FROM bi_corp_staging.rio74_tipo_documento
  WHERE partition_Date=IF("{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-History') }}" < "2020-09-02","2020-09-02","{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-History') }}" )
  AND CODIGO_CORREO IN ('2','3','4')) ti on di.tipo_documento= ti.CODIGO_CORREO
where
di.partition_Date="{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-History') }}"
