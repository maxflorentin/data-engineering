...
select
DISTINCT lo.msg_id 														             	    COD_MYA_MENSAJE,
...
left join bi_corp_staging.rio74_tmya_message me on (me.msg_id=lo.msg_id and me.partition_Date="{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-Daily') }}")
left join  bi_corp_staging.rio74_tmya_message_request_data req on (req.msg_req_id=me.msg_req_id and
        req.partition_date BETWEEN "{{ ti.xcom_pull(task_ids='InputConfig', key='previous_date_from', dag_id='LOAD_CMN_MYA-Daily') }}" AND
                                    "{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-Daily') }}")
left join  bi_corp_staging.rio74_entitlement ent on (ent.id_entitlement=lo.id_entitlement and
ent.partition_date=IF("{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-Daily') }}" < "2020-08-03","2020-08-03","{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-Daily') }}"))
left join bi_corp_staging.rio74_tmya_status_log sta on ( sta.id_estado=lo.id_estado and
sta.partition_date=IF("{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-Daily') }}" < "2020-08-03","2020-08-03","{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-Daily') }}"))
where lo.partition_date="{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-Daily') }}"

UNION ALL

select
DISTINCT lo_cd.msg_id 														             	    COD_MYA_MENSAJE,
...
left join bi_corp_staging.rio74_tmya_message_cd me_cd on (me_cd.msg_id=lo_cd.msg_id and me_cd.partition_Date="{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-Daily') }}")
left join  bi_corp_staging.rio74_tmya_message_request_data_cd req_cd on (req_cd.msg_req_id=me_cd.msg_req_id and req_cd.partition_date="{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-Daily') }}")
...
sta.partition_date=IF("{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-Daily') }}" < "2020-08-03","2020-08-03","{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-Daily') }}"))
where lo_cd.partition_date="{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MYA-Daily') }}"
