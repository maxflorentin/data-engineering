...
SELECT
DISTINCT est.id_estadistica 																						id_mob_estadistica,
...
(SELECT *
 ...
 partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MOB') }}') est
LEFT JOIN bi_corp_staging.rio78_transaccion tr on (tr.transaccion=est.transaccion and tr.partition_date=IF('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MOB') }}'< '2020-06-17', '2020-06-17','{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MOB') }}' ))
LEFT JOIN  bi_corp_staging.rio78_dispositivo dis on (dis.id_terminal= est.id_terminal and  dis.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_MOB') }}')
