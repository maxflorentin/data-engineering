...
select
tran.transaccion		 																	cod_obp_transaccion ,
...
left join bi_corp_staging.rio147_hb_desc_trx descrip on (descrip.transaccion= tran.transaccion and descrip.partition_Date=IF("{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_OBP-History') }}"< '2020-10-07','2020-10-07',"{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_OBP-History') }}"))
where tran.partition_date="{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_OBP-History') }}"
