...
LEFT JOIN bi_corp_staging.rio154_enc_estado EE
	...
	AND EE.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Sucursales') }}'
-------------------------------------------------------
LEFT JOIN bi_corp_staging.rio154_enc_turno_ext EX
	ON  EX.id_turno = ET.id_turno
	AND EX.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Sucursales') }}'
-------------------------------------------------------MOTIVO
LEFT JOIN bi_corp_staging.rio154_motivo_visita_v2 MV
	ON MV.id_motivo = ET.id_motivo
	AND MV.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Sucursales') }}'
-------------------------------------------------------SUCURSAL
LEFT JOIN bi_corp_staging.rio154_sucursalrio SUC
	ON SUC.codigo  = ET.id_suc
	AND SUC.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Sucursales') }}'
-------------------------------------------------------DS_TIPODOC
LEFT JOIN
