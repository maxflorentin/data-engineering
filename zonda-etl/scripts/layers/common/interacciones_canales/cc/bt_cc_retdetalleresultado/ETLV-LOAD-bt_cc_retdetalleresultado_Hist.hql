...
LEFT JOIN bi_corp_staging.rio3_tbl_encuesta en on (re.encuesta= en.codigo and en.partition_date='2021-04-28' )
LEFT JOIN bi_corp_staging.rio3_tbl_pregunta pre on (re.pregunta= pre.pregunta and re.encuesta=pre.encuesta and pre.partition_date='2021-04-28' )
LEFT JOIN bi_corp_staging.rio3_tbl_respuesta resp on (re.respuesta=resp.respuesta and re.encuesta=resp.encuesta
and re.pregunta=resp.pregunta and resp.partition_date= '2021-04-28')
LEFT JOIN bi_corp_staging.rio3_producto pro on (pro.codigo=re.producto and pro.partition_date='2021-04-28')
where re.partition_date="{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_CONTACT_CENTER-HIST') }}";
