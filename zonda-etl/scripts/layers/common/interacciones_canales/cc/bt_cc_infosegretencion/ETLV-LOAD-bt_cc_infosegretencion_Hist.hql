...
SELECT
         CASE WHEN trim(crec_fe_proceso)='null'
         ...
         CAST(cacn_nu_nup AS BIGINT)																				cod_per_nup,
          ...
          WHERE partition_date="2020-10-20"
          ...
          AND cacn_nu_cedula_rif = cace_cacn_nu_cedula_rif and cli.partition_date="2020-10-20")

          LEFT JOIN bi_corp_staging.rio6_cart_usuarios us ON (caus_cd_usuario= crec_cd_usuario and us.partition_date="2020-10-20")
          LEFT JOIN bi_corp_staging.rio6_cart_sucursal_banco suc ON (capj_cd_sucursal = crec_cd_sucursal_emision and suc.partition_date="2020-10-20")
          LEFT JOIN bi_corp_staging.rio6_cart_canales can ON (canl_cd_canal = capj_canl_cd_canal and can.partition_date="2020-10-20")
          LEFT JOIN bi_corp_staging.rio6_cart_ramos_polizas pol ON (carp_cd_ramo = crec_cd_ramo_nuevo and pol.partition_date="2020-10-20")
          LEFT JOIN (
          	SELECT *
          	FROM bi_corp_staging.rio6_cg_ref_codes
          	WHERE rv_domain = 'CART_CERTIFICADOS.CACE_ST_CERTIFICADO'
          	and partition_date="2020-10-20") a
          	ON (trim(a.rv_low_value) = trim(cace_st_certificado))
          LEFT JOIN (
          	SELECT *
          	FROM bi_corp_staging.rio6_cg_ref_codes
          	WHERE rv_domain = 'RETENCION_RESULTADOS_POSITIVOS'
          	and partition_date="2020-10-20") b
          	ON (b.rv_high_value = cres_cd_resultado )
          LEFT JOIN (
          	SELECT *
          	FROM bi_corp_staging.rio6_cg_ref_codes
          	WHERE rv_domain = 'RETENCION_CAUSA_BAJA'
          	and partition_date="2020-10-20") c
