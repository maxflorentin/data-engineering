...
left join bi_corp_staging.rio3_canal ca on (ca.codigo=op.canal and ca.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_CONTACT_CENTER-HIST') }}')
left join bi_corp_staging.rio3_tbl_canal_comunicacion com on (com.codigo=op.canal_comunicacion and com.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_CONTACT_CENTER-HIST') }}')
left join bi_corp_staging.rio3_tmk_sesionxoperacion ses on (ses.operacion= op.operacion and ses.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_CONTACT_CENTER-HIST') }}')
left join  bi_corp_staging.rio3_origen ori on (ori.codigo=op.origen and ori.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_CONTACT_CENTER-HIST') }}')
left join  bi_corp_staging.rio3_gestion ges on (ges.codigo=op.gestion and ges.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_CONTACT_CENTER-HIST') }}')
Where op.partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_CONTACT_CENTER-HIST') }}';
