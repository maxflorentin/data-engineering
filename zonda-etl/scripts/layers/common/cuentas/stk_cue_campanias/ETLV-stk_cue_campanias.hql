...
-- Busco para una cuenta, centro alta, divisa y partition la campaña vigente
...
AS
 ...
 WHERE cac.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Campania-Daily') }}'
 AND '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Campania-Daily') }}' >= cac.cac_fecha_desde
 AND '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Campania-Daily') }}' <= cac.cac_fecha_hasta;
