...
SELECT
    ...
    IF(lco.lco_producto_contab='',NULL,lco.lco_producto_contab)  as cod_cue_produ_contab,
    IF(lco.lco_subprodu_contab='',NULL,lco.lco_subprodu_contab)  as cod_cue_subprodu_contab,
	lco.LCO_PLAN as cod_cue_plan,
	lco.lco_concepto as cod_cue_concepto,
	IF(lco.lco_comision='', NULL,lco.lco_comision) as cod_cue_comision,
	lco.LCO_NUM_CONVENIO as cod_cue_num_convenio,
	IF(lco.lco_operacion='',NULL,lco.lco_operacion) as ds_cue_operacion,
	IF(lco.lco_zona='',NULL,lco.lco_zona) as ds_cue_zona,
	...
	LEFT JOIN santander_business_risk_arda.cotizaciones_bcra c on lco.lco_fecha_proliq = c.fec_info and lco.lco_divisa= c.cod_especie
	WHERE lco.LCO_CONCEPTO = 'MANT' AND lco.lco_divisa = 'USD' AND lco.lco_div_importe='USD'
	AND lco.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Comisiones_Cobradas-Hist2') }}'
