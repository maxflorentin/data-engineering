...
WHERE mov.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Mov_Cuentas') }}'
...
WHERE partition_date < '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Mov_Cuentas') }}'
	  and partition_date >= date_sub('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Mov_Cuentas') }}',7);
...
select
	  ...
	  A.mig_new_divisa as cod_cue_divisa
FROM bi_corp_staging.malbgc_zbdtmig A
WHERE A.partition_date = '2019-12-16' and mig_old_fech_baja='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Mov_Cuentas') }}';
...
WHERE A.partition_date < '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Mov_Cuentas') }}'
and A.partition_date >= date_sub('{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Mov_Cuentas') }}',7);
...
SELECT
    ...
    '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Mov_Cuentas') }}' as partition_date
...
GROUP BY 
	...
	'{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Mov_Cuentas') }}';	