...
DROP TABLE IF EXISTS pedt008_temp_clientes;
CREATE TEMPORARY TABLE pedt008_temp_clientes AS
...
where SUBSTRING(partition_date,1,7) ='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
AND pecalpar = 'TI'
...
where SUBSTRING(p.partition_date,1,7) ='{{ ti.xcom_pull(task_ids='InputConfig', key='month_partition_bdr', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
AND p.pecalpar = 'TI';



--De Las cuentas activas con clientes, obtengo el producto original cuando el actual es 70
...
AND b.fecha_informacion = case when '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}' <= '2020-06-11' then '2020-06-11' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_risksql5_productos', dag_id='LOAD_CMN_Cuentas_Hist2') }}' end
WHERE a.pecodpro ='70'	
...
CREATE TEMPORARY TABLE cant_firmantes AS
	...
	FROM
				...
				WHERE p08.pecodpro IN ('02', '03', '04', '05', '06', '07', '08', '98', '99', '14')
					  AND p08.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='last_calendar_day', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
				group by
						p08.penumper
				       ,p08.pecodofi
				       ,p08.pecodpro
...
WHERE
	partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
	and acu_fecha_inicio <='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
	and acu_fecha_vcto >='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
...
SELECT
	...
	,trim(plan.gen_datos) as ds_cue_tarifa
...
AND	mae.partition_date = aux.partition_date
...
LEFT JOIN bi_corp_staging.malpe_pedt042 pedt042
...
AND pedt042.partition_date = case when '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}' <= '2019-12-04' then '2019-12-04' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt042', dag_id='LOAD_CMN_Cuentas_Hist2') }}' end
LEFT JOIN bi_corp_staging.tcdtgen plan
ON trim(mae.plan_comi)=trim(plan.gen_clave)
AND plan.genTabla='0311'
AND plan.partition_date = case when '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}' <= '2020-01-08' then '2020-01-08' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_tcdtgen', dag_id='LOAD_CMN_Cuentas_Hist2') }}' end
LEFT JOIN bi_corp_staging.tcdtgen tarifa
ON trim(mae.plan_comi)=trim(tarifa.gen_clave)
AND tarifa.genTabla='0435'
AND tarifa.partition_date = case when '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}' <= '2020-01-08' then '2020-01-08' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_tcdtgen', dag_id='LOAD_CMN_Cuentas_Hist2') }}' end
LEFT JOIN bi_corp_staging.tcdtgen campania
ON trim(mae.plan_comi)=trim(campania.gen_clave)
AND campania.genTabla='0517'
AND campania.partition_date = case when '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}' <= '2020-01-08' then '2020-01-08' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_tcdtgen', dag_id='LOAD_CMN_Cuentas_Hist2') }}' end
...
AND garra_contratos.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_garra_wagucdex', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
...
AND movimientos_genuinos.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
...
AND paquetes_upgrade.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
...
WHERE mae.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
	  AND aux.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
...
SELECT
	...
	,case when trim(mae.ind_codbloq)='S' then 1 else 0 end flag_cue_bloqueo
	,case when LENGTH(trim(mae.campania))=0 then null else trim(mae.campania) end cod_cue_campania
	,trim(campania.gen_datos) as ds_cue_campania
	,trim(mae.num_convenio) as cod_cue_nro_convenio
	,trim(mae.plan_comi) as cod_cue_plan_comision
	,trim(plan.gen_datos) as ds_cue_plan_comision
	,case when trim(mae.saldo_dispue_sgn)='-' THEN cast(concat(substring(mae.saldo_dispue,0,13),'.',substring(mae.saldo_dispue,14,15)) AS decimal(18,2))*-1
		  else cast(concat(substring(mae.saldo_dispue,0,13),'.',substring(mae.saldo_dispue,14,15)) AS decimal(18,2)) end fc_cue_saldo_dispuesto
	,case when trim(mae.disp_total_acu_sgn)='-' THEN cast(concat(substring(mae.disp_total_acu,0,13),'.',substring(mae.disp_total_acu,14,15)) AS decimal(18,2))*-1
		  else cast(concat(substring(mae.disp_total_acu,0,13),'.',substring(mae.disp_total_acu,14,15)) AS decimal(18,2)) end fc_cue_disponible_total_acuerdos
...
AND mae.cuenta = paquetes_upgrade.cod_cue_cuenta
AND paquetes_upgrade.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
LEFT JOIN bi_corp_common.stk_cue_cuentas stk
ON	cast(mae.centro_alta as int) = stk.cod_suc_sucursal
AND	cast(mae.cuenta as bigint) = stk.cod_cue_cuenta
AND	mae.divisa = stk.cod_cue_divisa
AND trim(mae.producto) = stk.cod_cue_producto
AND trim(mae.subprodu) = stk.cod_cue_subproducto
AND	stk.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
WHERE mae.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
	  AND aux.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}'	
