...
where partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='max_malpe_pedt008', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
AND pecalpar = 'TI'
...
where p.partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='max_malpe_pedt008', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
AND p.pecalpar = 'TI';
...
AND b.fecha_informacion = case when '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}' < '2021-05-04' then '2021-05-04' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_risksql5_productos', dag_id='LOAD_CMN_Cuentas_Hist2') }}' end
...
CREATE TEMPORARY TABLE cant_firmantes AS
	...
	FROM
				...
				WHERE p08.pecodpro IN ('02', '03', '04', '05', '06', '07', '08', '98', '99', '14')
					  AND partition_date ='{{ ti.xcom_pull(task_ids='InputConfig', key='max_malpe_pedt008', dag_id='LOAD_CMN_Cuentas_Hist2') }}'
				group by
						p08.penumper
				       ,p08.pecodofi
				       ,p08.pecodpro
...
WHERE  tcdtgen.partition_date = case when '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}' < '2020-01-08' then '2020-01-08' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_tcdtgen', dag_id='LOAD_CMN_Cuentas_Hist2') }}' end
	  AND genTabla='0311'
;
