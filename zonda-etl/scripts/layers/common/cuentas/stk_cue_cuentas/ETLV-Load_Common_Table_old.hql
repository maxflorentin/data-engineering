...
where partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt008', dag_id='LOAD_CMN_Cuentas_Hist') }}'
AND pecalpar = 'TI'
...
where p.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt008', dag_id='LOAD_CMN_Cuentas_Hist') }}'
AND p.pecalpar = 'TI';
...
DROP TABLE IF EXISTS max_fecha_informacion;
CREATE TEMPORARY TABLE max_fecha_informacion AS
select max(b.fecha_informacion) max_fecha_informacion
from bi_corp_staging.risksql5_productos b
where b.fecha_informacion >= date_sub(current_date,7)
 	  and b.fecha_informacion <=current_date;

DROP TABLE IF EXISTS risksql5_productos;
CREATE TEMPORARY TABLE risksql5_productos AS
SELECT a.*
FROM bi_corp_staging.risksql5_productos a
INNER JOIN max_fecha_informacion b
ON a.fecha_informacion=b.max_fecha_informacion
where a.fecha_informacion >= date_sub(current_date,7)
 	  and a.fecha_informacion <=current_date;
...
LEFT JOIN risksql5_productos b
...
CREATE TEMPORARY TABLE cant_firmantes AS
	...
	FROM
				...
				WHERE p08.pecodpro IN ('02', '03', '04', '05', '06', '07', '08', '98', '99', '14')
					  AND p08.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt008', dag_id='LOAD_CMN_Cuentas_Hist') }}'
				group by
						p08.penumper
				       ,p08.pecodofi
				       ,p08.pecodpro
...
WHERE p042.partition_date = case when '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}' < '2019-12-04' then '2019-12-04' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malpe_pedt042', dag_id='LOAD_CMN_Cuentas_Hist2') }}' end
;
...
WHERE  tcdtgen.partition_date = case when '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist') }}' < '2020-01-08' then '2020-01-08' else '{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_tcdtgen', dag_id='LOAD_CMN_Cuentas_Hist') }}' end
	  AND genTabla='0311'
GROUP BY trim(gen_clave),
		 trim(gen_datos)
;
...
WHERE
	partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist') }}'
	and acu_fecha_inicio <='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist') }}'
	and acu_fecha_vcto >='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist') }}'
...
AND garra_contratos.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_garra_wagucdex', dag_id='LOAD_CMN_Cuentas_Hist') }}'
...
AND mae.partition_date=movimientos_genuinos.partition_date
...
WHERE mae.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist') }}'
	  AND aux.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist') }}'
	  AND (movimientos_genuinos.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist') }}' OR  movimientos_genuinos.partition_date IS NULL)
	  AND (paquetes_upgrade.partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas_Hist2') }}' OR  paquetes_upgrade.partition_date IS NULL)
