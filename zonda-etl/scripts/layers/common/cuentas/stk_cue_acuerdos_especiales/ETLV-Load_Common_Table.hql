...
WHERE
	partition_date='{{ ti.xcom_pull(task_ids='InputConfig', key='max_partition_malbgc_bgdtacu', dag_id='LOAD_CMN_Cuentas') }}'
	and acu_fecha_inicio <='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas') }}'
	and acu_fecha_vcto >='{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='LOAD_CMN_Cuentas') }}'
;