...
def read_files(spark, sparkSQL, hadoop, configFile):
    ...
    if configFile.model == 'transferencias':
        if not fs.exists(hadoop.fs.Path(configFile.location[1])) and not fs.exists(hadoop.fs.Path(configFile.location[0])):
            final = sparkSQL.createDataFrame(spark.emptyRDD(), schema=configFile.schema)
        elif not fs.exists(hadoop.fs.Path(configFile.location[1])):
        ...
        elif not fs.exists(hadoop.fs.Path(configFile.location[0])):
            final = sparkSQL.read.format(configFile.extension).option('delimiter', configFile.delimiter).schema(configFile.schema).load(configFile.location[1])
        else:
    ...
    if len(final.head(1)) != 0:
        ...
        final = final.withColumn('partition_execution_date', f.current_date())
...
def rename_write_df(finalDF, configFile, spark_session):
    ...
    result.repartition(1).write.mode('overwrite').partitionBy("partition_date", "servicio", "partition_execution_date").parquet(configFile.destination)
