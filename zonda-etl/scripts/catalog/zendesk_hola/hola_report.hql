...
where tk.partition_date = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='ZENDESK_HOLA_REPORT') }}'
and to_date(updated_at) = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='ZENDESK_HOLA_REPORT') }}'
and status = 'solved'
and tk.group_id in ('360013313692', '360013314372')
and UCASE(tk.via ) LIKE ('%CHAT%')
...
and to_date(updated_at) = '{{ ti.xcom_pull(task_ids='InputConfig', key='partition_date', dag_id='ZENDESK_HOLA_REPORT') }}'
and status = 'solved'
and recipient LIKE ('%hola%')
and UCASE(tk.via) LIKE ('%EMAIL%')
