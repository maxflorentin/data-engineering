...
OPERATORS_ALLOWED_FIELDS = {
    ...
    'MySqlOperator': ['connection_id', 'sql', 'autocommit']
}
    ...
    def _build_dag(self):
            ...
            if (channels or emails) and (notify_on_error or notify_on_success):
                if channels:
                    ...
                    dag['code']['notifications'] = {'comments': 'notifications',
                                                    'blocks': [{'lines': 'channels = {}'.format(str(channels)),
                                                                'active': True},
                                                               {'lines': 'users = {}\n'.format(str(users)),
                                                                'active': True}
                                                               ]}
                    ...
                    if notify_on_error:
                        ...
                        notification_function_block.append({
                                                               'lines': 'slack_alert_error = ZondaSlackOperator(task_id="slack_alert_error", status=1, channels=channels, users=users)',
                                                               'active': True, 'indent': 1})
                        notification_function_block.append(
        ...
        for t in config_tasks:
            ...
            if operator.lower() == 'dummyoperator':
                ...
                connection_config = config.get('connection_id')
                autocommit_config = config.get('autocommit')
                sql_config = config.get('sql')
                ...
                config['autocommit'] = config.get('autocommit') or False
...
# dag definition
with DAG(dag_id='CORE_ColomboDAG',
    ...
    discover_files = PythonOperator(task_id='DiscoverDags',
