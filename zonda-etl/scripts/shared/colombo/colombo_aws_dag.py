...
SEARCH_PATH = os.getenv('ZONDA_DIR') + '/repositories'
SERVICE_USER = os.getenv('SERVICE_USER')
DEFAULT_SCHEMA_FILE = os.getenv('ZONDA_DIR') + '/repositories/zonda-etl/scripts/shared/colombo/' + 'schema.json'
TESTING_PREFIXES = ['_test', '_testing', 'test_', 'testing_']
ENVIRONMENT = os.getenv('GIT_BRANCH', 'master').lower()
OPERATORS_ALLOWED_FIELDS = {
    'ZondaMoveItOperator': ['connection_id', 'path', 'table', 'shift', 'regexp_date_files', 'filewatch_ttl_min',
                            'model', 'hdfs_host', 'hdfs_port', 'hdfs_user_name', 'delta'],
    'ZondaHDFSOperator': ['operation', 'local_path', 'remote_path', 'prefix', 'replace_prefix', 'overwrite',
                          'partitioned_by_date', 'remove_local_path', 'skip_errors', 'hdfs_source_path',
                          'hdfs_destination_path'],
    'ZondaS3Operator': ['operation', 'local_path', 'remote_path', 'prefix', 'replace_prefix', 'overwrite','s3_bucket',
                        'source_s3_conn_id', 'partitioned_by_date', 'remove_local_path', 'skip_errors'],
    'ZondaFileSensor': ['path', 'poke_interval', 'change_owner', 'timeout', 'file_size_in_bytes', 'ignored_ext',
                        'soft_fail'],
    'ZondaDagSensor': ['external_dag_id', 'external_task_id', 'execution_date', 'timeout', 'soft_fail'],
    'ZondaHiveOperator': ['hql', 'is_hql_file', 'connection_id', 'schema', 'delimiter', 'output_file', 'show_header',
                          'output_format', 'empty_file'],
    'ZondaSlackOperator': ['connection_id', 'text', 'status', 'channels'],
    ...
    'HiveOperator': ['hql', 'is_hql_file', 'connection_id', 'schema', 'mapred_queue', 'mapred_queue', 'mapred_job_name',
                     'mapred_queue_priority'],
    'SparkSubmitOperator': ['name', 'application', 'java_class', 'application_args', 'executor_cores',
                            'executor_memory', 'num_executors', 'files', 'connection_id', 'verbose', 'spark_binary',
                            'py_files', 'conf', 'jars'],
    'SFTPOperator': ['connection_id', 'local_filepath', 'remote_filepath', 'operation', 'confirm',
                     'create_intermediate_dirs'],
    'TriggerDagRunOperator': ['trigger_dag_id'],
    'ZondaTriggerDagRunOperator': ['trigger_dag_id', 'conf'],
    'SqoopOperator': ['connection_id', 'cmd_type', 'table', 'query', 'target_dir', 'append', 'file_type', 'columns',
                      'num_mappers', 'split_by', 'where', 'export_dir'],
    ...
    def _build_dag(self):
            ...
            if (channels or emails) and (notify_on_error or notify_on_success):
                if channels:
                    ...
                    dag['code']['notifications'] = {'comments': 'notifications',
                                                    'blocks': [{'lines': 'channels = {}\n'.format(str(channels)),
                                                                'active': True}]}

                    if notify_on_error:
                        error_notifier_def_name = "on_error_notifier(context)"
                        notification_function_block.append(
                            {'lines': 'def {}:'.format(error_notifier_def_name), 'active': True})
                        notification_function_block.append(
                            {'lines': '"""\nSend error notification alerts\n"""', 'active': True, 'indent': 1})
                        notification_function_block.append({
                                                               'lines': 'slack_alert_error = ZondaSlackOperator(task_id="slack_alert_error", status=1, channels=channels)',
                                                               'active': True, 'indent': 1})
                        notification_function_block.append(
                            {'lines': 'slack_response = slack_alert_error.execute(context=context)\n', 'active': True,
                             'indent': 1})
                        notification_function_block.append(
                            {'lines': 'return slack_response\n', 'active': True, 'indent': 1})
                    ...
                    if notify_on_success:
                        ...
                        notification_function_block.append(
                            {'lines': 'def {}:'.format(success_notifier_def_name), 'active': True})
                        ...
                        if notify_on_start and notify_on_finish:
                            notification_function_block.append({
                                                                   ...
                                                                   'active': True, 'indent': 1})
                            notification_function_block.append(
                                {'lines': "slack_response = True", 'active': True, 'indent': 1})
                            notification_function_block.append({
                                                                   'lines': "init_last_tasks = [t.task_id for t in context.get('dag').tasks if not t.downstream_task_ids or not t.upstream_task_ids]\n",
                                                                   'active': True, 'indent': 1})
                            notification_function_block.append(
                                {'lines': "if context.get('task').task_id in init_last_tasks:", 'active': True,
                                 'indent': 1})
                            notification_function_block.append({
                                                                   'lines': 'slack_alert_success = ZondaSlackOperator(task_id="slack_alert_success", status=0, channels=channels)',
                                                                   'active': True, 'indent': 2})
                            notification_function_block.append(
                                {'lines': 'slack_response = slack_alert_success.execute(context=context)\n',
                                 'active': True, 'indent': 2})
                            notification_function_block.append(
                                {'lines': 'return slack_response\n', 'active': True, 'indent': 1})
                            ...
                            notification_function_block.append(
                                {'lines': "slack_response = True", 'active': True, 'indent': 1})
                            notification_function_block.append({
                                                                   'lines': "init_tasks = [t.task_id for t in context.get('dag').tasks if not t.upstream_task_ids]\n",
                                                                   'active': True, 'indent': 1})
                            notification_function_block.append(
                                {'lines': "if context.get('task').task_id in init_tasks:", 'active': True, 'indent': 1})
                            notification_function_block.append({
                                                                   'lines': 'slack_alert_success = ZondaSlackOperator(task_id="slack_alert_success", status=0, channels=channels)',
                                                                   'active': True, 'indent': 2})
                            notification_function_block.append(
                            ...
                            notification_function_block.append(
                                {'lines': "slack_response = True", 'active': True, 'indent': 1})
                            notification_function_block.append({
                                                                   'lines': "last_tasks = [t.task_id for t in context.get('dag').tasks if not t.downstream_task_ids]\n",
                                                                   'active': True, 'indent': 1})
                            notification_function_block.append(
                                {'lines': "if context.get('task').task_id in last_tasks:", 'active': True, 'indent': 1})
                            notification_function_block.append({
                                                                   'lines': 'slack_alert_success = ZondaSlackOperator(task_id="slack_alert_success", status=0, channels=channels)',
                                                                   'active': True, 'indent': 2})
                            notification_function_block.append(
        ...
        if is_there_input_task:
            input_function_block = [{'lines': 'def get_input_config(**kwargs):', 'active': not self.is_subdag},
                                    ...
                                    {
                                        'lines': "tmp = ZondaGenOperator(task_id='GenerateVariables', input=default_input)",
                                        'active': not self.is_subdag, 'indent': 1},
                                    {'lines': "vars_generated = tmp.execute(context=kwargs)\n",
                                     'active': not self.is_subdag, 'indent': 1},
                                    {'lines': "default_input.update({k: v for k, v in conf.items() if v})",
                                     'active': not self.is_subdag, 'indent': 1},
                                    {'lines': "default_input.update({k: v for k, v in vars_generated.items() if v})\n",
                                     'active': not self.is_subdag, 'indent': 1},
                                    {'lines': "for k, v in default_input.items():", 'active': not self.is_subdag,
                                     'indent': 1},
                                    {'lines': "kwargs['ti'].xcom_push(key=k, value=v)\n", 'active': not self.is_subdag,
                                     'indent': 2},
                                    {'lines': "kwargs['ti'].xcom_push(key='all', value=default_input)",
                                     'active': not self.is_subdag, 'indent': 1},
        ...
        if loop_config:
            loop_block = {'lines': '\nfor {} in {}:'.format(loop_iterator,
                                                            [loop_config['values']] if isinstance(loop_config['values'],
                                                                                                  str) else loop_config[
                                                                'values']), 'active': True}
        ...
        for t in config_tasks:
            ...
            if operator.lower() == 'dummyoperator':
                ...
                # update import block
                import_op = 'from airflow.sensors.zonda_file_sensor import ZondaFileSensor\n'
                if import_op not in import_block:
                    import_block += import_op
                ...
                config['external_task_id'] = config.get('external_task_id') or 'None'
                ...
                if config.get('operation'):
                    if not config.get('operation').lower() in ('put', 'get'):
                        raise Exception(
                            "Invalid SFTP operation '{}' (values allowed: put, get)".format(config.get('operation')))
                else:
                ...
                # update import block
                import_op = 'from airflow.operators.zonda_trigger_dag_run_operator import ZondaTriggerDagRunOperator\n'
                if import_op not in import_block:
                    import_block += import_op
                ...
                if op_args_config and not isinstance(op_args_config, list):
                    raise Exception(
                        "Property 'op_args' in task configuration '{}' must be a list (positional arguments)".format(
                            name))
            ...
            if not is_subdag:
                ...
                args = {
                    ...
                    'dag': parent_dag_name,
                    'subdag': 'build_{}("{}", {}, {}, {})'.format(variable_name, subdag_name, 'start_date', 'None',
                                                                  'params')
                }
...
def get_repositories(**kwargs):
    ...
    for x in os.listdir(search_path):
        if x.find("zonda-etl") != -1:
            list_repositories.append(x)
    print("Repositories detected: {}".format(list_repositories))
...
def discover_dags(**kwargs):
    ...
    print("current repository: {}".format(kwargs['repository']))
    try:
        airflow_home = context_config.get('airflow_home') if context_config else kwargs.get('airflow_home')
        ...
        search_path = context_config.get('search_path') if context_config else kwargs.get('search_path')
        if not search_path:
            search_path = SEARCH_PATH
            print('Using default search path {}...'.format(search_path))
        else:
        ...
        repository_path = search_path + '/' + kwargs['repository']
        search_paths = [repository_path]
        dags_folder = os.path.abspath(airflow_home + '/dags/autogenerated')

        for search_path in search_paths:
            print("search_path: {}".format(search_path))
            repository = search_path.split('/')[-1]
            search_path = search_path + '/scripts'
            ...
            for source_filename in glob.iglob(search_path + '**/**', recursive=True):
                ...
                if os.path.isfile(source_filename) and ext in ['.yml', '.yaml'] and ((not any(
                        [x in source_filename.lower() for x in
                         TESTING_PREFIXES]) and ENVIRONMENT != 'development') or ENVIRONMENT == 'development'):
                    if first_file:
                        print(' |')
                        first_file = False
                    ...
                    try:
                        ...
                        destination_filename = source_filename.replace(search_path,
                                                                       dags_folder + '/' + repository).replace(ext,
                                                                                                               '.py')
                        ...
                        if dag.get_status() == 'OK':
                            ...
                            if dag_modified:
                                ...
                                print(' | +-- Status: {}'.format(dag.get_status()))
                                print(' | +-- Tasks: {}'.format(
                                    len(dag.dag.get('code', {}).get('tasks', {}).get('blocks', []))))
...
# start date
start_date = datetime.datetime(2019, 7, 1, tzinfo=pendulum.timezone('utc'))

# schedule
schedule = "*/15 * * * *"

# input variables
input = {"airflow_home": "Airflow home directory path (Default: $AIRFLOW_HOME)",
         "search_path": "Directory where the configuration files are (Default: $AIRFLOW_HOME/dags)", }

# default arguments
default_args = {"owner": "BI-Corp", "input": input, "depends_on_past": False, "wait_for_downstream": True,
                "provide_context": True, "start_date": start_date, "run_as_user": SERVICE_USER}

# dag definition
with DAG(dag_id='CORE_AWS_ColomboDAG',
         description="Discover configuration files and create corresponding DAGs automatically",
         default_args=default_args,
         schedule_interval=schedule,
         catchup=False) as dag:
    # operators
    dummy_init = DummyOperator(task_id='StartExecution', dag=dag)
    for e in ['zonda-etl','zonda-etl-risk']:
        discover_files = PythonOperator(task_id='DiscoverDags_' + e,
                                        python_callable=discover_dags,
                                        provide_context=True,
                                        op_kwargs={'repository': e})

        # dependencies
        dummy_init >> discover_files