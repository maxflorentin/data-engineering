...
class Unloader:
    def __init__(self, **kwargs):
        ...
        self.error_flow = kwargs.get('error_flow')
        if self.error_flow:
            self.error_flow_query = self._read_query(kwargs.get('error_flow_query'))
    ...
    def execute(self):
        try:
            if self._is_valid_query(self.query):
                ...
                df.coalesce(1).write.csv(path=self._get_ouput(), header=self.header, sep=self.delimiter)
                files = self._list_directory(self._get_ouput())

                self.session.conf.set('hive.exec.dynamic.partition.mode', 'nonstrict')

                # enable error_flow control
                if self.error_flow:

                    print("Define Results Table Schema")

                    log_table = "bi_corp_bdr.length_controls"
                    df_ef = self.session.sql("select * from {table} limit 0".format(table=log_table))
                    df_ef.printSchema()

                    print("Executing Queries")
                    query_ef_result = self.session.sql(self.error_flow_query)
                    df_ef = df_ef.unionByName(query_ef_result)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--error_flow", help="Indicates if the error flow logic be included in the script or not. Default: False", type=str2bool, default=False)
    parser.add_argument("--error_flow_query", help="Path of file where the error flow query to be executed is")
    ...
    if args.error_flow and not args.error_flow_query:
        raise Exception("Missing parameter: --error_flow_query")
